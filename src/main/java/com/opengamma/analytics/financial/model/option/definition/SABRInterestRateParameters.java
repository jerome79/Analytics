...
public class SABRInterestRateParameters implements VolatilityModel<double[]> {
  ...
  public SABRInterestRateParameters(final InterpolatedDoublesSurface alpha, final InterpolatedDoublesSurface beta, final InterpolatedDoublesSurface rho, final InterpolatedDoublesSurface nu,
    ...
    ArgChecker.notNull(alpha, "alpha surface");
    ArgChecker.notNull(beta, "beta surface");
    ArgChecker.notNull(rho, "rho surface");
    ArgChecker.notNull(nu, "nu surface");
    ArgChecker.notNull(sabrFormula, "SABR formula");
    _alphaSurface = alpha;
    _betaSurface = beta;
    _rhoSurface = rho;
    _nuSurface = nu;
  ...
  public double getAlpha(final DoublesPair expiryMaturity) {
    return _alphaSurface.getZValue(expiryMaturity.getFirst(), expiryMaturity.getSecond());
  }
  ...
  public double getBeta(final DoublesPair expiryMaturity) {
    return _betaSurface.getZValue(expiryMaturity.getFirst(), expiryMaturity.getSecond());
  }
  ...
  public double getRho(final DoublesPair expiryMaturity) {
    return _rhoSurface.getZValue(expiryMaturity.getFirst(), expiryMaturity.getSecond());
  }
  ...
  public double getNu(final DoublesPair expiryMaturity) {
    return _nuSurface.getZValue(expiryMaturity.getFirst(), expiryMaturity.getSecond());
  }
  ...
  public Double getVolatility(final double[] data) {
    ...
    ArgChecker.isTrue(data.length == 4, "data should have four components (expiry time, maturity, strike and forward");
  ...
  public double[] getVolatilityAdjoint(final double expiryTime, final double maturity, final double strike, final double forward) {
    ArgChecker.isTrue(_sabrFunction instanceof SABRHaganVolatilityFunction, "Adjoint volatility available only for Hagan formula");
