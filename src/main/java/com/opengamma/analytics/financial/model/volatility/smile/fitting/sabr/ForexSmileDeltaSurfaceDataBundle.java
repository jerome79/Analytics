...
public class ForexSmileDeltaSurfaceDataBundle extends SmileSurfaceDataBundle {
  ...
  public ForexSmileDeltaSurfaceDataBundle(final double[] forwards, final double[] expiries, final double[] deltas, final double[] atms, final double[][] riskReversals,
    ...
    ArgChecker.notNull(deltas, "delta");
    ArgChecker.notNull(forwards, "forwards");
    ArgChecker.notNull(expiries, "expiries");
    ArgChecker.notNull(atms, "at-the-money");
    ArgChecker.notNull(riskReversals, "risk reversal");
    ...
    ArgChecker.isTrue(_nExpiries == atms.length, "atms wrong length; have {}, need {}", atms.length, _nExpiries);
    final int n = deltas.length;
    ArgChecker.isTrue(n > 0, "need at least one delta");
    ArgChecker.isTrue(n == riskReversals.length, "wrong number of rr sets; have {}, need {}", riskReversals.length, n);
    ArgChecker.isTrue(n == strangle.length, "wrong number of strangle sets; have {}, need {}", strangle.length, n);
    for (int i = 0; i < n; i++) {
      ArgChecker.isTrue(_nExpiries == riskReversals[i].length, "wrong number of rr; have {}, need {}", riskReversals[i].length, _nExpiries);
      ArgChecker.isTrue(_nExpiries == strangle[i].length, "wrong number of strangles; have {}, need {}", strangle[i].length, _nExpiries);
    }
    _forwards = forwards;
    _expiries = expiries;
  ...
  public ForexSmileDeltaSurfaceDataBundle(final ForwardCurve forwardCurve, final double[] expiries, final double[] deltas, final double[] atms, final double[][] riskReversals,
    ...
    ArgChecker.notNull(expiries, "expiries");
    ArgChecker.notNull(atms, "atms");
    ArgChecker.notNull(riskReversals, "risk reversals");
    ArgChecker.notNull(strangle, "strangle");
    _nExpiries = expiries.length;
    ArgChecker.isTrue(_nExpiries == atms.length, "atms wrong length; have {}, need {}", atms.length, _nExpiries);
    final int n = deltas.length;
  ...
  public ForexSmileDeltaSurfaceDataBundle(final ForwardCurve forwardCurve, final double[] expiries, final double[][] strikes, final double[][] vols, final boolean isCallData) {
    ...
    ArgChecker.notNull(vols, "vols");
    _nExpiries = expiries.length;
    ArgChecker.isTrue(_nExpiries == strikes.length, "strikes wrong length; have {}, need {}", strikes.length, _nExpiries);
    ArgChecker.isTrue(_nExpiries == vols.length, "implied vols wrong length; have {}, need {}", vols.length, _nExpiries);
    for (int i = 0; i < _nExpiries; i++) {
      ArgChecker.isTrue(strikes[i].length == vols[i].length, "wrong number of volatilities; have {}, need {}", strikes[i].length, vols[i].length);
    }
    _forwardCurve = forwardCurve;
    _expiries = expiries;
    _strikes = strikes;
