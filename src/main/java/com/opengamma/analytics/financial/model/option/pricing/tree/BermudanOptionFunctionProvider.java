...
public class BermudanOptionFunctionProvider extends OptionFunctionProvider1D {
  ...
  public BermudanOptionFunctionProvider(final double strike, final double timeToExpiry, final int steps, final boolean isCall, final double[] exerciseTimes) {
    ...
    ArgChecker.notNull(exerciseTimes, "exerciseDates");

    _nTimes = exerciseTimes.length;

    ArgChecker.isTrue(steps + 1 >= _nTimes, "Number of steps is not large enough");
    for (int i = 0; i < _nTimes; ++i) {
      ArgChecker.isTrue(exerciseTimes[i] >= 0., "exerciseDates should be non-negative");
      ArgChecker.isTrue(exerciseTimes[i] <= timeToExpiry, "exerciseDates should be less than timeToExpiry");
    }
  ...
  private int[] timesToSteps(final int nSteps, final double dt) {
    ...
    for (int i = 0; i < nSteps + 1; ++i) {
      ...
      if (currentTime >= _exerciseTimes[j]) {
        ...
        if (j != 0) {
          ArgChecker.isFalse(steps[j] == steps[j - 1], "Number of steps is not large enough");
        }
        ++j;
      }
      if (j == _nTimes) {
