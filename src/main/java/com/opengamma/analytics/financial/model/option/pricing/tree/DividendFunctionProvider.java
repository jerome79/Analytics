...
public abstract class DividendFunctionProvider {
  ...
  public DividendFunctionProvider(final double[] dividendTimes, final double[] dividends) {
    ...
    ArgChecker.isTrue(nDiv == dividends.length, "Wrong data length");
    for (int i = 0; i < nDiv; ++i) {
      ArgChecker.isTrue(dividendTimes[i] > 0., "dividendTimes should be positive");
      ArgChecker.isTrue(Doubles.isFinite(dividendTimes[i]), "dividendTimes should be finite");
      ArgChecker.isTrue(dividends[i] > 0., "dividends should be positive");
      ArgChecker.isTrue(Doubles.isFinite(dividends[i]), "dividends should be finite");
    }
    for (int i = 1; i < nDiv; ++i) {
      ArgChecker.isTrue(dividendTimes[i] - dividendTimes[i - 1] > 0., "dividendTimes should be in ascending order");
    }
