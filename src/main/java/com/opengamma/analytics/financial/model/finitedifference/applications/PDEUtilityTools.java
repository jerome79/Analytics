...
public class PDEUtilityTools {
  ...
  public static void printSurface(final String name, final double[][] data, final double[] x, final double[] y, final PrintStream out) {
    ...
    ArgChecker.notNull(x, "null x");
    ArgChecker.notNull(y, "null y");
    ArgChecker.notNull(out, "null printStream");
    final int n = data.length;
    final int m = data[0].length;
    ArgChecker.isTrue(n == y.length, "Size of data is {} {}, but length of y is {}", n, m, y.length);
    ArgChecker.isTrue(m == x.length, "Size of data is {} {}, but length of x is {}", n, m, x.length);
