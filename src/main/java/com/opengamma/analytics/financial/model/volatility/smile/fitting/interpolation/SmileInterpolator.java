...
public abstract class SmileInterpolator<T extends SmileModelData> implements GeneralSmileInterpolator {
  ...
  public List<T> getFittedModelParameters(final double forward, final double[] strikes, final double expiry, final double[] impliedVols) {
    ...
    ArgChecker.isTrue(n > 2, "cannot fit less than three points; have {}", n);
    ArgChecker.isTrue(impliedVols.length == n, "#strikes != # vols; have {} and {}", impliedVols.length, n);
  ...
  protected void validateStrikes(final double[] strikes) {
    final int n = strikes.length;
    for (int i = 1; i < n; i++) {
      ArgChecker.isTrue(strikes[i] > strikes[i - 1], "strikes must be in ascending order; have {} (element {}) and {} (element {})", strikes[i - 1], i - 1, strikes[i], i);
    }
  }
