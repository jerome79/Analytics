...
public class MultiHorizonMixedLogNormalModelData {
  ...
  public MultiHorizonMixedLogNormalModelData(final double[] parameters, final boolean useShiftedMeans) {
    ...
    int n;
    if (useShiftedMeans) {
      ArgChecker.isTrue(_nParams % 3 == 2, "Wrong length of parameters - length {}, but must be 3n-2, where n is an integer", _nParams);
      n = (_nParams + 1) / 3;
    } else {
  ...
  * @param weights The weights of (i.e. probability of being in) each state <b>These weights must sum to 1</b> 
  ...
  public MultiHorizonMixedLogNormalModelData(final double[] weights, final double[] sigmas, final double[] mus) {
    ...
    _nNorms = n;
    ArgChecker.isTrue(n == weights.length, "Weights not the same length as sigmas");
    ArgChecker.isTrue(n == mus.length, "Partial forwards not the same length as sigmas");
    ArgChecker.isTrue(n > 0, "no weights");
    double sum = 0.0;
    for (int i = 0; i < n; i++) {
      ArgChecker.isTrue(sigmas[i] > 0.0, "zero or negative sigma");
