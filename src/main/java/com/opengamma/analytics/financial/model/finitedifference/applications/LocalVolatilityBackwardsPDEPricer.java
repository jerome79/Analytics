...
public class LocalVolatilityBackwardsPDEPricer {
  ...
  public LocalVolatilityBackwardsPDEPricer(final boolean useBurnin, final double burninFrac) {
    ArgChecker.isTrue(burninFrac < 0.5, "burn-in fraction too high");
    _useBurnin = useBurnin;
  ...
  public LocalVolatilityBackwardsPDEPricer(final boolean useBurnin, final double burninFrac, final double burninTheta, final double mainTheta) {
    ArgChecker.isTrue(burninFrac < 0.5, "burn-in fraction too high");
    ArgChecker.isTrue(0 <= burninTheta && burninTheta <= 1.0, "burn-in theta must be between 0 and 1.0");
    ArgChecker.isTrue(0 <= mainTheta && mainTheta <= 1.0, "main theta must be between 0 and 1.0");
    _useBurnin = useBurnin;
    _burninFrac = burninFrac;
  ...
  public double price(final ForwardCurve fwd, final Curve<Double, Double> riskFreeRate, final EuropeanVanillaOption option, final LocalVolatilitySurfaceStrike localVol, final boolean isAmerican,
    ...
    ArgChecker.isTrue(n == theta.length, "#theta does not match #grid");
    final double t = option.getTimeToExpiry();
    final double k = option.getStrike();
    final double s0 = fwd.getSpot();
    ...
    ArgChecker.isTrue(grid[0].getTimeNode(0) == 0.0, "time nodes not starting from zero");
    ArgChecker.isTrue(Double.compare(grid[n - 1].getTimeNode(grid[n - 1].getNumTimeNodes() - 1), t) == 0, "time nodes not ending at t");
    for (int ii = 1; ii < n; ii++) {
      ArgChecker.isTrue(Arrays.equals(grid[ii].getSpaceNodes(), xNodes), "different xNodes not supported");
      ArgChecker.isTrue(Double.compare(grid[ii - 1].getTimeNode(grid[ii - 1].getNumTimeNodes() - 1), grid[ii].getTimeNode(0)) == 0, "time nodes not consistent");
    }

    final double sMin = xNodes[0];
    final double sMax = xNodes[xNodes.length - 1];
    ArgChecker.isTrue(sMin <= k, "strike lower than sMin");
    ArgChecker.isTrue(sMax >= k, "strike higher than sMax");

    final int index = Arrays.binarySearch(xNodes, s0);
    ArgChecker.isTrue(index >= 0, "cannot find spot on grid");
