...
public class CappedPowerOptionFunctionProvider extends OptionFunctionProvider1D {
  ...
  public CappedPowerOptionFunctionProvider(final double strike, final double timeToExpiry, final int steps, final boolean isCall, final double power, final double cap) {
    ...
    ArgChecker.isTrue(power > 0., "power should be positive");
    ArgChecker.isTrue(Doubles.isFinite(power), "power should be finite");
    ArgChecker.isTrue(cap > 0., "cap should be positive");
    ArgChecker.isTrue(Doubles.isFinite(cap), "cap should be finite");
    if (!isCall) {
      ArgChecker.isTrue(cap < strike, "cap should be smaller than strike for put");
    }
    _power = power;
    _cap = cap;
  }
