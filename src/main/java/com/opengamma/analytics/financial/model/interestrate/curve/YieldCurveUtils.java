...
public class YieldCurveUtils {
  ...
  public static YieldCurve withParallelShift(YieldCurve curve, double shift, ShiftType shiftType, String nameSuffix) {
    ...
    ArgChecker.notNull(nameSuffix, "nameSuffix");
  ...
  public static YieldCurve withBucketedShifts(YieldCurve curve,
    ...
    ArgChecker.isTrue(buckets.size() == shifts.size(), "must have one shift per bucket");
    ArgChecker.notNull(shiftType, "shift type");
    ArgChecker.notNull(nameSuffix, "nameSuffix");
    ...
    switch (shiftType) {
      case ABSOLUTE: {
        ...
        stepCurvePoints.add(DoublesPair.of(oldPair.getFirst(), shift));
        while (iterBuckets.hasNext()) {
          DoublesPair pair = iterBuckets.next();
          shift = iterShifts.next();
          if (Double.compare(pair.getFirst(), oldPair.getSecond()) != 0) {
            stepCurvePoints.add(DoublesPair.of(oldPair.getSecond(), 0));
            stepCurvePoints.add(DoublesPair.of(pair.getFirst(), shift));
          } else {
            stepCurvePoints.add(DoublesPair.of(oldPair.getSecond(), shift));
          }
          oldPair = pair;
        }
        stepCurvePoints.add(DoublesPair.of(oldPair.getSecond(), 0));
      ...
      case RELATIVE: {
        ...
        stepCurvePoints.add(DoublesPair.of(oldPair.getFirst(), 1 + shift));
        while (iterBuckets.hasNext()) {
          DoublesPair pair = iterBuckets.next();
          shift = iterShifts.next();
          if (Double.compare(pair.getFirst(), oldPair.getSecond()) != 0) {
            stepCurvePoints.add(DoublesPair.of(oldPair.getSecond(), 1));
            stepCurvePoints.add(DoublesPair.of(pair.getFirst(), 1 + shift));
          } else {
            stepCurvePoints.add(DoublesPair.of(oldPair.getSecond(), 1 + shift));
          }
          oldPair = pair;
        }
        stepCurvePoints.add(DoublesPair.of(oldPair.getSecond(), 1));
  ...
  public static YieldCurve withPointShifts(YieldCurve curve,
    ...
    ArgChecker.noNulls(shifts, "shifts");
    ArgChecker.notNull(nameSuffix, "nameSuffix");
    ArgChecker.isTrue(t.size() == shifts.size(), "must have one shift per point");
    ArgChecker.isTrue(
