...
public class EuropeanOptionPrice {
  ...
  public EuropeanOptionPrice(final double price, final double strike, final boolean isCall, final double expiry) {
    ArgChecker.isTrue(price >= 0, "price is negative {}", price);
    ArgChecker.isTrue(strike >= 0, "strike is negative {}", strike);
    ArgChecker.isTrue(expiry >= 0, "expiry is negative {}", expiry);
    _price = price;
    _expiry = expiry;
    _isCall = isCall;
    _strike = strike;
