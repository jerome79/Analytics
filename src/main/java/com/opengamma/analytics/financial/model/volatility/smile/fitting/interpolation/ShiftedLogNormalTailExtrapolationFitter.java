...
public class ShiftedLogNormalTailExtrapolationFitter {
  ...
  public double[] fitTwoPrices(final double forward, final double[] strikes, final double[] prices, final double timeToExpiry, final boolean isCall) {
    ArgChecker.isTrue(strikes[0] < strikes[1], "strikes must be in ascending order");
    ArgChecker.isTrue(strikes[1] < forward || strikes[0] > forward, "strikes cannot be either side of forward");
    if (isCall) {
      ArgChecker.isTrue(prices[0] > prices[1], "Call prices are not decreasing with strike. Either the input data is wrong or there is an arbitrage");
    } else {
      ArgChecker.isTrue(prices[0] < prices[1], "Put prices are not increasing with strike. Either the input data is wrong or there is an arbitrage");
    }
  ...
  public double[] fitPriceAndGrad(final double forward, final double strike, final double price, final double priceGrad, final double timeToExpiry, final boolean isCall) {
    ...
    ArgChecker.isTrue(forward > 0, "Forward must be greater that zero. value given is {}", forward);
    ArgChecker.isTrue(strike >= 0, "Strike must be greater that or equal to zero. value given is {}", strike);
    ArgChecker.isTrue(price > 0, "Price must be greater that zero. value given is {}", price);
    ArgChecker.isTrue(timeToExpiry > 0, "timeToExpiry must be greater that zero. value given is {}", timeToExpiry);
    if (isCall) {
      ArgChecker.isTrue(price < forward, "call price must be less than forward. Price  is {} and foward is {} ", price, forward);
      ArgChecker.isTrue(priceGrad < 0.0, "Call prices must decrease with strike, but priceGrad is {}", priceGrad);
      ...
      ArgChecker.isTrue(price < strike, "put price must be less than strike. Price is {} and strike is  {}", price, strike);
      ArgChecker.isTrue(priceGrad > 0.0, "Put prices must increase with strike, but priceGrad is {}", priceGrad);
    }
  ...
  public double[] fitTwoVolatilities(final double forward, final double[] strikes, final double[] vols, final double timeToExpiry) {
    ArgChecker.isTrue(strikes[0] < strikes[1], "strikes must be in assending order");
    ArgChecker.isTrue(strikes[1] < forward || strikes[0] > forward, "strikes cannot be either side of forward");
  ...
  public ArrayList<Double> fitVolatilityAndGradRecursivelyByTossingPoints(final double forward, final double[] strikes, final double[] vols,
    ...
    ArgChecker.isTrue(vols.length == n, "Lengths of strikes and vols unexpectedly differ!");
    double[] shiftAndVol;
  ...
  public ArrayList<Double> fitVolatilityAndGradRecursivelyByTossingPoints(final double forward, final double[] strikes, final double[] vols, final double[] dSigmaDx,
    ...
    final int n = strikes.length;
    ArgChecker.isTrue(vols.length == n, "Lengths of strikes and vols unexpectedly differ!");
    ArgChecker.isTrue(dSigmaDx.length == n, "Lengths of slopes and vols unexpectedly differ!");
