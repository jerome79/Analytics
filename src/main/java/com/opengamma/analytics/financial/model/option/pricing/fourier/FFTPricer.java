...
public class FFTPricer {
  ...
  public double[][] price(final double forward, final double discountFactor, final double t, final boolean isCall, final MartingaleCharacteristicExponent ce, final int nStrikes,
    ...
    ArgChecker.notNull(ce, "characteristicExponent");
    ArgChecker.isTrue(tol > 0.0, "need tol > 0");
    ArgChecker.isTrue(nStrikes > 0, "need number of strikes > 0");
    ArgChecker.isTrue(maxDeltaMoneyness > 0, "need max delta moneyness > 0");
    ArgChecker.isTrue(alpha != 0.0 && alpha != -1.0, "alpha cannot be -1 or 0");
    ArgChecker.isTrue(limitSigma > 0.0, "need limitSigma > 0");
