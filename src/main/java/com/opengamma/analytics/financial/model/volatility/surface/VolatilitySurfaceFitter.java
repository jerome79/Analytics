...
public abstract class VolatilitySurfaceFitter<T extends SmileModelData> {
  ...
  public VolatilitySurfaceFitter(final double[] forwards, final double[][] strikes, final double[] expiries, final double[][] impliedVols, final double[][] errors,
    ...
    ArgChecker.notNull(forwards, "null forwards");
    ...
    ArgChecker.notNull(errors, "null error");
    ArgChecker.notNull(model, "null model");

    _nExpiries = expiries.length;
    ArgChecker.isTrue(forwards.length == _nExpiries, "#forwards != #expiries");
    ArgChecker.isTrue(strikes.length == _nExpiries, "#strike sets != #expiries");
    ArgChecker.isTrue(impliedVols.length == _nExpiries, "#vol sets != #expiries");
    ArgChecker.isTrue(errors.length == _nExpiries, "#error sets != #expiries");
    ...
    for (int i = 0; i < _nExpiries; i++) {
      ...
      ArgChecker.isTrue(impliedVols[i].length == n, "#vols in strip " + i + " is wrong");
      ArgChecker.isTrue(errors[i].length == n, "#vols in strip " + i + " is wrong");
  ...
  protected Function1D<DoubleMatrix1D, DoubleMatrix1D> getModelValueFunction() {
    ...
    return new Function1D<DoubleMatrix1D, DoubleMatrix1D>() {
      ...
      public DoubleMatrix1D evaluate(final DoubleMatrix1D x) {
        ...
        ArgChecker.isTrue(x.getNumberOfElements() == _nKnotPoints, "number of elements {} does not equal number of knot points {}",
    ...
    return new Function1D<DoubleMatrix1D, DoubleMatrix2D>() {
      ...
      public DoubleMatrix2D evaluate(final DoubleMatrix1D x) {
        ...
        for (int i = 0; i < _nExpiries; i++) {
          ...
          final int l = temp.length;
          ArgChecker.isTrue(l == _strikes[i].length, "number of elements {} does not equal number of knot points {}", l, _strikes[i].length); //TODO remove when working properly
          for (int j = 0; j < l; j++) {
            int paramOffset = 0;
