...
public class InflationZeroCouponCapFloorParameters {
  ...
  public InflationZeroCouponCapFloorParameters(final double[] expiryTimes, final double[] strikes, final double[][] volatility, final IndexPrice index) {
    ArgChecker.notNull(expiryTimes, "Inflation zero coupon options expiry times");
    ArgChecker.notNull(strikes, "Inflation zero coupon options strikes");
    ArgChecker.notNull(volatility, "Inflation zero coupon options volatilities");
    ArgChecker.isTrue(expiryTimes.length == volatility.length, "number of expiry should be the same in the volatility matrix and in the expiry vector");
    ArgChecker.isTrue(strikes.length == volatility[0].length, "number of strikes should be the same in the volatility matrix and in the strikes vector");
  ...
  */
  ...
  public final void setVolatility(final double[][] volatility, final int expiryIndex) {
    ArgChecker.notNull(volatility, "LMM volatility");
    ArgChecker.isTrue(volatility[0].length == _strikes.length, "LMM: incorrect number of factors");
  ...
  public final void setExpiryTimes(final double[] expiryTimes, final int startIndex) {
    ArgChecker.notNull(expiryTimes, "Inflation year on year options expiry times");
  ...
  public final void setStrikes(final double[] strikes, final int startIndex) {
    ArgChecker.notNull(_strikes, "Inflation year on year options strikes");
