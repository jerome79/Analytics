...
public class MarkovChainApprox {
  ...
  public MarkovChainApprox(final double vol1, final double vol2, final double lambda12, final double lambda21, final double probState1, final double expiry) {
    ArgChecker.isTrue(vol1 >= 0, "The first volatility must be greater than or equal to 0; have {}", vol1);
    ArgChecker.isTrue(vol2 >= 0, "The second volatility must be greater than or equal to 0; have {}", vol2);
    ArgChecker.isTrue(lambda12 >= 0, "The first jump parameter must be greater than or equal to 0; have {}", lambda12);
    ArgChecker.isTrue(lambda21 >= 0, "The second jump parameter must be greater than or equal to 0; have {}", lambda21);
    ArgChecker.isTrue(probState1 >= 0 && probState1 <= 1.0, "The probability of being in the first state must be >= 0 and <= 1");
    _vol1 = vol1;
    _vol2 = vol2;
    _nu1 = vol1 * vol1;
    _nu2 = vol2 * vol2;
  ...
  private double getModMoment(final int n, final double a, final double b, final double mu, final double moment) {
    ArgChecker.isTrue(n > 0, "n must be greater than zero");
