...
public class G2ppPiecewiseConstantParameters {
  ...
  public G2ppPiecewiseConstantParameters(final double[] meanReversion, final double[][] volatility, final double[] volatilityTime, final double correlation) {
    ArgChecker.notNull(meanReversion, "mean reversion");
    ArgChecker.notNull(volatility, "volatility");
    ArgChecker.notNull(volatilityTime, "volatility time");
    ArgChecker.isTrue(meanReversion.length == 2, "Two mean reversions required");
    ArgChecker.isTrue(volatility.length == 2, "Two volatility arrays required");
    ArgChecker.isTrue(volatility[0].length == volatility[1].length, "Volatility length");
    ArgChecker.isTrue(volatility[0].length == volatilityTime.length + 1, "Number of times incorrect; had {}, need {}", volatilityTime.length + 1, volatility[0].length);
  ...
  public void setVolatility(final double[][] volatility) {
    ArgChecker.isTrue(volatility.length == 2, "Two volatility arrays required");
    ArgChecker.isTrue(volatility[0].length == volatility[1].length, "Volatility length");
    ArgChecker.isTrue(volatility[0].length == _volatilityTime.size() - 1, "Volatility length");
    _volatility[0] = new DoubleArrayList(volatility[0]);
    _volatility[1] = new DoubleArrayList(volatility[1]);
  }
  ...
  public void setLastVolatilities(final double[] volatility) {
    ArgChecker.isTrue(volatility.length == 2, "Two volatilities required");
  ...
  public void addVolatility(final double[] volatility, final double volatilityTime) {
    ArgChecker.isTrue(volatility.length == 2, "Two volatilities required");
    ArgChecker.isTrue(volatilityTime > _volatilityTime.get(_volatilityTime.size() - 2), "Volatility times should be increasing");
