...
public class PDE1DDataBundle<T extends PDE1DCoefficients> {
  ...
  public PDE1DDataBundle(final T coefficients,
    ...
    ArgChecker.notNull(freeBoundary, "null freeBoundary");
    _initialCondition = getInitialConditions(coefficients, initialCondition, lowerBoundary, upperBoundary, grid);
    _coefficients = coefficients;
    _lowerBoundary = lowerBoundary;
    _upperBoundary = upperBoundary;
  ...
  private static double[] getInitialConditions(final PDE1DCoefficients coefficients, final Function1D<Double, Double> initialCondition,
    ...
    ArgChecker.notNull(coefficients, "null coefficients");
    ArgChecker.notNull(initialCondition, "null initialCondition");
    ArgChecker.notNull(lowerBoundary, "null lowerBoundary");
    ArgChecker.notNull(upperBoundary, "null upperBoundary");
    ArgChecker.notNull(grid, "null grid");

    final int n = grid.getNumSpaceNodes();

    ArgChecker.isTrue(Math.abs((grid.getSpaceNode(0) - lowerBoundary.getLevel()) / (1.0 + Math.abs(lowerBoundary.getLevel()))) < 1e-12,
    ...
    ArgChecker.isTrue(Math.abs((grid.getSpaceNode(n - 1) - upperBoundary.getLevel()) / (1.0 + Math.abs(upperBoundary.getLevel()))) < 1e-12,
  ...
  private static void checkData(final PDE1DCoefficients coefficients, final double[] initialCondition, final BoundaryCondition lowerBoundary,
    ...
    final int n = grid.getNumSpaceNodes();
    ArgChecker.isTrue(initialCondition.length == n, n + " space nodes, but " + initialCondition.length + " values specified for initial condition");
    ArgChecker.isTrue(Math.abs((grid.getSpaceNode(0) - lowerBoundary.getLevel()) / (1.0 + lowerBoundary.getLevel())) < 1e-12,
        "space grid not consistent with lower boundary level. Lowerst grid point at " + grid.getSpaceNode(0) + " but lower boundary at "
            + lowerBoundary.getLevel());
    ArgChecker.isTrue(Math.abs((grid.getSpaceNode(n - 1) - upperBoundary.getLevel()) / (1.0 + upperBoundary.getLevel())) < 1e-12,
        "space grid not consistent with upper boundary level. Highest grid point at " + grid.getSpaceNode(n - 1) + " but upper boundary at "
            + upperBoundary.getLevel());
