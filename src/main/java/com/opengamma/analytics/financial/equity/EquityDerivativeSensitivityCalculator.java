...
public class EquityDerivativeSensitivityCalculator {
  ...
  public NodalDoublesSurface calcBlackVegaForEntireSurface(final InstrumentDerivative derivative, final StaticReplicationDataBundle market, final double shift) {
    ...
    if (market.getVolatilitySurface().getSurface() instanceof InterpolatedDoublesSurface) {
      ...
      ArgChecker.isTrue(nNodes == strikes.length, "Number of strikes must match number of nodes");
      // Bump and reprice
      ...
      if (derivative instanceof EquityIndexOption) {
        ...
        throw new IllegalArgumentException("Calculator with BlackVolatilitySurfaceMoneynessFcnBackedByGrid was expecting an EquityIndexOption.");
      }
      // Unpack
      ...
      for (int t = Math.max(0, expiryIndex - 3); t < Math.min(nExpiries, expiryIndex + 4); t++) {
        ...
        for (int k = Math.max(0, strikeIndex - 6); k < Math.min(nStrikes, strikeIndex + 7); k++) {
          ...
          ArgChecker.notNull(pvScenario, "Null PV in shifted scenario, T = " + volExpiries[t] + ", k = " + strikes[t][k]);
          final Double vega = (pvScenario - pvBase) / -shift;
          final Triple<Double, Double, Double> xyz = Triple.of(volExpiries[t], strikes[t][k], vega);
      ...
      throw new IllegalArgumentException("Currently will only accept an Equity Volatility Surface based on an InterpolatedDoublesSurface, "
  ...
  public double calcBlackVegaForSinglePoint(final InstrumentDerivative derivative, final StaticReplicationDataBundle market, final double maturity, final double strike,
    ...
    ArgChecker.isTrue(surface instanceof InterpolatedDoublesSurface, "Currently will only accept a Equity VolatilitySurfaces based on an InterpolatedDoublesSurface");
