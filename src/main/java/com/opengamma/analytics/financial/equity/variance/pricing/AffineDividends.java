...
public class AffineDividends {
  ...
  public AffineDividends(final double[] tau, final double[] alpha, final double[] beta) {
    ArgChecker.notNull(tau, "null tau");
    ArgChecker.notNull(alpha, "null alpha");
    ArgChecker.notNull(beta, "null beta");
    _n = tau.length;
    ArgChecker.isTrue(_n == alpha.length, "alpha wrong length");
    ArgChecker.isTrue(_n == beta.length, "beta wrong length");

    if (_n > 0) {
      ArgChecker.isTrue(tau[0] >= 0.0, "first dividend at negative time. Please remove from list");
      ArgChecker.isTrue(alpha[0] >= 0.0, "first cash dividend is negative.");
      ArgChecker.isTrue(beta[0] >= 0.0 && beta[0] < 1.0, "Proportional dividend must be between 0.0 (inclusive) and 1.0 (exclusive). Value is {}", beta[0]);
      for (int i = 1; i < _n; i++) {
        ArgChecker.isTrue(tau[i] > tau[i - 1], "Dividends not increasing. {}th dividend is {}, and {}th is {}", i, tau[i], i - 1, tau[i - 1]);
        ArgChecker.isTrue(alpha[i] >= 0.0, "Cash dividend is negative. alpha[{}] = {}", i, alpha[i]);
        ArgChecker.isTrue(beta[i] >= 0.0 && beta[i] < 1.0, "Proportional dividend must be between 0.0 (inclusive) and 1.0 (exclusive). beta[{}] = {}", i, beta[i]);
      }
    }
    _tau = tau;
    _alpha = alpha;
  ...
  public AffineDividends withTau(final double value, final int index) {
    ArgChecker.isTrue(value >= 0.0, "negative tau");
    ArgChecker.isTrue(index >= 0 && index < _n, "index out of range");
    final double[] tau = Arrays.copyOf(_tau, _n);
    tau[index] = value;
  ...
  public AffineDividends withAlpha(final double value, final int index) {
    ArgChecker.isTrue(value >= 0.0, "negative alpha");
    ArgChecker.isTrue(index >= 0 && index < _n, "index out of range");
    final double[] alpha = Arrays.copyOf(_alpha, _n);
    alpha[index] = value;
  ...
  public AffineDividends withBeta(final double value, final int index) {
    ArgChecker.isTrue(value >= 0.0, "negative beta");
    ArgChecker.isTrue(index >= 0 && index < _n, "index out of range");
    final double[] beta = Arrays.copyOf(_beta, _n);
    beta[index] = value;
