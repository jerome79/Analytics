...
public class VarianceSwapStaticReplication {
  ...
  public double presentValue(final VarianceSwap deriv, final StaticReplicationDataBundle market) {
    ArgChecker.notNull(deriv, "VarianceSwap deriv");
    ArgChecker.notNull(market, "EquityOptionDataBundle market");
    ...
    if (deriv.getTimeToObsStart() > 0) { // no observations have been made
      ...
      ArgChecker.isTrue(deriv.getObservations().length > 0, "presentValue requested after first observation date, yet no observations have been provided.");
  ...
  private void validateData(final VarianceSwap deriv, final StaticReplicationDataBundle market) {
    ...
    ArgChecker.isTrue(timeToFirstObs < timeToLastObs, "timeToLastObs is not sufficiently longer than timeToFirstObs");
  }
