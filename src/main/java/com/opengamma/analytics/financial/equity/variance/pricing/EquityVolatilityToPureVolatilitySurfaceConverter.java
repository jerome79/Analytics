...
public class EquityVolatilityToPureVolatilitySurfaceConverter {
  ...
  public static PureImpliedVolatilitySurface getConvertedSurface(final double spot, final YieldAndDiscountCurve discountCurve, final AffineDividends dividends,
    ...
    ArgChecker.notNull(discountCurve, "discount curve");
    ArgChecker.notNull(dividends, "dividends");
    ArgChecker.notNull(expiries, "expiries");
    final int nExp = expiries.length;
    ArgChecker.isTrue(strikes.length == nExp, "number of strike strips ({}) not equal to number of expiries({})", strikes.length, nExp);
    ArgChecker.isTrue(otmPrices.length == nExp, "number of price strips ({}) not equal to number of expiries({})", strikes.length, nExp);
    for (int i = 0; i < nExp; i++) {
      ArgChecker.isTrue(strikes[i].length == otmPrices[i].length, "number of prices and strikes in strip #{} (expiry = {}) do not match. {} prices and {} strikes",
