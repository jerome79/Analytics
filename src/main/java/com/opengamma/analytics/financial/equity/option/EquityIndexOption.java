...
public class EquityIndexOption implements InstrumentDerivative {
  ...
  public EquityIndexOption(final double timeToExpiry, final double timeToSettlement, final double strike, final boolean isCall, final Currency currency,
    ...
    ArgChecker.isTrue(timeToExpiry >= 0, "Time to expiry must not be negative");
    ArgChecker.isTrue(timeToSettlement >= 0, "Time to settlement must not be negative");
    ArgChecker.isTrue(timeToSettlement >= timeToExpiry, "Settlement time must be after expiry");
    ArgChecker.notNegativeOrZero(strike, "strike");
    ArgChecker.notNull(currency, "currency");
    ArgChecker.notZero(unitAmount, 1e-15, "unit amount");
    ArgChecker.notNull(exerciseType, "exercise type");
