...
public class RealizedVariance extends Function1D<VarianceSwap, Double> {
  ...
  public Double evaluate(final VarianceSwap swap) {
    ...
    if (swap.getObservationWeights().length == 0) {
      ...
      ArgChecker.isTrue(nWeights == nObs - 1,
    ...
    ArgChecker.isTrue(obs[0] != 0.0, "In VarianceSwap, the first observation is zero so the estimate of RealizedVariance is undefined. Check time series.");
    double logReturns = 0;
    for (int i = 1; i < nObs; i++) {
      ArgChecker.isTrue(Double.compare(obs[i], 0.0) != 0, "Encountered an invalid observation of zero in VarianceSwap at {}'th observation. "
