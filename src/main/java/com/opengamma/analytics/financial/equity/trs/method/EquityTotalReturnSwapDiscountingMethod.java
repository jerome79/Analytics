...
public final class EquityTotalReturnSwapDiscountingMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final EquityTotalReturnSwap trs, final EquityTrsDataBundle equityMulticurves) {
    ArgChecker.notNull(trs, "equity TRS");
    ArgChecker.notNull(equityMulticurves, "multi-curve provider with equity price");
    ArgChecker.isTrue(trs.getDividendPercentage() == 1.0, "equity TRS dividend ration should be 1.0");
    MultiCurrencyAmount equityPv = MultiCurrencyAmount.of(trs.getEquity().getCurrency(),
    ...
    if ((trs.getFundingLeg().getNumberOfPayments() == 1) ||
      ...
      MultiCurrencyAmount previousFixingPv = MultiCurrencyAmount.of(trs.getNotionalCurrency(),
      ...
      final MultiCurrencyAmount fundingLegPV = trs.getFundingLeg().getNthPayment(0).accept(PVDC, equityMulticurves.getCurves());
    ...
    if ((trs.getFundingLeg().getNthPayment(1) instanceof CouponFixed)
      ...
      final MultiCurrencyAmount fundingLegPv0 = trs.getFundingLeg().getNthPayment(0).accept(PVDC, equityMulticurves.getCurves());
      final MultiCurrencyAmount fundingLegPv1 = trs.getFundingLeg().getNthPayment(1).accept(PVDC, equityMulticurves.getCurves());
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValueAssetLeg(final EquityTotalReturnSwap trs, final EquityTrsDataBundle equityMulticurves) {
    ArgChecker.notNull(trs, "equity TRS");
    ArgChecker.notNull(equityMulticurves, "multi-curve provider with equity price");
    MultiCurrencyAmount equityPv = MultiCurrencyAmount.of(trs.getEquity().getCurrency(),
        equityMulticurves.getSpotEquity() * trs.getEquity().getNumberOfShares());
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValueFundingLeg(final EquityTotalReturnSwap trs, final EquityTrsDataBundle equityMulticurves) {
    ArgChecker.notNull(trs, "equity TRS");
    ArgChecker.notNull(equityMulticurves, "multi-curve provider with equity price");
    // First coupon ON or Ibor: Payment up to the fist payment taken into account OR Only one payment left
    if ((trs.getFundingLeg().getNumberOfPayments() == 1) ||
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount currencyExposure(final EquityTotalReturnSwap trs, final EquityTrsDataBundle equityMulticurves) {
    ArgChecker.notNull(trs, "equity TRS");
    ArgChecker.notNull(equityMulticurves, "multi-curve provider with equity price");
    MultiCurrencyAmount equityPv = MultiCurrencyAmount.of(trs.getEquity().getCurrency(),
        equityMulticurves.getSpotEquity() * trs.getEquity().getNumberOfShares());
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount assetExposure(final EquityTotalReturnSwap trs, final EquityTrsDataBundle equityMulticurves) {
    ArgChecker.notNull(trs, "equity TRS");
    ArgChecker.notNull(equityMulticurves, "multi-curve provider with equity price");
    ArgChecker.isTrue(trs.getDividendPercentage() == 1.0, "equity TRS dividend ration should be 1.0");
    return MultiCurrencyAmount.of(trs.getEquity().getCurrency(),
        equityMulticurves.getSpotEquity() * trs.getEquity().getNumberOfShares());
  }
