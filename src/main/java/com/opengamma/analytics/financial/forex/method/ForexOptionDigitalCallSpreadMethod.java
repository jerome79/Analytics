...
public class ForexOptionDigitalCallSpreadMethod implements ForexPricingMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final ForexOptionDigital optionDigital, final SmileDeltaTermStructureDataBundle smile) {
    ...
    // Spread value
    final MultiCurrencyAmount pvM = _baseMethod.presentValue(callSpread[0], smile);
    final MultiCurrencyAmount pvP = _baseMethod.presentValue(callSpread[1], smile);
    return pvM.plus(pvP);
  }
   ...
   */
  public MultiCurrencyAmount currencyExposure(final ForexOptionDigital optionDigital, final SmileDeltaTermStructureDataBundle smile) {
    ArgChecker.notNull(optionDigital, "Forex option difital");
    ArgChecker.notNull(smile, "Curve and smile data");
    final ForexOptionVanilla[] callSpread = callSpread(optionDigital, _spread);
    // Spread value
    final MultiCurrencyAmount ceM = _baseMethod.currencyExposure(callSpread[0], smile);
    final MultiCurrencyAmount ceP = _baseMethod.currencyExposure(callSpread[1], smile);
    return ceM.plus(ceP);
  }
