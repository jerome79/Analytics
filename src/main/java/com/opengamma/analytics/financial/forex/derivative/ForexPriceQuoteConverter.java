...
public class ForexPriceQuoteConverter {
  ...
  public ForexPriceQuoteConverter(final double domesticPipsPrice, final double spotRate, final double strike, final double domesticNotional, final double foreignNotional) {
    ...
    ArgChecker.isTrue(domesticPipsPrice >= 0.0, "Negative price given");
    ArgChecker.isTrue(spotRate > 0.0, "Spot rate must be greater than zero. value gvien is {}", spotRate);
    ArgChecker.isTrue(strike > 0.0, "Strike  must be greater than zero. value gvien is {}", strike);
    ArgChecker.isTrue(domesticNotional > 0.0, "Domestic Notional must be greater than zero. value gvien is {}", domesticNotional);
    ArgChecker.isTrue(foreignNotional > 0.0, "Foreign Notional must be greater than zero. value gvien is {}", foreignNotional);
