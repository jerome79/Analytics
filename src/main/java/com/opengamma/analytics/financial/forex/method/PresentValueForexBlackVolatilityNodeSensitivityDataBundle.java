...
public class PresentValueForexBlackVolatilityNodeSensitivityDataBundle {
  ...
  public PresentValueForexBlackVolatilityNodeSensitivityDataBundle(final Currency ccy1, final Currency ccy2, final int numberExpiry, final int numberDelta) {
    ...
    ArgChecker.isTrue(numberExpiry >= 0, "number of expiries must be greater than or equal to zero");
    ArgChecker.isTrue(numberDelta >= 0, "number of deltas must be greater than or equal to zero");
  ...
  public PresentValueForexBlackVolatilityNodeSensitivityDataBundle(final Currency ccy1, final Currency ccy2, final DoubleMatrix1D expiries, final DoubleMatrix1D delta, final DoubleMatrix2D vega) {
    ...
    ArgChecker.notNull(delta, "strikes");
    ArgChecker.notNull(vega, "Matrix");
    ArgChecker.isTrue(vega.getNumberOfRows() == expiries.getNumberOfElements(), "Number of rows did not match number of expiries");
    ArgChecker.isTrue(vega.getNumberOfColumns() == delta.getNumberOfElements(), "Number of columns did not match number of delta");
    _currencyPair = Pair.of(ccy1, ccy2);
    _expiries = expiries;
    _delta = delta;
    _vega = vega;
