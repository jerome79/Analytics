...
public class ForexOptionDigitalCallSpreadBlackSmileMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final ForexOptionDigital optionDigital, final BlackForexSmileProviderInterface smileMulticurves) {
    ArgChecker.notNull(optionDigital, "Forex option");
    ForexOptionVanilla[] callSpread = callSpread(optionDigital, _spread);
    // Spread value
    MultiCurrencyAmount pvM = BASE_METHOD.presentValue(callSpread[0], smileMulticurves);
    MultiCurrencyAmount pvP = BASE_METHOD.presentValue(callSpread[1], smileMulticurves);
    return pvM.plus(pvP);
  }
   ...
   */
  public MultiCurrencyAmount currencyExposure(final ForexOptionDigital optionDigital, final BlackForexSmileProviderInterface smileMulticurves) {
    ArgChecker.notNull(optionDigital, "Forex option");
    ForexOptionVanilla[] callSpread = callSpread(optionDigital, _spread);
    // Spread value
    MultiCurrencyAmount ceM = BASE_METHOD.currencyExposure(callSpread[0], smileMulticurves);
    MultiCurrencyAmount ceP = BASE_METHOD.currencyExposure(callSpread[1], smileMulticurves);
    return ceM.plus(ceP);
  }
  ...
  public PresentValueForexBlackVolatilityNodeSensitivityDataBundle presentValueBlackVolatilityNodeSensitivity(final ForexOptionDigital optionDigital,
    ...
    for (final DoublesPair point : pointSensitivity.getVega().getMap().keySet()) {
      final VolatilityAndBucketedSensitivities volAndSensitivities = smileMulticurves.getVolatilityAndSensitivities(optionDigital.getCurrency1(), optionDigital.getCurrency2(),
          optionDigital.getExpirationTime(), point.getSecond(), forward);
