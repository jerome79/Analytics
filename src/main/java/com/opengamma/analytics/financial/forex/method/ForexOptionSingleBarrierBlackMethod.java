...
public final class ForexOptionSingleBarrierBlackMethod implements ForexPricingMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final ForexOptionSingleBarrier optionForex, final SmileDeltaTermStructureDataBundle smile) {
  ...
  public MultiCurrencyAmount presentValue(final InstrumentDerivative instrument, final YieldCurveBundle curves) {
    ArgChecker.isTrue(instrument instanceof ForexOptionSingleBarrier, "Single barrier Forex option");
   ...
   */
  public MultiCurrencyAmount currencyExposure(final ForexOptionSingleBarrier optionForex, final SmileDeltaTermStructureDataBundle smile) {
    ArgChecker.notNull(optionForex, "Forex option");
    ArgChecker.notNull(smile, "Smile");
    final double payTime = optionForex.getUnderlyingOption().getUnderlyingForex().getPaymentTime();
    final double rateDomestic = smile.getCurve(optionForex.getUnderlyingOption().getUnderlyingForex().getPaymentCurrency2().getFundingCurveName()).getInterestRate(
