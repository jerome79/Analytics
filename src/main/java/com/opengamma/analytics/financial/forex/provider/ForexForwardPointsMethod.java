...
public final class ForexForwardPointsMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final Forex fx, final MulticurveProviderInterface multicurves, final DoublesCurve forwardRates, final Pair<Currency, Currency> ccyPair) {
    ...
    return MultiCurrencyAmount.of(ccy2, pv);
  }
   ...
   */
  public MultiCurrencyAmount presentValue(final Forex fx, final MulticurveForwardPointsProviderInterface multicurvesForward) {
    ArgChecker.notNull(multicurvesForward, "multi-curve provider");
   ...
   */
  public MultiCurrencyAmount currencyExposure(final Forex fx, final MulticurveProviderInterface multicurves, final DoublesCurve forwardRates, final Pair<Currency, Currency> ccyPair) {
    final double payTime = fx.getPaymentTime();
    final double fwdRate = forwardRates.getYValue(payTime);
    final Currency ccy1;
    final Currency ccy2;
    ...
    MultiCurrencyAmount ce = MultiCurrencyAmount.of(ccy1, ce1);
    ce = ce.plus(ccy2, ce2);
    return ce;
  }
  ...
  private boolean checkCurrency(final Forex fx, final Pair<Currency, Currency> ccyPair) {
    ArgChecker.isTrue(fx.getCurrency1().equals(ccyPair.getFirst()) || fx.getCurrency1().equals(ccyPair.getSecond()), fx.getCurrency1() + "not in currency pair for forward point method");
    ArgChecker.isTrue(fx.getCurrency2().equals(ccyPair.getFirst()) || fx.getCurrency2().equals(ccyPair.getSecond()), fx.getCurrency2() + "not in currency pair for forward point method");
