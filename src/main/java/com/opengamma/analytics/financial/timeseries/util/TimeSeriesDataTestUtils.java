...
public class TimeSeriesDataTestUtils {
  ...
  public static void testNotNullOrEmpty(final DoubleTimeSeries<?> ts) {
    ...
    ArgChecker.isTrue(!ts.isEmpty(), "time series");
  }
  ...
  public static void testTimeSeriesSize(final DoubleTimeSeries<?> ts, final int minLength) {
    ArgChecker.notNull(ts, "time series");
    ArgChecker.isTrue(minLength >= 0, "Minumum length must be greater than zero");
    ArgChecker.isTrue(ts.size() >= minLength, "time series must contain at least " + minLength + " values");
  }
