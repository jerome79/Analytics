...
public class CapFloorInflationYearOnYearInterpolationDefinition extends CouponInflationDefinition implements CapFloor {
  ...
  */
  ...
  public CapFloorInflationYearOnYearInterpolationDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
    ...
    ArgChecker.notNull(lastKnownFixingDate, "Last known fixing date");
    ArgChecker.notNull(referenceStartDate, "Reference start date");
    ArgChecker.notNull(referenceEndDate, "Reference end date");
    ArgChecker.isTrue(conventionalMonthLag <= monthLag, "the month lag should be superior (or equal)to the conventional month lag");
  ...
  public CapFloorInflationYearOnYearInterpolation toDerivative(final ZonedDateTime date) {
    ...
    ArgChecker.isTrue(!date.isAfter(getPaymentDate()), "Do not have any fixing data but are asking for a derivative after the payment date");
