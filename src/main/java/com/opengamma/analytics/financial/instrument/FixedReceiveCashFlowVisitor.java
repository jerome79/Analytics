 ...
 */
public final class FixedReceiveCashFlowVisitor extends InstrumentDefinitionVisitorSameValueAdapter<LocalDateDoubleTimeSeries, Map<LocalDate, MultiCurrencyAmount>> {
  ...
  public Map<LocalDate, MultiCurrencyAmount> visitForwardRateAgreementDefinition(final ForwardRateAgreementDefinition forwardRateAgreement) {
    ...
    ArgChecker.isTrue(forwardRateAgreement.getNotional() < 0, "Receive floating FRAs need an index fixing time series to find receive cash flows");
  ...
  public Map<LocalDate, MultiCurrencyAmount> visitSwapFixedIborDefinition(final SwapFixedIborDefinition swap) {
    ...
    ArgChecker.isFalse(swap.getFixedLeg().isPayer(), "Payer swaps need an index fixing series to calculate receive cash-flows");
  ...
  public Map<LocalDate, MultiCurrencyAmount> visitForexNonDeliverableForwardDefinition(final ForexNonDeliverableForwardDefinition ndf) {
    ...
    return Collections.singletonMap(endDate, MultiCurrencyAmount.of(ndf.getCurrency2(), Math.abs(ndf.getNotional())));
  }
