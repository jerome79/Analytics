 ...
 */
public class VarianceSwapDefinition implements InstrumentDefinitionWithData<VarianceSwap, LocalDateDoubleTimeSeries> {
  /** The currency */
  ...
  public VarianceSwapDefinition(final ZonedDateTime obsStartDate, final ZonedDateTime obsEndDate, final ZonedDateTime settlementDate, final Currency currency, final HolidayCalendar calendar,
    ...
    ArgChecker.notNull(settlementDate, "settlementDate");
  ...
  public VarianceSwap toDerivative(final ZonedDateTime date) {
    return toDerivative(date, LocalDateDoubleTimeSeries.empty());
  }
   ...
   */
  @Override
  public VarianceSwap toDerivative(final ZonedDateTime date, final LocalDateDoubleTimeSeries underlyingTimeSeries) {
    ArgChecker.notNull(date, "date");
    ArgChecker.notNull(underlyingTimeSeries, "underlyingTimeSeries");
    ...
    ArgChecker.isTrue(nObsDisrupted >= 0, "Have more observations {} than good business days {}", observations.length, nObservations);
    return new VarianceSwap(
        timeToObsStart, timeToObsEnd, timeToSettlement, _varStrike, _varNotional,
        _currency, _annualizationFactor, _nObsExpected, nObsDisrupted, observations, observationWeights);
  }
