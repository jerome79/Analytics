...
public class CouponIborFxResetDefinition extends CouponDefinition implements
  ...
  public CouponIborFxResetDefinition(final Currency currency, final ZonedDateTime paymentDate,
    ...
    ArgChecker.notNull(fxFixingDate, "FX fixing date");
    ArgChecker.notNull(fxDeliveryDate, "FX delivery date");
  ...
  public CouponIborFxResetDefinition(final Currency currency, final ZonedDateTime paymentDate,
    ...
    super(currency, paymentDate, accrualStartDate, accrualEndDate, paymentAccrualFactor, notional);
    ArgChecker.notNull(iborIndexFixingDate, "ibor index fixing date");
    ArgChecker.notNull(iborIndexFixingPeriodStartDate, "ibor index fixing period start date");
    ArgChecker.notNull(iborIndexFixingPeriodEndDate, "ibor index fixing period end date");
    ArgChecker.notNull(index, "index");
    ArgChecker.notNull(calendar, "calendar");
    ArgChecker.notNull(referenceCurrency, "reference currency");
    ArgChecker.notNull(fxFixingDate, "FX fixing date");
  ...
  public CouponIborFxReset toDerivative(ZonedDateTime dateTime) {
    ...
    ArgChecker.isTrue(!dayConversion.isAfter(getIborIndexFixingDate().toLocalDate()),
  ...
  public Coupon toDerivative(ZonedDateTime dateTime, DoubleTimeSeries<ZonedDateTime>[] timeSeries) {
    ...
    LocalDate dayConversion = dateTime.toLocalDate();
    ArgChecker.isTrue(!dayConversion.isAfter(getPaymentDate().toLocalDate()), "date is after payment date");
    ArgChecker.noNulls(timeSeries, "Time series");
    ArgChecker.isTrue(timeSeries.length >= 2, "At least two time series");
    ...
    if (dayConversion.isAfter(indexFixingDate)) { // Index fixing should be known
      ...
      if (fixedRate == null) {
        throw new IllegalStateException("Could not get ibor index fixing value for date " +
    ...
    if (dayConversion.isAfter(fxFixingDate)) { // FX fixing should be known
      ...
      if (fxRate == null) {
        throw new IllegalStateException("Could not get fixing value for date " + _fxFixingDate);
      }
