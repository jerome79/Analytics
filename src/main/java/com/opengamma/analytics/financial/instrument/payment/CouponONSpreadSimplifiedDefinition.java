...
public class CouponONSpreadSimplifiedDefinition extends CouponDefinition {
  ...
  public CouponONSpreadSimplifiedDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
    ...
    ArgChecker.notNull(index, "Coupon OIS Simplified: index");
    ArgChecker.notNull(fixingPeriodStartDate, "Coupon OIS Simplified: fixingPeriodStartDate");
    ArgChecker.notNull(fixingPeriodEndDate, "Coupon OIS Simplified: fixingPeriodEndDate");
    ArgChecker.isTrue(currency.equals(index.getCurrency()), "Currency and index not compatible");
    _index = index;
  ...
  public static CouponONSpreadSimplifiedDefinition from(final IndexON index, final ZonedDateTime settlementDate, final ZonedDateTime endFixingPeriodDate,
      final double notional, final double spread, final int settlementDays, final HolidayCalendar calendar) {
  ...
  public CouponONSpread toDerivative(final ZonedDateTime date) {
    ...
    ArgChecker.isTrue(!date.isAfter(_fixingPeriodStartDate) || date.toLocalDate().equals(_fixingPeriodStartDate.toLocalDate()),
