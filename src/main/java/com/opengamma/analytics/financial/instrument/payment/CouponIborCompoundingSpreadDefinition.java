...
public class CouponIborCompoundingSpreadDefinition extends CouponDefinition implements InstrumentDefinitionWithData<Payment, DoubleTimeSeries<ZonedDateTime>> {
  ...
  protected CouponIborCompoundingSpreadDefinition(
    ...
    ArgChecker.isTrue(accrualStartDates.length == accrualEndDates.length, "Accrual start and end dates should have same length");
    ArgChecker.isTrue(accrualStartDates.length == fixingDates.length, "Same length");
    ArgChecker.isTrue(accrualStartDates.length == fixingPeriodStartDates.length, "Same length");
    ArgChecker.isTrue(accrualStartDates.length == fixingPeriodEndDates.length, "Same length");
    ArgChecker.isTrue(accrualStartDates.length == fixingPeriodAccrualFactors.length, "Same length");
    _index = index;
  ...
  public static CouponIborCompoundingSpreadDefinition from(final double notional, final ZonedDateTime accrualStartDate, final ZonedDateTime accrualEndDate, final IborIndex index,
                                                           final double spread, final StubType stub, final BusinessDayConvention businessDayConvention, final boolean endOfMonth, final HolidayCalendar calendar, final
  ...
  public CouponIborCompoundingSpread toDerivative(final ZonedDateTime dateTime) {
    ...
    ArgChecker.isTrue(!dateConversion.isAfter(_fixingDates[0].toLocalDate()), "toDerivative without time series should have a date before the first fixing date.");
