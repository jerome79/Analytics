...
public class CouponONDefinition extends CouponDefinition implements InstrumentDefinitionWithData<Payment, DoubleTimeSeries<ZonedDateTime>> {
  ...
  public Coupon toDerivative(final ZonedDateTime valZdt, final DoubleTimeSeries<ZonedDateTime> indexFixingTimeSeries) {
    ...
    while ((fixedPeriod < _fixingPeriodDate.length - 1) &&
      ...
      if (!fixedRate.isPresent()) {
        ...
        if (currentDate.isAfter(latestDate)) {
          throw new IllegalStateException("Could not get fixing value of index " + _index.getName() + " for date " + currentDate + ". The last data is available on " + latestDate);
        }
    ...
    if (fixedPeriod < _fixingPeriodDate.length - 1) { // Some OIS period left
      ...
      OptionalDouble fixing = indexFixingDateSeries.get(_fixingPeriodDate[fixedPeriod].toLocalDate());
      if (fixing.isPresent()) {
        accruedNotional *= 1 + _fixingPeriodAccrualFactor[fixedPeriod] * fixing.getAsDouble();
        fixedPeriod++;
      }
      ...
      if (fixedPeriod < _fixingPeriodDate.length - 1) { // More OIS period left
        ...
        return new CouponON(getCurrency(), paymentTime, getPaymentYearFraction(), getNotional(), _index, fixingPeriodStartTime,
