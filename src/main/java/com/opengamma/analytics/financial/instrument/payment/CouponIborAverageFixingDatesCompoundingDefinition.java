...
public class CouponIborAverageFixingDatesCompoundingDefinition extends CouponDefinition
  ...
  public CouponIborAverageFixingDatesCompoundingDefinition(final Currency currency, final ZonedDateTime paymentDate,
      ...
      final double[][] weights, final HolidayCalendar iborCalendar) {
    ...
    ArgChecker.isTrue(nPeriods == weights.length, "weights length different from fixingDate length");
    ...
    for (int i = 0; i < nPeriods; ++i) {
      ...
      for (int j = 0; j < nDates[i]; ++j) {
        ...
        _fixingPeriodAccrualFactors[i][j] = DayCountUtils.yearFraction(index.getDayCount(), _fixingPeriodStartDates[i][j], _fixingPeriodEndDates[i][j], iborCalendar);
      }
    }
  }
  ...
  public CouponIborAverageFixingDatesCompoundingDefinition(final Currency currency, final ZonedDateTime paymentDate,
    ...
    final int nPeriods = fixingDates.length;
    ArgChecker.isTrue(nPeriods == weights.length, "weights length different from fixingDate length");
    ArgChecker.isTrue(nPeriods == fixingPeriodStartDates.length, "fixingPeriodStartDates length different from fixingDate length");
    ArgChecker.isTrue(nPeriods == fixingPeriodEndDates.length, "fixingPeriodEndDates length different from fixingDate length");
    ArgChecker.isTrue(nPeriods == fixingPeriodAccrualFactors.length, "fixingPeriodAccrualFactors length different from fixingDate length");
    ArgChecker.isTrue(currency.equals(index.getCurrency()), "index currency different from payment currency");
    _weights = new double[nPeriods][];
    _fixingDates = new ZonedDateTime[nPeriods][];
    _fixingPeriodStartDates = new ZonedDateTime[nPeriods][];
  ...
  public CouponIborAverageFixingDatesCompounding toDerivative(final ZonedDateTime date) {
    ...
    ArgChecker.isTrue(!dayConversion.isAfter(getFixingDates()[0][0].toLocalDate()), "Do not have any fixing data but are asking for a derivative at " + date
  ...
  public Coupon toDerivative(ZonedDateTime dateTime, DoubleTimeSeries<ZonedDateTime> indexFixingTimeSeries) {
    ...
    ArgChecker.isTrue(!dateConversion.isAfter(getPaymentDate().toLocalDate()), "date is after payment date");
    ...
    while (posPeriod < nPeriods && !(dayConversion.isBefore(getFixingDates()[posPeriod][0].toLocalDate()))) {
      ...
      while (posDate < nDates[posPeriod] && dayConversion.isAfter(getFixingDates()[posPeriod][posDate].toLocalDate())) {
        ...
        if (fixedRate == null) {
          throw new IllegalStateException("Could not get fixing value for date " + getFixingDates()[posPeriod][posDate]);
        }
