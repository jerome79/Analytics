...
import com.opengamma.analytics.financial.credit.isdastandardmodel.DoublesScheduleGenerator;
...
public class PortfolioSwapAdjustment {
  ...
  public IntrinsicIndexDataBundle adjustCurves(final double indexPUF, final CDSAnalytic indexCDS, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
    ...
    ArgChecker.isTrue(indexCoupon < 10, "indexCoupon should be a fraction. The value of {} would be a coupon of {}", indexCoupon, indexCoupon * 1e4);
  ...
  public IntrinsicIndexDataBundle adjustCurves(final double[] indexPUF, final CDSAnalytic[] indexCDS, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
    ...
    ArgChecker.isTrue(nIndexTerms == indexPUF.length, "number of indexCDS ({}) does not match number of indexPUF ({})", nIndexTerms, indexPUF.length);
    if (nIndexTerms == 1) {
    ...
    for (int i = 0; i < nCurves; i++) {
      if (creditCurves[i] == null) {
        ...
        final double[] comKnots = DoublesScheduleGenerator.combineSets(ccKnots, indexKnots);
