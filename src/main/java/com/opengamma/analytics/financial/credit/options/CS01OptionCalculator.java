...
public class CS01OptionCalculator {
  ...
  public double indexCurveApprox(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic[] pillarCDS, final CDSQuoteConvention[] quotes, final double indexCoupon,
    ...
    ArgChecker.isTrue(Math.abs(bumpAmount) > BUMP_MIN, "Bump given as {}, but must have abs value greater than {}", bumpAmount, BUMP_MIN);
    ArgChecker.isTrue(Math.abs(bumpAmount) < BUMP_MAX, "Bump given as {}, but must have abs value less than {}. Bump must be given as fraction", bumpAmount, BUMP_MAX);
