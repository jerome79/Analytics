...
public class BlackIndexOptionPricer {
  private static final Logger LOGGER = LoggerFactory.getLogger(BlackIndexOptionPricer.class.getName());
  ...
  private BlackIndexOptionPricer(final CDSAnalytic fwdCDS, final double timeToExpiry, final ISDACompliantYieldCurve yieldCurve, final double indexCoupon, final double[] fwdSpreadAndAnnity) {
    ArgChecker.notNull(fwdCDS, "fwdCDS");
    ArgChecker.isTrue(timeToExpiry > 0.0, "timeToExpiry must be positive. Value given {}", timeToExpiry);
    ArgChecker.isTrue(fwdCDS.getEffectiveProtectionStart() == 0.0, "fwdCDS should be a Forward CDS");
    ArgChecker.notNull(yieldCurve, "yieldCurve");
    ArgChecker.isTrue(indexCoupon > 0.0, "indexCoupon must be positive");
    ArgChecker.isTrue(fwdSpreadAndAnnity.length == 2, "too many parameters passed in");
    final double defaultAdjustedFwdSpread = fwdSpreadAndAnnity[0];
    final double pvFwdAnnuity = fwdSpreadAndAnnity[1];
    ArgChecker.isTrue(defaultAdjustedFwdSpread > 0.0, "defaultAdjustedFwdSpread must be positive");
    if (indexCoupon > 1) {
    ...
    ArgChecker.isTrue(pvFwdAnnuity > 0, "pvFwdAnnuity must be positive");
    ArgChecker.isTrue(pvFwdAnnuity < fwdCDS.getProtectionEnd() * 1.1, "Value of annuity of {} is greater than length (in years) of forward CDS. Annuity should be given for unit notional",
        pvFwdAnnuity);
  ...
  public double getOptionPremium(final IndexOptionStrike strike, final double vol, final boolean isPayer) {
    ArgChecker.notNull(strike, "strike");
  ...
  public double getOptionPriceForSpreadQuotedIndex(final double strike, final double vol, final boolean isPayer) {
    ArgChecker.isTrue(strike >= 0.0, "strike cannot be negative");
  ...
  public double getOptionPriceForPriceQuotedIndex(final double gK, final double vol, final boolean isPayer) {
    ArgChecker.isTrue(gK >= _minExercisePrice && gK < _maxExercisePrice, "The exercise price must be in the range {} to {} - value of {} is outside this", _minExercisePrice, _maxExercisePrice,
        gK);
