...
public class CreditCurveCalibrator {
  ...
  public CreditCurveCalibrator(final CDSAnalytic[] cds, final ISDACompliantYieldCurve yieldCurve, final AccrualOnDefaultFormulae formula, final ISDACompliantCreditCurveBuilder.ArbitrageHandling arbHandle) {
    ...
    for (int i = 1; i < _nCDS; i++) {
      ...
      ArgChecker.isTrue(effectProtStart == cds[i].getEffectiveProtectionStart(), "All CDSs must has same effective protection start");
    ...
    //TODO this will not match ISDA C for forward starting (i.e. accStart > tradeDate) CDS, and will give different answers 
    //if the Markit 'fix' is used in that case
    final double[] knots = DoublesScheduleGenerator.getIntegrationsPoints(effectProtStart, _t[_nCDS - 1], yieldCurve.getKnotTimes(), _t);
