...
public class LegalEntityCreditRatings implements LegalEntityFilter<LegalEntity>, Bean {
  ...
  private Set<String> perAgencyRatings;

  /**
   * True if the rating description is to be used as a filter.
   */
  @PropertyDefinition
  private boolean useRatingDescription;

  /**
   * A set of agencies to be used to filter by rating description.
   */
  @PropertyDefinition(validate = "notNull", set = "manual")
  private Set<String> perAgencyRatingDescriptions;
  ...
  public Object getFilteredData(final LegalEntity legalEntity) {
    ...
    if (!(useRating || useRatingDescription)) {
    ...
    for (final CreditRating creditRating : creditRatings) {
      ...
      if (useRating) {
        final Pair<String, String> agencyRatingPair = Pair.of(agencyName, creditRating.getRating());
        if (perAgencyRatings.isEmpty()) {
          selections.add(agencyRatingPair);
        }
        if (perAgencyRatings.contains(agencyName)) {
          selections.add(agencyRatingPair);
          ratingCount++;
        }
      }
      if (useRatingDescription) {
        ...
        final Pair<String, String> agencyRatingDescription = Pair.of(agencyName, creditRating.getRatingDescription());
        if (perAgencyRatingDescriptions.isEmpty()) {
          selections.add(agencyRatingDescription);
        }
        if (perAgencyRatingDescriptions.contains(agencyName)) {
          selections.add(agencyRatingDescription);
          ratingDescriptionCount++;
        }
      }
    }
    if (useRating && ratingCount != perAgencyRatings.size()) {
      throw new IllegalStateException("Credit ratings " + creditRatings + " do not contain matches for " + perAgencyRatings);
    }
    if (useRatingDescription && ratingDescriptionCount != perAgencyRatingDescriptions.size()) {
      throw new IllegalStateException("Credit ratings " + creditRatings + " do not contain matches for " + perAgencyRatingDescriptions);
    }
    return selections;
  }
  ...
  /**
   * Sets the agencies with which to filter ratings. This also sets the {@link LegalEntityCreditRatings#useRating} field to true.
   * 
  ...
  public void setPerAgencyRatings(final Set<String> perAgencyRatings) {
    ...
    this.perAgencyRatings = perAgencyRatings;
  }

  /**
   * Sets the agencies with which to filter rating descriptions. This also sets the {@link LegalEntityCreditRatings#useRatingDescription} field to true.
   * 
  ...
  public void setPerAgencyRatingDescriptions(final Set<String> perAgencyRatingDescriptions) {
    ...
    this.perAgencyRatingDescriptions = perAgencyRatingDescriptions;
  }
  ...
  public void setUseRatingDescription(boolean useRatingDescription) {
    this.useRatingDescription = useRatingDescription;
  }
  ...
  public static class Meta extends DirectMetaBean {
    ...
    protected void validate(Bean bean) {
      JodaBeanUtils.notNull(((LegalEntityCreditRatings) bean).perAgencyRatings, "perAgencyRatings");
      JodaBeanUtils.notNull(((LegalEntityCreditRatings) bean).perAgencyRatingDescriptions, "perAgencyRatingDescriptions");
    }
