...
public class CrossGammaMultiCurveCalculator {
  ...
  public CrossGammaMultiCurveCalculator(final double shift,
    ...
    ArgChecker.isTrue(Math.abs(shift) > 1.0E-10, "shift should not be larger in absolute value than 1.0E-10");
  ...
  private Currency checkUniqueCurrency(final MulticurveProviderDiscount multicurve) {
    ...
    ArgChecker.isTrue(ccys.size() == 1, "only one currency allowed for multi-curve gamma");
    Currency ccy = ccys.iterator().next();
    Set<IborIndex> iborIndexes = multicurve.getIndexesIbor();
    for (IborIndex index : iborIndexes) {
      ArgChecker.isTrue(index.getCurrency().equals(ccy), "Ibor index should be in the same currency as discounting curve");
    }
    Set<IndexON> onIndexes = multicurve.getIndexesON();
    for (IndexON index : onIndexes) {
      ArgChecker.isTrue(index.getCurrency().equals(ccy), "Overnight index should be in the same currency as discounting curve");
    }
    return ccy;
  }
