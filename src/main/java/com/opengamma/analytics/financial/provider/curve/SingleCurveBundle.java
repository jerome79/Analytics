...
public class SingleCurveBundle<T extends GeneratorCurve> {
  ...
  public SingleCurveBundle(final String curveName, final InstrumentDerivative[] derivatives,
    ...
    ArgChecker.notNull(curveGenerator, "curve generator");
    ArgChecker.isTrue(derivatives.length == startingPoint.length, "Must have one starting point per derivative");
