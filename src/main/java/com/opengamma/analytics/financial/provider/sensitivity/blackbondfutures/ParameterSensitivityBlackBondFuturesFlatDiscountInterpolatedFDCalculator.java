...
public class ParameterSensitivityBlackBondFuturesFlatDiscountInterpolatedFDCalculator {
   ...
   */
  private final InstrumentDerivativeVisitor<BlackBondFuturesProviderInterface, MultiCurrencyAmount> _valueCalculator;
  /**
   ...
   */
  public ParameterSensitivityBlackBondFuturesFlatDiscountInterpolatedFDCalculator(final InstrumentDerivativeVisitor<BlackBondFuturesProviderInterface, MultiCurrencyAmount> valueCalculator,
      final double shift) {
  ...
  public MultipleCurrencyParameterSensitivity calculateSensitivity(final InstrumentDerivative instrument, final BlackBondFuturesFlatProviderDiscount black) {
    ...
    for (final Pair<Object, LegalEntityFilter<LegalEntity>> ic : issuerCcies) {
      ...
      for (int loopnode = 0; loopnode < nbNodePoint; loopnode++) {
        ...
        final MultiCurrencyAmount pvBumpedPlus = instrument.accept(_valueCalculator, providerIcBumpedPlus);
        final double[] yieldBumpedMinus = curveInt.getYDataAsPrimitive().clone();
        yieldBumpedMinus[loopnode] -= _shift;
        ...
        final MultiCurrencyAmount pvBumpedMinus = instrument.accept(_valueCalculator, providerIcBumpedMinus);
        final MultiCurrencyAmount pvDiff = pvBumpedPlus.plus(pvBumpedMinus.multipliedBy(-1.0));
        for (int loopccypv = 0; loopccypv < nbCcy; loopccypv++) {
          sensitivity[loopccypv][loopnode] = pvDiff.getAmount(ccyList.get(loopccypv)).getAmount() / (2 * _shift);
        }
