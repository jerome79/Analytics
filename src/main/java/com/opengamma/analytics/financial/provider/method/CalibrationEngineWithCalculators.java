...
public abstract class CalibrationEngineWithCalculators<DATA_TYPE extends ParameterProviderInterface> extends CalibrationEngineWithPrices<DATA_TYPE> {
  ...
  */
  private final List<InstrumentDerivativeVisitor<DATA_TYPE, MultiCurrencyAmount>> _calculators;
  ...
  public void computeCalibrationPrice(final DATA_TYPE data) {
    ...
    for (int loopins = 0; loopins < nbInstrument; loopins++) {
      final MultiCurrencyAmount pvMCA = getBasket().get(loopins).accept(_calculators.get(loopins), data);
   ...
   * @return the method.
   */
  public List<InstrumentDerivativeVisitor<DATA_TYPE, MultiCurrencyAmount>> getMethod() {
    return _calculators;
  }
