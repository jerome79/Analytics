...
public class VarianceSwap implements InstrumentDerivative {
  ...
  public VarianceSwap(final double timeToObsStart, final double timeToObsEnd, final double timeToSettlement, final double varStrike, final double varNotional, final Currency currency,
    ...
    ArgChecker.isTrue(varStrike >= 0, "Require varStrike >= 0");
    ArgChecker.isTrue(varNotional > 0, "Require varNotional > 0");
    ArgChecker.notNull(currency, "currency");
    ArgChecker.isTrue(annualizationFactor > 0, "Require annualizationFactor > 0");
    ArgChecker.isTrue(nObsExpected > 0, "Encountered a VarianceSwap with 0 expected observations");
    ArgChecker.notNull(observations, "observations");
    ArgChecker.notNull(observationWeights, "observationWeights");
    ArgChecker.isTrue(nObsExpected >= observations.length, "Number of observations ({}) great than expected ({})", observations.length, nObsExpected);
    ...
    if (_observationWeights.length > 1) {
      ...
      ArgChecker.isTrue(nWeights + 1 == nObs, "If provided, observationWeights must be of length one less than observations, as they weight returns log(obs[i]/obs[i-1])."
  ...
  public VarianceSwap(final VarianceSwap other) {
    ArgChecker.notNull(other, "variance swap to copy");
