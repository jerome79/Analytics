...
public class CarrLeeFXVolatilitySwapCalculator extends InstrumentDerivativeVisitorAdapter<CarrLeeFXData, VolatilitySwapCalculatorResult> {
  ...
  public CarrLeeFXVolatilitySwapCalculator(final double lowestPutDelta, final double highestCallDelta, final int numPoints) {
    ArgChecker.isTrue(lowestPutDelta < 0. && lowestPutDelta > -1., "-1 < lowestPutDelta < 0 should be true");
    ArgChecker.isTrue(highestCallDelta > 0. && highestCallDelta < 1., "0 < highestCallDelta < 1 should be true");
    ArgChecker.isTrue(numPoints > 2, "numPoints should be greater than 2");
  ...
  public CarrLeeFXVolatilitySwapCalculator(final int numPoints, final double[] strikeRange) {
    ArgChecker.isTrue(numPoints > 2, "numPoints should be greater than 2");
    ArgChecker.notNull(strikeRange, "strikeRange");
    ArgChecker.isTrue(strikeRange.length == 2, "length of strikeRange should be 2");
    ArgChecker.isTrue(strikeRange[0] < strikeRange[1], "upper bound should be greater than lower bound");
  ...
  public VolatilitySwapCalculatorResultWithStrikes visitFXVolatilitySwap(final FXVolatilitySwap swap, final CarrLeeFXData data) {
    ...
    ArgChecker.isTrue(Doubles.isFinite(domesticRate), "domestic rate should be finite");
    ArgChecker.isTrue(Doubles.isFinite(foreignRate), "foreign rate should be finite");
    ...
    if (_strikeRange == null) {
      ...
      ArgChecker.isTrue((forward > strikeRange[0] && forward < strikeRange[1]), "forward is outside of strike range");
    }
