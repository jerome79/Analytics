...
public class CarrLeeSeasonedSyntheticVolatilitySwapCalculator {
  ...
  public VolatilitySwapCalculatorResult evaluate(final double spot, final double[] putStrikes, final double[] callStrikes, final double timeToExpiry, final double timeFromInception,
    ...
    ArgChecker.notNull(callStrikes, "callStrikes");
    ArgChecker.notNull(putStrikes, "putStrikes");
    ArgChecker.notNull(callVols, "callVols");
    ArgChecker.notNull(putVols, "putVols");

    final int nCalls = callStrikes.length;
    final int nPuts = putStrikes.length;

    ArgChecker.isTrue(callVols.length == nCalls, "callVols.length == callStrikes.length should hold");
    ArgChecker.isTrue(putVols.length == nPuts, "putVols.length == putStrikes.length should hold");
    ArgChecker.isTrue(Doubles.isFinite(spot), "spot should be finite");
    ArgChecker.isTrue(spot > 0., "spot should be positive");
    ArgChecker.isTrue(Doubles.isFinite(timeToExpiry), "timeToExpiry should be finite");
    ArgChecker.isTrue(timeToExpiry > 0., "timeToExpiry should be positive");
    ArgChecker.isTrue(Doubles.isFinite(timeFromInception), "timeFromInception should be finite");
    ArgChecker.isTrue(timeFromInception > 0., "timeFromInception should be positive");
    ...
    ArgChecker.isTrue(Doubles.isFinite(rvReturns), "rvReturns should be finite");
    ArgChecker.isTrue(rvReturns > 0., "rvReturns should be positive");

    final double deltaK = (callStrikes[nCalls - 1] - putStrikes[0]) / (nCalls + nPuts - 1);
    for (int i = 0; i < nCalls; ++i) {
      ArgChecker.isTrue(Doubles.isFinite(callStrikes[i]), "callStrikes should be finite");
      ArgChecker.isTrue(callStrikes[i] > 0., "callStrikes should be positive");
      ArgChecker.isTrue(Doubles.isFinite(callVols[i]), "callVols should be finite");
      ArgChecker.isTrue(callVols[i] > 0., "callVols should be positive");
      if (i < nCalls - 1) {
        ArgChecker.isTrue(Math.abs(callStrikes[i + 1] - callStrikes[i] - deltaK) < EPS, "All of the strikes  should be equally spaced");
      }
    }
    for (int i = 0; i < nPuts; ++i) {
      ArgChecker.isTrue(Doubles.isFinite(putStrikes[i]), "putStrikes should be finite");
      ArgChecker.isTrue(putStrikes[i] > 0., "putStrikes should be positive");
      ArgChecker.isTrue(Doubles.isFinite(putVols[i]), "putVols should be finite");
      ArgChecker.isTrue(putVols[i] > 0., "putVols should be positive");
      if (i < nPuts - 1) {
        ArgChecker.isTrue(Math.abs(putStrikes[i + 1] - putStrikes[i] - deltaK) < EPS, "All of the strikes  should be equally spaced");
      }
    }

    final double rate = interestRate - dividend;
    final double forward = spot * Math.exp(rate * timeToExpiry);
    ArgChecker.isTrue((callStrikes[0] >= forward && putStrikes[nPuts - 1] <= forward), "Max(putStrikes) <= forward <= Min(callStrikes) should hold");
