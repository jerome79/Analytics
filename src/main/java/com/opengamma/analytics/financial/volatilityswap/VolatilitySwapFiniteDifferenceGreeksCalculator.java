...
public class VolatilitySwapFiniteDifferenceGreeksCalculator {
  ...
  private CarrLeeFXData getSpotBumpedData(final CarrLeeFXData data) {
    final FxMatrix spotBumpedfxMatrix =
        FxMatrix.builder()
            .addRate(data.getCurrencyPair().getFirst(), data.getCurrencyPair().getSecond(), data.getSpot() + _bumpSpot)
            .build();
    ...
    if (provider instanceof MulticurveProviderDiscount) {
      ...
      spotBumpedCurves = new MulticurveProviderDiscount(discountCurves.getDiscountingCurves(), new LinkedHashMap<>(),
          new LinkedHashMap<>(), spotBumpedfxMatrix);
    } else {
