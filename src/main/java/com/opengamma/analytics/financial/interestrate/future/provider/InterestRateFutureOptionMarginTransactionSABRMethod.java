...
public final class InterestRateFutureOptionMarginTransactionSABRMethod extends InterestRateFutureOptionMarginTransactionGenericMethod<SABRSTIRFuturesProviderInterface> {
   ...
   */
  public MultiCurrencyAmount presentValueFromFuturePrice(final InterestRateFutureOptionMarginTransaction transaction, final SABRSTIRFuturesProviderInterface sabrData, final double priceFuture) {
    ArgChecker.notNull(transaction, "Transaction on option on STIR futures");
    ArgChecker.notNull(sabrData, "SABR / multi-curves provider");
    double priceSecurity = getSecurityMethod().priceFromFuturePrice(transaction.getUnderlyingSecurity(), sabrData, priceFuture);
    MultiCurrencyAmount priceTransaction = presentValueFromPrice(transaction, priceSecurity);
