...
public class CouponIborAverage extends CouponFloating {
  ...
  public CouponIborAverage(final Currency currency, final double paymentTime, final double paymentYearFraction, final double notional, final double fixingTime,
    ...
    ArgChecker.isTrue(fixingPeriodStartTime1 >= fixingTime, "fixing period start < fixing time");
    _fixingPeriodStartTime1 = fixingPeriodStartTime1;
    ArgChecker.isTrue(fixingPeriodEndTime1 >= fixingPeriodStartTime1, "fixing period end < fixing period start");
    _fixingPeriodEndTime1 = fixingPeriodEndTime1;
    ArgChecker.isTrue(fixingYearFraction1 >= 0, "forward year fraction < 0");
    _fixingAccrualFactor1 = fixingYearFraction1;
    ArgChecker.notNull(index1, "Index");
    ArgChecker.isTrue(currency.equals(index1.getCurrency()), "Index currency incompatible with coupon currency");
    _index1 = index1;
    _weight1 = weight1;
    ArgChecker.isTrue(fixingPeriodStartTime2 >= fixingTime, "fixing period start < fixing time");
    _fixingPeriodStartTime2 = fixingPeriodStartTime2;
    ArgChecker.isTrue(fixingPeriodEndTime2 >= fixingPeriodStartTime2, "fixing period end < fixing period start");
    _fixingPeriodEndTime2 = fixingPeriodEndTime2;
    ArgChecker.isTrue(fixingYearFraction2 >= 0, "forward year fraction < 0");
    _fixingAccrualFactor2 = fixingYearFraction2;
    ArgChecker.notNull(index2, "Index");
    ArgChecker.isTrue(currency.equals(index2.getCurrency()), "Index currency incompatible with coupon currency");
    _index2 = index2;
    _weight2 = weight2;
  }
