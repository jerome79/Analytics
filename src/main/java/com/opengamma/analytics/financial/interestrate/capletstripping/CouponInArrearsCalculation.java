...
public class CouponInArrearsCalculation {
  ...
  public CouponInArrearsCalculation(CapletStripper stripper, List<CapFloor> caps, double[] mrkPrices,
    ...
    ArgChecker.notNull(stripper, "stripper");
    ArgChecker.notNull(caps, "caps");
    ArgChecker.notNull(type, "type");
    ArgChecker.notNull(guess, "guess");
    ArgChecker.notNull(curves, "curves");

    ArgChecker.notNull(mrkPrices, "mrkPrices");
    ArgChecker.notNull(errors, "errors");
  ...
  public double simpleCapletPrice(CapFloorIbor caplet) {
    ArgChecker.notNull(caplet, "caplet");
   ...
   */
  public MultiCurrencyAmount presentValue(CapFloorIbor caplet, final GeneralSmileInterpolator interpolator) {
    ArgChecker.notNull(caplet, "caplet");
   ...
   * @return The present value
   */
  public MultiCurrencyAmount presentValue(CouponIbor couponIbor, final GeneralSmileInterpolator interpolator) {
    ArgChecker.notNull(couponIbor, "couponIbor");
    ArgChecker.notNull(interpolator, "interpolator");
   ...
   * @return The present value
   */
  public MultiCurrencyAmount presentValue(Swap<? extends Payment, ? extends Payment> swap,
      final GeneralSmileInterpolator interpolator) {
    ArgChecker.notNull(swap, "swap");
    ArgChecker.notNull(interpolator, "interpolator");

    MultiCurrencyAmount firstPV;
    MultiCurrencyAmount secondPV;
  ...
  private MultipleCurrencyParameterSensitivity pointToParameterSensitivity(
    ...
    for (final Currency ccySensi : sensitivity.getCurrencies()) {
      ...
      for (final Map.Entry<String, List<DoublesPair>> entry : sensitivityDsc.entrySet()) {
        if (_curves.getAllNames().contains(entry.getKey())) {
          result = result
              .plus(Pair.of(entry.getKey(), ccySensi),
    ...
    for (final Currency ccySensi : sensitivity.getCurrencies()) {
      ...
      for (final Map.Entry<String, List<ForwardSensitivity>> entry : sensitivityFwd.entrySet()) {
        if (_curves.getAllNames().contains(entry.getKey())) {
          result = result.plus(Pair.of(entry.getKey(), ccySensi),
  ...
  public MultipleCurrencyParameterSensitivity presentValueCurveSensitivity(
      ...
      final GeneralSmileInterpolator interpolator) {
    ArgChecker.notNull(swap, "swap");
    ArgChecker.notNull(blockBundle, "blockBundle");
    ArgChecker.notNull(interpolator, "interpolator");
  ...
  public Double parRate(final Swap<? extends Payment, ? extends Payment> swap,
    ...
    MultiCurrencyAmount pvSecond;

    Annuity<? extends Payment> annuity = swap.getFirstLeg();
    ArgChecker.isTrue(annuity.getNthPayment(0) instanceof Coupon, "The first leg should be coupon payment");
    double pvbp = 0;
    ...
    double pvSecondDouble = pvSecond.getAmount(swap.getSecondLeg().getCurrency()).getAmount() *
