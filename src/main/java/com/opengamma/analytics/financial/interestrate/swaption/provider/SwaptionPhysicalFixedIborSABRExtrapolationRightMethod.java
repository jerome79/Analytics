...
public class SwaptionPhysicalFixedIborSABRExtrapolationRightMethod {
  ...
  public MultiCurrencyAmount presentValue(final SwaptionPhysicalFixedIbor swaption, final SABRSwaptionProviderInterface sabrData) {
    ...
    if (strikeModified <= _cutOffStrike) { // No extrapolation
      ...
      return MultiCurrencyAmount.of(ccy, func.evaluate(dataBlack) * (swaption.isLong() ? 1.0 : -1.0));
    }
    // With extrapolation
    ...
    return MultiCurrencyAmount.of(ccy, pvbpModified * sabrExtrapolation.price(option) * (swaption.isLong() ? 1.0 : -1.0));
  }
