...
public class CapFloorIborInArrearsSABRCapGenericReplicationMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final CapFloorIbor cap, final SABRCapProviderInterface sabr) {
    ArgChecker.notNull(cap, "The cap/floor shoud not be null");
    ArgChecker.notNull(sabr, "SABR cap provider");
    ...
    final double strikePart = (1.0 + cap.getFixingAccrualFactor() * cap.getStrike()) * _baseMethod.presentValue(capStandard, sabr).getAmount(ccy).getAmount();
  ...
  private static final class InArrearsIntegrant extends Function1D<Double, Double> {
    ...
    public Double evaluate(final Double x) {
      ...
      return _basePricingMethod.presentValue(capStrike, _sabrData).getAmount(_capStandard.getCurrency()).getAmount();
    }
  }
