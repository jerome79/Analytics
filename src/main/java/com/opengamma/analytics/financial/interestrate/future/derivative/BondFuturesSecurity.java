...
public class BondFuturesSecurity extends FuturesSecurity {
  ...
  public BondFuturesSecurity(final double tradingLastTime, final double noticeFirstTime, final double noticeLastTime, final double deliveryFirstTime,
    ...
    ArgChecker.notNull(deliveryBasketAtSpotDate, "Delivery basket at spot date");
    ArgChecker.notNull(conversionFactor, "Conversion factors");
    ArgChecker.isTrue(deliveryBasketAtDeliveryDate.length > 0, "At least one bond in basket");
    ArgChecker.isTrue(deliveryBasketAtDeliveryDate.length == conversionFactor.length, "Conversion factor size");
    ArgChecker.isTrue(deliveryBasketAtDeliveryDate.length == deliveryBasketAtSpotDate.length, "Delivery basket size");
