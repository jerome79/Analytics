...
public final class InterestRateFutureOptionMarginTransactionBlackSmileMethod extends InterestRateFutureOptionMarginTransactionGenericMethod<BlackSTIRFuturesProviderInterface> {
   ...
   */
  public MultiCurrencyAmount presentValueFromFuturePrice(final InterestRateFutureOptionMarginTransaction transaction, final BlackSTIRFuturesProviderInterface blackData,
      final double priceFuture) {
    ArgChecker.notNull(transaction, "Transaction on option on STIR futures");
    ArgChecker.notNull(blackData, "Black / multi-curves provider");
    ...
    final MultiCurrencyAmount priceTransaction = presentValueFromPrice(transaction, priceSecurity);
