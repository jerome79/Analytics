...
public class CouponIborFxReset extends Coupon implements DepositIndexCoupon<IborIndex> {
  ...
  public CouponIborFxReset(final Currency currency, final double paymentTime, final double paymentYearFraction,
    ...
    ArgChecker.isTrue(iborIndexFixingPeriodStartTime >= iborIndexFixingTime,
        "ibor index fixing period start < ibor index fixing time");
    ArgChecker.isTrue(iborIndexFixingPeriodEndTime >= iborIndexFixingPeriodStartTime,
        "ibor index fixing period end < ibor index fixing period start");
    ArgChecker.isTrue(iborIndexFixingYearFraction >= 0, "ibor index fixing year fraction < 0");
    ArgChecker.notNull(index, "Index");
    ArgChecker.notNull(referenceCurrency, "reference currency");
    ArgChecker.isTrue(iborIndexFixingTime >= 0.0, "ibor index fixing time < 0");
