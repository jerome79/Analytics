    ...
    for (final String name : curves.getAllNames()) {
      if (senseA.containsKey(name)) {
        ...
        for (int i = 0; i < (m - 1); i++) {
          ...
          temp.add(DoublesPair.of(pair.getFirst(), factor * pair.getSecond()));
        }
        final DoublesPair pair = list.get(m - 1);
        temp.add(DoublesPair.of(pair.getFirst(), principlePayment.getPaymentTime() * df / a + factor * pair.getSecond()));
