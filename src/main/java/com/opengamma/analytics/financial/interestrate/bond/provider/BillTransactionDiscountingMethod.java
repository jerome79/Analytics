...
public final class BillTransactionDiscountingMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final BillTransaction bill, final IssuerProviderInterface issuer) {
    ArgChecker.notNull(bill, "Bill");
    ArgChecker.notNull(issuer, "Issuer and multi-curves provider");
    final Currency ccy = bill.getCurrency();
    final MultiCurrencyAmount pvBill = METHOD_SECURITY.presentValue(bill.getBillPurchased(), issuer);
    ...
    return pvBill.multipliedBy(bill.getQuantity()).plus(MultiCurrencyAmount.of(ccy, pvSettle));
  }
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValueFromYield(final BillTransaction bill, final double yield, final IssuerProviderInterface issuer) {
    ArgChecker.notNull(bill, "Bill");
    ArgChecker.notNull(issuer, "Issuer and multi-curves provider");
    final Currency ccy = bill.getCurrency();
    final MultiCurrencyAmount pvSecurity = METHOD_SECURITY.presentValueFromYield(bill.getBillStandard(), yield, issuer);
    final double pvSettle = bill.getSettlementAmount() * issuer.getMulticurveProvider().getDiscountFactor(ccy, bill.getBillPurchased().getSettlementTime());
    return pvSecurity.plus(MultiCurrencyAmount.of(bill.getCurrency(), pvSettle));
  }
