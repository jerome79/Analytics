...
public final class FuturesPriceMulticurveCalculator extends InstrumentDerivativeVisitorAdapter<ParameterProviderInterface, Double> {
  ...
  public Double visitSwapFuturesPriceDeliverableSecurity(final SwapFuturesPriceDeliverableSecurity futures, final ParameterProviderInterface multicurve) {
    ...
    MultiCurrencyAmount pv = futures.getUnderlyingSwap().accept(PresentValueDiscountingCalculator.getInstance(), multicurve.getMulticurveProvider());
    return 1.0d + pv.getAmount(futures.getCurrency()).getAmount() / multicurve.getMulticurveProvider().getDiscountFactor(futures.getCurrency(), futures.getDeliveryTime());
  }
