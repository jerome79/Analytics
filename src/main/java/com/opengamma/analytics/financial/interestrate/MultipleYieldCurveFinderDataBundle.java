...
public class MultipleYieldCurveFinderDataBundle {
  ...
  public MultipleYieldCurveFinderDataBundle(final List<InstrumentDerivative> derivatives, final double[] marketValues, final YieldCurveBundle knownCurves,
    ...
    ArgChecker.noNulls(derivatives, "derivatives");
    ArgChecker.notNull(marketValues, "market values null");
    ArgChecker.notNull(unknownCurveNodePoints, "unknown curve node points");
    ArgChecker.notNull(unknownCurveInterpolators, "unknown curve interpolators");
    ArgChecker.notEmpty(unknownCurveNodePoints, "unknown curve node points");
    ArgChecker.notEmpty(unknownCurveInterpolators, "unknown curve interpolators");
    ArgChecker.isTrue(derivatives.size() == marketValues.length, "marketValues wrong length; must be one par rate per derivative (have {} values for {} derivatives",
    ...
    while (nodePointsIterator.hasNext()) {
      ...
      ArgChecker.notNull(entry1.getValue(), "curve node points for " + name1);
      ArgChecker.notNull(entry2.getValue(), "interpolator for " + name1);
      _names.add(name1);
    }
    int nNodes = 0;
