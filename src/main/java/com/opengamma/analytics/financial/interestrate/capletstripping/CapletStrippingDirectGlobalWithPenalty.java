...
public class CapletStrippingDirectGlobalWithPenalty {
  ...
  public CapletStrippingDirectGlobalWithPenalty(final List<CapFloor>[] caps, final MulticurveProviderInterface curves, final CapletVolatilityNodalSurfaceProvider nodalSurfaceProvider,
    ...
    ArgChecker.noNulls(caps, "caps");
    ArgChecker.notNull(curves, "curves");
    ArgChecker.notNull(nodalSurfaceProvider, "nodalSurfaceProvider");
    ArgChecker.isTrue(Doubles.isFinite(lambdaStrike), "lambdaStrike should be finite");
    ArgChecker.isTrue(lambdaStrike >= 0, "lambdaStrike should be non-negative");
    ArgChecker.isTrue(Doubles.isFinite(lambdaTime), "lambdaTime should be finite");
    ArgChecker.isTrue(lambdaTime >= 0, "lambdaTime should be non-negative");
  ...
  public LeastSquareResults solveForVol(final double[][] capVols) {
    ArgChecker.noNulls(capVols, "capVols");
  ...
  public LeastSquareResults solveForVol(final double[][] capVols, final double[][] errors, final double[][] guess) {
    ArgChecker.noNulls(capVols, "capVols");
    ArgChecker.noNulls(errors, "errors");
    ArgChecker.noNulls(guess, "guess");
  ...
  private DoubleMatrix1D toVector(final double[][] doubleArray, final int length) {
    ...
    ArgChecker.isTrue(k == length, "number of elements in input is different form expected vector length");
