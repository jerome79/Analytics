...
public abstract class CapletStrippingResult {
  ...
  public CapletStrippingResult(DoubleMatrix1D fitParms, DiscreteVolatilityFunction func, MultiCapFloorPricer pricer) {
    ArgChecker.notNull(fitParms, "fitParms");
  ...
  public void printCapletVols(PrintStream out) {
    ...
    for (int i = 0; i < n; i++) {
      out.println(expStrikes[i].getFirst() + "\t" + expStrikes[i].getSecond() + "\t" + vols.getEntry(i));
    }
    out.println();
  }
  ...
  public void printSurface(PrintStream out, int nExpPoints, int nStrikePoints) {
    ArgChecker.notNull(out, "out");
    ArgChecker.isTrue(nExpPoints > 1, "need at least 2 expiry points");
    ArgChecker.isTrue(nExpPoints > 2, "need at least 2 strike points");
