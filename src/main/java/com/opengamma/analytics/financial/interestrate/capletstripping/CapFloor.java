...
public class CapFloor extends Annuity<CapFloorIbor> {
  ...
  public CapFloor(final CapFloorIbor[] payments) {
    super(payments);
    ArgChecker.notEmpty(payments, "no payments given");
    final double k = payments[0].getStrike();
    final boolean isCap = payments[0].isCap();
    final int n = payments.length;
    for (int i = 1; i < n; i++) {
      ArgChecker.isTrue(payments[i].getStrike() == k, "Different strikes");
      ArgChecker.isTrue(payments[i].isCap() == isCap, "Mixed caplets and floorlets");
    }
  }
