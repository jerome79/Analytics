...
public class SwaptionBermudaFixedIbor implements InstrumentDerivative {
  ...
  public SwaptionBermudaFixedIbor(final SwapFixedCoupon<? extends Coupon>[] underlyingSwap, final boolean isLong, final double[] expiryTime, final double[] settlementTime) {
    ArgChecker.notNull(expiryTime, "Expiry time");
    ArgChecker.notNull(underlyingSwap, "Underlying swap");
    ArgChecker.notNull(settlementTime, "Settlement time");
    ArgChecker.isTrue(underlyingSwap.length == expiryTime.length, "Number of swaps not in line with number of expiry times");
    ArgChecker.isTrue(underlyingSwap.length == settlementTime.length, "Number of swaps not in line with number of settlement times");
