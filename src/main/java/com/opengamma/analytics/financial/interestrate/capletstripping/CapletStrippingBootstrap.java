...
public class CapletStrippingBootstrap {
  ...
  public CapletStrippingBootstrap(final List<CapFloor> caps, final MulticurveProviderInterface curves) {
    ArgChecker.noNulls(caps, "caps null");
    ...
    while (iter.hasNext()) {
      ...
      ArgChecker.isTrue(cap.getStrike() == strike, "caps must have same strike for this method");
      ArgChecker.isTrue(cap.getStartTime() == startTime, "caps must be co-starting");
      final double temp = cap.getEndTime();
      ArgChecker.isTrue(temp > endTime, "caps must be in order of increasing end time"); // TODO remove this by sorting caps
  ...
  public double[] capletVolsFromPrices(final double[] mktCapFlPrices) {
    ArgChecker.notEmpty(mktCapFlPrices, "null cap prices");
    final int n = _caplets.length;
    ArgChecker.isTrue(n == mktCapFlPrices.length, "length of prices does not match number of caps");
    ArgChecker.isTrue(mktCapFlPrices[0] > _intrinsicValues[0], "prices must be greater than or equal to their intrinsic values");
    final double[] diffs = new double[n];
    diffs[0] = mktCapFlPrices[0];
    for (int i = 1; i < n; i++) {
      diffs[i] = mktCapFlPrices[i] - mktCapFlPrices[i - 1];
      ArgChecker.isTrue(diffs[i] >= _intrinsicValues[i], "prices must be greater than or equal to their intrinsic values");
    }
  ...
  public double[] capletVolsFromCapVols(final double[] mktCapFlVols) {
    ArgChecker.notEmpty(mktCapFlVols, "null cap vols");
    final int n = _caplets.length;
    ArgChecker.isTrue(n == mktCapFlVols.length, "length of vols does not match number of caps");
