...
public final class BondTransactionDiscountingMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final BondFixedTransaction bond,
    ...
    final MultiCurrencyAmount pvNominal = bond.getBondTransaction().getNominal().accept(PVDC, multicurvesDecorated);
    final MultiCurrencyAmount pvCoupon = bond.getBondTransaction().getCoupon().accept(PVDC, multicurvesDecorated);
    ...
    final MultiCurrencyAmount pvSettlement = settlement.accept(PVDC, issuerMulticurves.getMulticurveProvider());
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValue(final BondIborTransaction bond,
      final IssuerProviderInterface issuerMulticurves) {
    ArgChecker.notNull(bond, "Bond");
    final Currency ccy = bond.getBondTransaction().getCurrency();
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValueFromCleanPrice(
    ...
    ArgChecker.notNull(bond, "Bond");
    ArgChecker.isTrue(bond instanceof BondFixedTransaction, "Present value from clean price only for fixed coupon bond");
    final Currency ccy = bond.getBondTransaction().getCurrency();
    final MulticurveProviderInterface multicurvesDecorated = new MulticurveProviderDiscountingDecoratedIssuer(issuerMulticurves, ccy, bond.getBondTransaction().getIssuerEntity());
    ...
    final MultiCurrencyAmount pvPriceStandard = MultiCurrencyAmount.of(ccy, (cleanPrice * bondFixed.getNotionalStandard() + bondFixed.getBondStandard().getAccruedInterest()) * dfSettle);
    final MultiCurrencyAmount pvNominalStandard = bond.getBondStandard().getNominal().accept(PVDC, multicurvesDecorated);
    final MultiCurrencyAmount pvCouponStandard = bond.getBondStandard().getCoupon().accept(PVDC, multicurvesDecorated);
    final MultiCurrencyAmount pvDiscountingStandard = pvNominalStandard.plus(pvCouponStandard);
    final MultiCurrencyAmount pvNominalTransaction = bond.getBondTransaction().getNominal().accept(PVDC, multicurvesDecorated);
    final MultiCurrencyAmount pvCouponTransaction = bond.getBondTransaction().getCoupon().accept(PVDC, multicurvesDecorated);
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValueFromYield(
      final BondTransaction<? extends BondSecurity<? extends Payment, ? extends Coupon>> bond, 
          final IssuerProviderInterface issuerMulticurves, final double yield) {
    ArgChecker.notNull(bond, "Bond");
    ArgChecker.isTrue(bond instanceof BondFixedTransaction,
    ...
    final MultiCurrencyAmount pvPriceStandard = MultiCurrencyAmount.of(ccy, dirtyPrice * dfSettle);
    final MultiCurrencyAmount pvNominalStandard = bond.getBondStandard().getNominal().accept(PVDC, multicurvesDecorated);
    final MultiCurrencyAmount pvCouponStandard = bond.getBondStandard().getCoupon().accept(PVDC, multicurvesDecorated);
    final MultiCurrencyAmount pvDiscountingStandard = pvNominalStandard.plus(pvCouponStandard);
    final MultiCurrencyAmount pvNominalTransaction = bond.getBondTransaction().getNominal().accept(PVDC, multicurvesDecorated);
  ...
  public double parSpread(final BondFixedTransaction bond, final IssuerProviderInterface issuerMulticurves) {
    ArgChecker.notNull(bond, "Bond");
    ArgChecker.notNull(issuerMulticurves, "Issuer and multi-curves provider");
    final Currency ccy = bond.getBondTransaction().getCurrency();
    ...
    final double pvNominalAtSettlement = 
        nominalAtSettlement.accept(PVDC, issuerMulticurves.getMulticurveProvider()).getAmount(ccy).getAmount();
    return presentValue(bond, issuerMulticurves).getAmount(ccy).getAmount() / pvNominalAtSettlement;
  }
  ...
  public double parSpread(final BondIborTransaction bond, final IssuerProviderInterface issuerMulticurves) {
    ...
    final double pvNominalAtSettlement = nominalAtSettlement.accept(PVDC, 
        issuerMulticurves.getMulticurveProvider()).getAmount(ccy).getAmount();
    return -presentValue(bond, issuerMulticurves).getAmount(ccy).getAmount() / pvNominalAtSettlement;
  }
  ...
  public MulticurveSensitivity parSpreadCurveSensitivity(final BondFixedTransaction bond, final IssuerProviderInterface issuerMulticurves) {
    ...
    final double pvBond = presentValue(bond, issuerMulticurves).getAmount(ccy).getAmount();
