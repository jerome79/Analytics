...
public class CouponIborAverageFixingDatesCompoundingFlatSpread extends Coupon {
  ...
  public CouponIborAverageFixingDatesCompoundingFlatSpread(final Currency currency, final double paymentTime,
    ...
    ArgChecker.isTrue(nPeriods == fixingPeriodAccrualFactor.length, "fixingPeriodAccrualFactor length different from fixingTime length");
    ...
    for (int i = 0; i < nPeriods; ++i) {
      ...
      ArgChecker.isTrue(nDates[i] == fixingPeriodAccrualFactor[i].length, "{}-th array: fixingPeriodAccrualFactor length different from fixingTime length", i);
    }
