...
public final class BondFutureOptionPremiumTransactionBlackSmileMethod {
   ...
   */
  public MultiCurrencyAmount presentValueFromPrice(final BondFutureOptionPremiumTransaction option, final MulticurveProviderInterface multicurves, final double price) {
    ArgChecker.notNull(option, "option");
    final Currency ccy = option.getCurrency();
    final MultiCurrencyAmount premiumPV = METHOD_PAY_FIXED.presentValue(option.getPremium(), multicurves);
    final double optionPV = price * option.getQuantity() * option.getUnderlyingOption().getUnderlyingFuture().getNotional();
    return premiumPV.plus(MultiCurrencyAmount.of(ccy, optionPV));
  }
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValue(final BondFutureOptionPremiumTransaction transaction, final BlackBondFuturesSmilePriceProviderInterface blackPrice) {
    ArgChecker.notNull(transaction, "transaction");
    ArgChecker.notNull(blackPrice, "Black parameters and futures price");
    final double priceSecurity = METHOD_SECURITY.price(transaction.getUnderlyingOption(), blackPrice);
    final MultiCurrencyAmount pvTransaction = presentValueFromPrice(transaction, blackPrice.getMulticurveProvider(), priceSecurity);
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValue(final BondFutureOptionPremiumTransaction transaction, final BlackBondFuturesSmileProviderInterface black) {
    ArgChecker.notNull(transaction, "transaction");
    ArgChecker.notNull(black, "Black parameters");
    final double priceSecurity = METHOD_SECURITY.price(transaction.getUnderlyingOption(), black);
    final MultiCurrencyAmount pvTransaction = presentValueFromPrice(transaction, black.getMulticurveProvider(), priceSecurity);
    return pvTransaction;
  }
  ...
  public MultipleCurrencyMulticurveSensitivity presentValueCurveSensitivity(final BondFutureOptionPremiumTransaction transaction, final BlackBondFuturesSmilePriceProviderInterface blackPrice) {
    ArgChecker.notNull(transaction, "transaction");
    ArgChecker.notNull(blackPrice, "Black parameters");
