...
public final class BondCapitalIndexedSecurityDiscountingMethod {
   ...
   */
  public MultiCurrencyAmount presentValue(final BondCapitalIndexedSecurity<?> bond,
    ...
    final MultiCurrencyAmount pvNominal = bond.getNominal().accept(PVIC, inflationDecorated);
    final MultiCurrencyAmount pvCoupon = bond.getCoupon().accept(PVIC, inflationDecorated);
  ...
  public double dirtyRealPriceFromCurves(final BondCapitalIndexedSecurity<?> bond, final InflationIssuerProviderInterface issuerMulticurves) {
    ...
    final double notional = bond.getCoupon().getNthPayment(0).getNotional();
    return pv.getAmount(bond.getCurrency()).getAmount() / settlement / notional;
  }
   ...
   */
  public MultiCurrencyAmount netAmount(final BondCapitalIndexedSecurity<Coupon> bond, final InflationIssuerProviderInterface market, final double cleanPriceReal) {
    final double notional = bond.getCoupon().getNthPayment(0).getNotional();
    final double netAmountRealByUnit = cleanPriceReal + bond.getAccruedInterest() / notional;
    final MultiCurrencyAmount netAmount = bond.getSettlement().accept(NAIC, market.getInflationProvider());
  ...
  public double yieldRealFromDirtyRealPrice(final BondCapitalIndexedSecurity<?> bond, final double dirtyPrice) {
    ...
    return ROOT_FINDER.getRoot(priceResidual, range[0], range[1]);
  }
  ...
  public double modifiedDurationFromYieldStandard(final BondCapitalIndexedSecurity<?> bond, final double yield) {
    ...
    return mdAtFirstCoupon * Math.pow(factorOnPeriod, -bond.getAccrualFactorToNextCoupon()) / pv;
  }
  ...
  public double yieldRealFromCurves(final BondCapitalIndexedSecurity<?> bond, final InflationIssuerProviderInterface provider) {
    ArgChecker.notNull(bond, "Bond");
    ArgChecker.notNull(provider, "inflation and multi-curves provider");
    double dirtyPrice;
    if (bond.getYieldConvention().equals(INDEX_LINKED_FLOAT)) {
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValueFromZSpread(final BondCapitalIndexedSecurity<?> bond, final InflationIssuerProviderInterface issuerMulticurves, final double zSpread) {
   ...
   * @return The z-spread.
   */
  public double zSpreadFromCurvesAndPV(final BondCapitalIndexedSecurity<?> bond, final InflationIssuerProviderInterface issuerMulticurves, final MultiCurrencyAmount pv) {
    ArgChecker.notNull(bond, "Bond");
    ArgChecker.notNull(issuerMulticurves, "Issuer and multi-curves provider");
