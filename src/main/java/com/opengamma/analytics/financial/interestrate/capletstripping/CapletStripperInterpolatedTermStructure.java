...
public class CapletStripperInterpolatedTermStructure implements CapletStripper {
  ...
  public CapletStripperInterpolatedTermStructure(MultiCapFloorPricer pricer, double[] knots) {
    ...
    ArgChecker.isTrue(pricer.getNumCaps() >= knots.length, "#knots ({}) is greater than number of caps ({}). Please reduce the number of knots", knots.length, pricer.getNumCaps());
    _pricer = pricer;
    _transform = TRANSFORM;
  ...
  private CapletStrippingCore getImp(double[] values) {
    ...
    ArgChecker.notEmpty(values, "values");
    int nCaps = _pricer.getNumCaps();
    ArgChecker.isTrue(nCaps == values.length, "Expected {} cap prices, but only given {}", nCaps, values.length);
  ...
  private double[] getKnots(double[] s, double[] e, int nCaps) {
    ...
    ArgChecker.isTrue(times.length >= nCaps, "Cannot auto generate knots for this set of caps. Please supply knots");
