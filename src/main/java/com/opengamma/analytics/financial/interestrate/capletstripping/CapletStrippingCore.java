...
public class CapletStrippingCore {
  ...
  public CapletStrippingCore(MultiCapFloorPricer pricer, DiscreteVolatilityFunctionProvider volFuncProv) {
    ArgChecker.notNull(pricer, "pricer");
    ArgChecker.notNull(volFuncProv, "volFuncProv");
  ...
  public CapletStrippingResult solveForCapPrices(double[] capPrices, DoubleMatrix1D start) {
    ArgChecker.notNull(capPrices, "capPrices");
  ...
  public CapletStrippingResult solveForCapVols(double[] capVols, DoubleMatrix1D start) {
    ArgChecker.notNull(capVols, "capVols");
    // try to root-find first
  ...
  public CapletStrippingResult leastSqrSolveForCapPrices(double[] capPrices, double[] errors, DoubleMatrix1D start) {
    ...
    ArgChecker.isTrue(start.getNumberOfElements() == _nModelParms, "length of start ({}) not equal to expected number of model parameters ({})", start.getNumberOfElements(), _nModelParms);
    ArgChecker.isTrue(capPrices.length >= _nModelParms, "Number of cap prices ({}) is less than number of model parameters ({}). "
  ...
  public CapletStrippingResult leastSqrSolveForCapVols(double[] capVols, double[] errors, DoubleMatrix1D start) {
    ArgChecker.notNull(start, "start");
    ArgChecker.isTrue(start.getNumberOfElements() == _nModelParms, "length of start ({}) not equal to expected number of model parameters ({})", start.getNumberOfElements(), _nModelParms);
    ArgChecker.isTrue(capVols.length >= _nModelParms, "Number of cap prices ({}) is less than number of model parameters ({}). "
  ...
  public CapletStrippingResult rootFindForCapPrices(double[] capPrices, DoubleMatrix1D start) {
    ArgChecker.notNull(start, "start");
    ArgChecker.isTrue(start.getNumberOfElements() == _nModelParms, "length of start ({}) not equal to expected number of model parameters ({})", start.getNumberOfElements(), _nModelParms);
    ArgChecker.isTrue(capPrices.length == _nModelParms, "Number of cap prices ({}) is not equal to number of model parameters ({}). " + "To root find they must equal", capPrices.length,
        _nModelParms);
  ...
  public CapletStrippingResult rootFindForCapVols(double[] capVols, DoubleMatrix1D start) {
    ArgChecker.notNull(start, "start");
    ArgChecker.isTrue(start.getNumberOfElements() == _nModelParms, "length of start ({}) not equal to expected number of model parameters ({})", start.getNumberOfElements(), _nModelParms);
    ArgChecker.isTrue(capVols.length == _nModelParms, "Number of cap prices ({}) is not equal to number of model parameters ({}). " + "To root find they must equal", capVols.length, _nModelParms);
    checkVols(capVols);
  ...
  public CapletStrippingResult solveForCapPrices(double[] capPrices, double[] errors, DoubleMatrix1D start, DoubleMatrix2D penaltyMatrix, Function1D<DoubleMatrix1D, Boolean> allowed) {
    ArgChecker.notNull(start, "start");
    ArgChecker.notNull(penaltyMatrix, "penaltyMatrix");
    ArgChecker.isTrue(start.getNumberOfElements() == _nModelParms, "length of start ({}) not equal to expected number of model parameters ({})", start.getNumberOfElements(), _nModelParms);
    checkErrors(errors);
    checkPrices(capPrices);
    ArgChecker.isTrue(penaltyMatrix.getNumberOfRows() == _nModelParms && penaltyMatrix.getNumberOfColumns() == _nModelParms,
  ...
  private void checkPrices(double[] capPrices) {
    ArgChecker.notEmpty(capPrices, "null cap prices");
    int nCaps = getNumCaps();
    ArgChecker.isTrue(nCaps == capPrices.length, "wrong number of capPrices, should have {}, but {} given", nCaps, capPrices.length);
    double[] base = _pricer.getIntrinsicCapValues();
    for (int i = 0; i < nCaps; i++) {
      ArgChecker.isTrue(capPrices[i] >= base[i], "Cap price {} lower that intrinisic value {}", capPrices[i], base[i]);
    }
  }

  private void checkVols(double[] capVols) {
    ArgChecker.notEmpty(capVols, "null cap vols");
    int nCaps = getNumCaps();
    ArgChecker.isTrue(nCaps == capVols.length, "wrong number of capVols, should have {}, but {} given", nCaps, capVols.length);
    for (int i = 0; i < nCaps; i++) {
      ArgChecker.isTrue(capVols[i] >= 0.0, "Cap vol {} less than zero", capVols[i]);
    }
  }

  private void checkErrors(double[] errors) {
    ArgChecker.notEmpty(errors, "null errors");
    int nCaps = getNumCaps();
    ArgChecker.isTrue(nCaps == errors.length, "wrong number of errors, should have {}, but {} given", nCaps, errors.length);
    for (int i = 0; i < nCaps; i++) {
      ArgChecker.isTrue(errors[i] > 0.0, "erros {} less than zero or equal to zero", errors[i]);
    }
  }
  ...
  private Function1D<DoubleMatrix1D, DoubleMatrix1D> getDiffFunc(final Function1D<DoubleMatrix1D, DoubleMatrix1D> func, final DoubleMatrix1D mktVals) {
    ArgChecker.notNull(mktVals, "mktVals");
    return new Function1D<DoubleMatrix1D, DoubleMatrix1D>() {
      @Override
      public DoubleMatrix1D evaluate(DoubleMatrix1D x) {
