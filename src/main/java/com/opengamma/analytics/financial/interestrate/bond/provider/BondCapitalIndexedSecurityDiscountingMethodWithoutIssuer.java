...
public class BondCapitalIndexedSecurityDiscountingMethodWithoutIssuer {
   ...
   */
  public MultiCurrencyAmount presentValue(final BondCapitalIndexedSecurity<?> bond, final InflationProviderInterface provider) {
    ArgChecker.notNull(bond, "Bond");
    final MultiCurrencyAmount pvNominal = bond.getNominal().accept(PVIC, provider);
    final MultiCurrencyAmount pvCoupon = bond.getCoupon().accept(PVIC, provider);
   ...
   */
  public MultiCurrencyAmount netAmount(final BondCapitalIndexedSecurity<Coupon> bond, final InflationProviderInterface market, final double cleanPriceReal) {
    final double notional = bond.getCoupon().getNthPayment(0).getNotional();
   ...
   * @return The present value.
   */
  public MultiCurrencyAmount presentValueFromZSpread(final BondCapitalIndexedSecurity<?> bond, final InflationProviderInterface issuerMulticurves, final double zSpread) {
   ...
   * @return The z-spread.
   */
  public double zSpreadFromCurvesAndPV(final BondCapitalIndexedSecurity<?> bond, final InflationProviderInterface issuerMulticurves, final MultiCurrencyAmount pv) {
    ArgChecker.notNull(bond, "Bond");
    ArgChecker.notNull(issuerMulticurves, "Issuer and multi-curves provider");
