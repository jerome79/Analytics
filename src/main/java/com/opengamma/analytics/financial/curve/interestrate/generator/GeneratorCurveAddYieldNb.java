...
public class GeneratorCurveAddYieldNb extends GeneratorYDCurve {
  ...
  public GeneratorCurveAddYieldNb(final GeneratorYDCurve[] generators, final int[] nbParameters, final boolean substract) {
    ArgChecker.notNull(generators, "Generators");
    _generators = generators;
    ArgChecker.isTrue(generators.length == nbParameters.length, "Number of parameters should be the same a number of generatros.");
  ...
  public YieldAndDiscountCurve generateCurve(final String name, final double[] x) {
    ArgChecker.isTrue(x.length == getNumberOfParameter(), "Incorrect number of parameters");
  ...
  public GeneratorYDCurve finalGenerator(final Object data) {
    ArgChecker.isTrue(data instanceof InstrumentDerivative[], "data should be an array of InstrumentDerivative");
    final InstrumentDerivative[] instruments = (InstrumentDerivative[]) data;
    ArgChecker.isTrue(instruments.length == _totalNbParameters, "The data should have the size prescribed by the _nbParameters");
