...
public class TenorUtils {
   ...
   */
  public static ZonedDateTime adjustDateByTenor(final ZonedDateTime date, final ComparableTenor tenor) {
    ArgChecker.notNull(date, "date");
    ArgChecker.notNull(tenor, "tenor");
    ArgChecker.isTrue(!tenor.isBusinessDayTenor(), "Business day tenors must have a calendar and initial offset to adjust a date");
   ...
   * @throws IllegalStateException If the tenor is not backed by a {@link Period}
   */
  public static LocalDateTime adjustDateByTenor(final LocalDateTime date, final ComparableTenor tenor) {
    ArgChecker.notNull(date, "date");
    ArgChecker.notNull(tenor, "tenor");
    ArgChecker.isTrue(!tenor.isBusinessDayTenor(), "Business day tenors must have a calendar and initial offset to adjust a date");
    return date.plus(tenor.getPeriod());
   ...
   * @throws IllegalStateException If the tenor is not backed by a {@link Period}
   */
  public static LocalDate adjustDateByTenor(final LocalDate date, final ComparableTenor tenor) {
    ArgChecker.notNull(date, "date");
    ArgChecker.notNull(tenor, "tenor");
    ArgChecker.isTrue(!tenor.isBusinessDayTenor(), "Business day tenors must have a calendar and initial offset to adjust a date");
    return date.plus(tenor.getPeriod());
  ...
  public static ZonedDateTime adjustDateByTenor(final ZonedDateTime date, final ComparableTenor tenor, final Calendar calendar, final int spotDays) {
    ...
    ArgChecker.isTrue(spotDays >= 0, "number of spot days must be greater than zero; have {}", spotDays);
   ...
   */
  public static ComparableTenor plus(final ComparableTenor tenor1, final ComparableTenor tenor2) {
    if ((!tenor1.isBusinessDayTenor()) && (!tenor2.isBusinessDayTenor())) { // Standard periods
      return ComparableTenor.of(tenor1.getPeriod().plus(tenor2.getPeriod()));
    }
    if (tenor1.equals(ComparableTenor.of(Period.ZERO))) { // First tenor is ZERO
      return tenor2;
    }
    if (tenor2.equals(ComparableTenor.of(Period.ZERO))) { // Second tenor is ZERO
      return tenor1;
    }
    if ((tenor1.equals(ComparableTenor.ON)) && (tenor1.equals(ComparableTenor.ON))) { // Both tenors are ON
      return ComparableTenor.TN;
    }
