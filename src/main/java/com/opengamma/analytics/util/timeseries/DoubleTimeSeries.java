...
//  double getEarliestValueDouble();
  ...
  /**
   * Convert to the new style LocalDateDoubleTimeSeries.
   *
   * @return this timeseries converted into a LocalDateDoubleTimeSeries
   */
  public default LocalDateDoubleTimeSeries toLocalDateDoubleTimeSeries() {

    T[] times = timesArray();
    double[] values = valuesArrayFast();

    return IntStream.range(0, size())
        .mapToObj(i -> LocalDateDoublePoint.of(localDateConverter().apply(times[i]), values[i]))
        .collect(LocalDateDoubleTimeSeries.collector());
  }

  public abstract Function<T, LocalDate> localDateConverter();
