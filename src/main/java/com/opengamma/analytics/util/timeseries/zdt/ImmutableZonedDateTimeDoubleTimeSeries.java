...
public final class ImmutableZonedDateTimeDoubleTimeSeries
  ...
  //-------------------------------------------------------------------------
  @Override
  public ZonedDateTimeDoubleTimeSeries operate(DoubleUnaryOperator operator) {
    double[] valuesArray = valuesArrayFast();
    for (int i = 0; i < valuesArray.length; i++) {
      valuesArray[i] = operator.applyAsDouble(valuesArray[i]);
    }
    return new ImmutableZonedDateTimeDoubleTimeSeries(_times, valuesArray, _zone);  // immutable, so can share times
  }

  @Override
  public ZonedDateTimeDoubleTimeSeries operate(double other, DoubleBinaryOperator operator) {
    double[] valuesArray = valuesArrayFast();
    for (int i = 0; i < valuesArray.length; i++) {
      valuesArray[i] = operator.applyAsDouble(valuesArray[i], other);
    }
    return new ImmutableZonedDateTimeDoubleTimeSeries(_times, valuesArray, _zone);  // immutable, so can share times
  }
