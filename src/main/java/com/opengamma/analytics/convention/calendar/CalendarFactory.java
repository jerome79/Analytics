    ...
    for (final String calendarName : calendars.keySet()) {
      try {
        ...
        if ((uri != null) && (nameURIConstructor >= 0)) {
          ...
          throw new IllegalStateException("No suitable constructor for '" + calendarName + "'");
        }
        addInstance(instance);
      } catch (final InstantiationException | IllegalAccessException | ClassNotFoundException | InvocationTargetException | IllegalArgumentException ex) {
        throw new IllegalStateException("Error initialising Calendars", ex);
      }
    }
  }

  private void loadCountryDefinitions() {
    final ResourceBundle countries = ResourceBundle.getBundle("com.opengamma.analytics.convention.calendar.Country");
    for (final String countryCode : countries.keySet()) {
      ...
      try {
        ...
        throw new IllegalStateException("Cannot find calendar '" + calendarName + "' for country '" + countryCode + "'");
      }
    }
  }
