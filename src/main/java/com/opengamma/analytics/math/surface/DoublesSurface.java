...
public abstract class DoublesSurface extends Surface<Double, Double, Double> {
  ...
  public DoublesSurface(final double[] xData, final double[] yData, final double[] zData) {
    ...
    ArgChecker.isTrue(xData.length == yData.length, "X data size different from Y data size");
    ArgChecker.isTrue(xData.length == zData.length, "X data size different from Z data size");
    _n = xData.length;
