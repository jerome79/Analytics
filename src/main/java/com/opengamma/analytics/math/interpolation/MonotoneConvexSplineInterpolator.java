...
public class MonotoneConvexSplineInterpolator extends PiecewisePolynomialInterpolator {
  ...
  public PiecewisePolynomialResult interpolate(final double[] xValues, final double[] yValues) {
    ...
    for (int i = 0; i < nDataPts; ++i) {
      if (xValues[i] == 0.) {
        ArgChecker.isTrue(yValues[i] == 0., "r_i * t_i = 0 if t_i =0");
      }
    }
    ...
    for (int i = 0; i < coefMatrixIntegrate.getNumberOfRows(); ++i) {
      for (int j = 0; j < coefMatrixIntegrate.getNumberOfColumns(); ++j) {
        ArgChecker.isFalse(Double.isNaN(coefMatrixIntegrate.getData()[i][j]), "Too large input");
        ArgChecker.isFalse(Double.isInfinite(coefMatrixIntegrate.getData()[i][j]), "Too large input");
      }
    }
  ...
  public double interpolate(final double[] xValues, final double[] yValues, final double x) {
    ...
    ArgChecker.isFalse(Double.isInfinite(res), "Too large/small data values or xKey");
    ArgChecker.isFalse(Double.isNaN(res), "Too large/small data values or xKey");
  ...
  public DoubleMatrix1D interpolate(final double[] xValues, final double[] yValues, final double[] x) {
    ...
    for (int j = 0; j < keyLength; ++j) {
      ...
      ArgChecker.isFalse(Double.isInfinite(res[j]), "Too large/small data values or xKey");
      ArgChecker.isFalse(Double.isNaN(res[j]), "Too large/small data values or xKey");
    }
  ...
  public DoubleMatrix2D interpolate(final double[] xValues, final double[] yValues, final double[][] xMatrix) {
    ...
    for (int j = 0; j < keyDim; ++j) {
      for (int k = 0; k < keyLength; ++k) {
        ...
        ArgChecker.isFalse(Double.isInfinite(res[j][k]), "Too large input");
        ArgChecker.isFalse(Double.isNaN(res[j][k]), "Too large input");
      }
    }
