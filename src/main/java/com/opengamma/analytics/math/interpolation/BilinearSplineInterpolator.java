...
public class BilinearSplineInterpolator extends PiecewisePolynomialInterpolator2D {
  ...
  public PiecewisePolynomialResult2D interpolate(final double[] x0Values, final double[] x1Values, final double[][] yValues) {
    ...
    ArgChecker.isTrue(nData0 == yValues.length, "x0Values length = yValues number of rows");
    ArgChecker.isTrue(nData1 == yValues[0].length, "x1Values length = yValues number of columns");
    ArgChecker.isTrue(nData0 > 1, "Data points along x0 direction should be more than 1");
    ArgChecker.isTrue(nData1 > 1, "Data points along x1 direction should be more than 1");

    for (int i = 0; i < nData0; ++i) {
      ArgChecker.isFalse(Double.isNaN(x0Values[i]), "x0Values containing NaN");
      ArgChecker.isFalse(Double.isInfinite(x0Values[i]), "x0Values containing Infinity");
    }
    for (int i = 0; i < nData1; ++i) {
      ArgChecker.isFalse(Double.isNaN(x1Values[i]), "x1Values containing NaN");
      ArgChecker.isFalse(Double.isInfinite(x1Values[i]), "x1Values containing Infinity");
    }
    for (int i = 0; i < nData0; ++i) {
      for (int j = 0; j < nData1; ++j) {
        ArgChecker.isFalse(Double.isNaN(yValues[i][j]), "yValues containing NaN");
        ArgChecker.isFalse(Double.isInfinite(yValues[i][j]), "yValues containing Infinity");
      }
    }

    for (int i = 0; i < nData0; ++i) {
      for (int j = i + 1; j < nData0; ++j) {
        ArgChecker.isFalse(x0Values[i] == x0Values[j], "x0Values should be distinct");
      }
    }
    for (int i = 0; i < nData1; ++i) {
      for (int j = i + 1; j < nData1; ++j) {
        ArgChecker.isFalse(x1Values[i] == x1Values[j], "x1Values should be distinct");
      }
    }
      ...
      for (int j = 0; j < nData1 - 1; ++j) {
        ...
        for (int k = 0; k < order; ++k) {
          for (int l = 0; l < order; ++l) {
            ArgChecker.isFalse(Double.isNaN(coefMatTmp[k][l]), "Too large/small input");
            ArgChecker.isFalse(Double.isInfinite(coefMatTmp[k][l]), "Too large/small input");
