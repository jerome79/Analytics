...
public abstract class WeightingFunction {
  ...
  /**
   * Get the function weight for point x, based on the lower bound index.
   * 
  ...
  /**
   * For an arbitrary point x, let the two data points immediately below and above x be, x_a and x_b,
   * then define y = (x_b - x)/(x_b - x_a).
   * 
   * @param xs  all the independent data points
  ...
  private double getY(double[] xs, int lowerBoundIndex, double x) {
    ArgChecker.notNull(xs, "strikes");
    ArgChecker.notNegative(lowerBoundIndex, "index");
    ArgChecker.isTrue(lowerBoundIndex <= xs.length - 2, "index cannot be larger than {}, have {}", xs.length - 2, lowerBoundIndex);
