...
public class ShapePreservingCubicSplineInterpolator extends PiecewisePolynomialInterpolator {
  ...
  public PiecewisePolynomialResult interpolate(final double[] xValues, final double[] yValues) {
    ...
    ArgChecker.isTrue(xValues.length > 2, "Data points should be more than 1");
    ...
    for (int i = 0; i < coefMatrix.length; ++i) {
      ...
      for (int j = 0; j < 4; ++j) {
        ...
        ArgChecker.isFalse(Double.isNaN(coefMatrix[i][j]), "Too large input");
        ArgChecker.isFalse(Double.isInfinite(coefMatrix[i][j]), "Too large input");
      }
      ...
      ArgChecker.isTrue(Math.abs(ref - yVal) < ERROR * bound, "Input is too large/small or data points are too close");
    }
