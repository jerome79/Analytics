...
public class RecentDataSetsMulticurveOisMeetingDatesGbp {
  ...
  private static final BusinessDayAdjustment ADJUSTED_DATE_ON =
      BusinessDayAdjustment.of(GENERATOR_OIS_GBP.getBusinessDayConvention(), LON);
  private static final DaysAdjustment OFFSET_FIXING = DaysAdjustment.ofBusinessDays(
      0, LON, BusinessDayAdjustment.of(BusinessDayConventions.FOLLOWING, LON));
  ...
  private static InstrumentDefinition<?>[] generateDatesOis(ZonedDateTime[] dates, double[] fixedRate) {
    ArgChecker.isTrue(dates.length == fixedRate.length + 1, "dates and rate lengths not compatible");
