...
public class RecentDataSetsMulticurveStandardGbp {
  ...
  public static Pair<MulticurveProviderDiscount, CurveBuildingBlockBundle> getCurvesGbpOisL6Spread(ZonedDateTime calibrationDate,
    ...
    ArgChecker.isTrue(dscMarketQuotes.length == DSC_GBP_GENERATORS.length, "Discounting market quotes of the wrong length");
    ArgChecker.isTrue(fwd6MarketQuotes.length == FWD6_GBP_GENERATORS.length, "Forward 6M market quotes of the wrong length");
  ...
  public static InstrumentDefinition<?>[] getDefinitionForFirstInstruments(ZonedDateTime calibrationDate,
    ...
    for (int i = 0; i < definitionsDsc.length; ++i) {
      if (definitionsDsc[i] instanceof CashDefinition) {
        ...
        throw new IllegalStateException("Instrument definition type not supported: "
