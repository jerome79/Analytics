...
public class VolatilitySwapFiniteDifferenceGreeksTest {
  ...
  public void newlyIssuedSwapTest() {
    ...
    final Currency base = EUR;
    final Currency counter = USD;
    final Map<Currency, YieldAndDiscountCurve> discountingCurves = new LinkedHashMap<>();
    discountingCurves.put(EUR, new YieldCurve("domestic", ConstantDoublesCurve.from(dr)));
    discountingCurves.put(USD, new YieldCurve("foreign", ConstantDoublesCurve.from(fr)));
    final FxMatrix fxMatrix = FxMatrix.builder().addRate(base, counter, spot).build();
    ...
    final FxMatrix bumpedFxMatrix = FxMatrix.builder().addRate(base, counter, bumpedSpot).build();
    final MulticurveProviderDiscount spotBumpedCurves = new MulticurveProviderDiscount(discountingCurves, new LinkedHashMap<>(),
        new LinkedHashMap<>(), bumpedFxMatrix);
    final CarrLeeFXData spotBumpedData = new CarrLeeFXData(Pair.of(base, counter), smile, spotBumpedCurves);
    ...
    final CarrLeeFXData volBumpedData = new CarrLeeFXData(Pair.of(base, counter), volBumpedSmile, curves);
  ...
  public void SeasonedSwapTest() {
    ...
    final MulticurveProviderDiscount spotBumpedCurves = new MulticurveProviderDiscount(discountingCurves, new LinkedHashMap<>(),
        new LinkedHashMap<>(), bumpedFxMatrix);
    final CarrLeeFXData spotBumpedData = new CarrLeeFXData(Pair.of(base, counter), smile, spotBumpedCurves, rv);
    final double spotBumpedFV = swap.accept(baseCal, spotBumpedData).getFairValue();
    final double spotBumpedFVRange = swap.accept(baseCalRange, spotBumpedData).getFairValue();
    ...
    final SmileDeltaTermStructureParametersStrikeInterpolation volBumpedSmile = new SmileDeltaTermStructureParametersStrikeInterpolation(timeSet, delta, bumpedVolatility, interp);
    final CarrLeeFXData volBumpedData = new CarrLeeFXData(Pair.of(base, counter), volBumpedSmile, curves, rv);
    final double volBumpedFV = swap.accept(baseCal, volBumpedData).getFairValue();
    final double volBumpedFVRange = swap.accept(baseCalRange, volBumpedData).getFairValue();
