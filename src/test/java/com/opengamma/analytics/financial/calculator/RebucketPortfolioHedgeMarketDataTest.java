...
public class RebucketPortfolioHedgeMarketDataTest {
  ...
  private static final FxMatrix FX_MATRIX = FxMatrix.builder()
      .addRate(Currency.GBP, Currency.USD, GBPUSD)
      .addRate(Currency.ZAR, Currency.GBP, 1. / GBPZAR)
      .addRate(Currency.NZD, Currency.GBP, 1. / GBPNZD)
      .addRate(Currency.AUD, Currency.GBP, 1. / GBPAUD)
      .addRate(Currency.HUF, Currency.GBP, 1. / GBPHUF)
      .addRate(Currency.HKD, Currency.GBP, 1. / GBPHKD)
      .addRate(Currency.SGD, Currency.GBP, 1. / GBPSGD)
      .addRate(Currency.NOK, Currency.GBP, 1. / GBPNOK)
      .addRate(Currency.CAD, Currency.GBP, 1. / GBPCAD)
      .addRate(Currency.SEK, Currency.GBP, 1. / GBPSEK)
      .addRate(Currency.JPY, Currency.GBP, 1. / GBPJPY)
      .addRate(Currency.CZK, Currency.GBP, 1. / GBPCZK)
      .addRate(Currency.DKK, Currency.GBP, 1. / GBPDKK)
      .addRate(Currency.PLN, Currency.GBP, 1. / GBPPLN)
      .addRate(Currency.CHF, Currency.GBP, 1. / GBPCHF)
      .addRate(Currency.EUR, Currency.GBP, 1. / GBPEUR)
      .build();

  private static final MulticurveProviderDiscount ALL_CURVES = new MulticurveProviderDiscount(FX_MATRIX);
  ...
  public void FRATest() {
    ...
    order.add(Pair.of(GBP_FWD_IBOR_NAME, TIME_10.length));
  ...
  public void SwapTest() {
    ...
    LinkedHashSet<Pair<String, Integer>> order = new LinkedHashSet<>();
    order.add(Pair.of(EUR_DSC_NAME, TIME_9.length));
    order.add(Pair.of(EUR_FWD_IBOR_NAME, TIME_9.length));
    ParameterSensitivityWeightMatrixCalculator matrixCalc = new ParameterSensitivityWeightMatrixCalculator();
    double[] objectiveNodes = new double[] {2, 5, 10, 30 };
    DoubleMatrix2D matrix = matrixCalc.projectCurveNodes(ALL_CURVES, order, objectiveNodes);
  ...
  public void OISTest() {
    ...
    order.add(Pair.of(EUR_DSC_NAME, TIME_9.length));
    order.add(Pair.of(EUR_FWD_IBOR_NAME, TIME_9.length));
    order.add(Pair.of(EUR_FWD_ON_NAME, TIME_8.length));
    ParameterSensitivityWeightMatrixCalculator matrixCalc = new ParameterSensitivityWeightMatrixCalculator();
    double[] objectiveNodes = new double[] {2, 5, 10, 30 };
    DoubleMatrix2D matrix = matrixCalc.projectCurveNodes(ALL_CURVES, order, objectiveNodes);
  ...
  public void MixedTest() {
    ...
    order.add(Pair.of(GBP_DSC_NAME, TIME_10.length));
    order.add(Pair.of(GBP_FWD_IBOR_NAME, TIME_10.length));
    order.add(Pair.of(GBP_FWD_ON_NAME, TIME_8.length));
    order.add(Pair.of(EUR_DSC_NAME, TIME_9.length));
    order.add(Pair.of(EUR_FWD_IBOR_NAME, TIME_9.length));
    order.add(Pair.of(EUR_FWD_ON_NAME, TIME_8.length));
    ParameterSensitivityWeightMatrixCalculator matrixCalc = new ParameterSensitivityWeightMatrixCalculator();
