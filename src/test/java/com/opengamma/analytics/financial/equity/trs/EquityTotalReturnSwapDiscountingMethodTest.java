...
public class EquityTotalReturnSwapDiscountingMethodTest {
  ...
  private static final AnnuityDefinition<CouponIborSpreadDefinition> FUNDING_LEG_IBOR_PAY_DEFINITION = AnnuityDefinitionBuilder.couponIborSpread(EFFECTIVE_DATE_2,
      TERMINATION_DATE_2, USDLIBOR3M.getTenor(), NOTIONAL_TRS_USD, SPREAD, USDLIBOR3M, true, USDLIBOR3M.getDayCount(), USDLIBOR3M.getBusinessDayConvention(), true, NYC, StubConvention.SHORT_INITIAL,
      0);
  ...
  public void presentValueONSameCurrencyBeforeEffective() {
    MultiCurrencyAmount pvComputedPay = METHOD_TRS_EQT.presentValue(TRS_PAY_ON_REC_1, EQUITY_MULTICURVE);
    ...
    assertTrue("EquityTotalReturnSwapDiscountingMethod: present value", pvComputedPay.getAmount(GBP).getAmount() != 0.0);
    MultiCurrencyAmount pvEquity = MultiCurrencyAmount.of(GBP, -NB_SHARES * EQUITY_PRICE);
    MultiCurrencyAmount pvPreviousFixing = MultiCurrencyAmount.of(GBP, NOTIONAL_TRS_GBP * MULTICURVE.getDiscountFactor(GBP, TRS_PAY_ON_REC_1.getFundingLeg().getNthPayment(0).getPaymentTime()));
    MultiCurrencyAmount pvFunding = FUNDING_LEG_ON_REC_1.accept(PVDC, MULTICURVE);
    MultiCurrencyAmount pvExpected = pvEquity.plus(pvPreviousFixing).plus(pvFunding);
    ...
    MultiCurrencyAmount pvComputedRec = METHOD_TRS_EQT.presentValue(TRS_REC_ON_PAY_1, EQUITY_MULTICURVE);
    ...
    MultiCurrencyAmount pvAssetLeg = METHOD_TRS_EQT.presentValueAssetLeg(TRS_PAY_ON_REC_1, EQUITY_MULTICURVE);
    MultiCurrencyAmount pvFundingLeg = METHOD_TRS_EQT.presentValueFundingLeg(TRS_PAY_ON_REC_1, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: present value", pvExpected.getAmount(GBP).getAmount(), pvFundingLeg.plus(pvAssetLeg).getAmount(GBP).getAmount(), TOLERANCE_PV);
    // Currency exposure
    MultiCurrencyAmount ceComputedPay = METHOD_TRS_EQT.currencyExposure(TRS_PAY_ON_REC_1, EQUITY_MULTICURVE);
    MultiCurrencyAmount ceComputedRec = METHOD_TRS_EQT.currencyExposure(TRS_REC_ON_PAY_1, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: currency exposure", pvComputedRec.getAmount(GBP).getAmount(), ceComputedRec.getAmount(GBP).getAmount(), TOLERANCE_PV);
    assertEquals("BondTRSDiscountingMethod: currency exposure", pvComputedPay.getAmount(GBP).getAmount(), ceComputedPay.getAmount(GBP).getAmount(), TOLERANCE_PV);
    // Method vs Calculator
    MultiCurrencyAmount pvCalculatorPay = TRS_PAY_ON_REC_1.accept(PVEDC, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: present value", pvCalculatorPay.getAmount(GBP).getAmount(), pvComputedPay.getAmount(GBP).getAmount(), TOLERANCE_PV);
    // Asset exposure
    MultiCurrencyAmount ae = METHOD_TRS_EQT.assetExposure(TRS_PAY_ON_REC_1, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: asset exposure", pvEquity.getAmount(GBP).getAmount(), ae.getAmount(GBP).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueONSameCurrencyAfterEffective() {
    MultiCurrencyAmount pvComputed = METHOD_TRS_EQT.presentValue(TRS_PAY_ON_REC_2, EQUITY_MULTICURVE);
    ...
    MultiCurrencyAmount pvEquity = MultiCurrencyAmount.of(GBP, -NB_SHARES * EQUITY_PRICE);
    MultiCurrencyAmount pvPreviousFixing = MultiCurrencyAmount.of(GBP, NOTIONAL_TRS_GBP * MULTICURVE.getDiscountFactor(GBP, TRS_PAY_ON_REC_2.getFundingLeg().getNthPayment(0).getPaymentTime()));
    MultiCurrencyAmount pvFunding = FUNDING_LEG_ON_REC_2.accept(PVDC, MULTICURVE);
    MultiCurrencyAmount pvExpected = pvEquity.plus(pvPreviousFixing).plus(pvFunding);
    assertEquals("BondTRSDiscountingMethod: present value", pvExpected.getAmount(GBP).getAmount(), pvComputed.getAmount(GBP).getAmount(), TOLERANCE_PV); // Equity and funding in same currency
    // Currency exposure
    MultiCurrencyAmount ceComputedRec = METHOD_TRS_EQT.currencyExposure(TRS_PAY_ON_REC_2, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: currency exposure", pvComputed.getAmount(GBP).getAmount(), ceComputedRec.getAmount(GBP).getAmount(), TOLERANCE_PV);
    // Asset exposure
    MultiCurrencyAmount ae = METHOD_TRS_EQT.assetExposure(TRS_PAY_ON_REC_2, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: asset exposure", pvEquity.getAmount(GBP).getAmount(), ae.getAmount(GBP).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueIborDiffCurrencyBeforeEffective() {
    MultiCurrencyAmount pvComputedRec = METHOD_TRS_EQT.presentValue(TRS_REC_IBOR_PAY_1, EQUITY_MULTICURVE);
    ...
    MultiCurrencyAmount pvEquityGbp = MultiCurrencyAmount.of(GBP, NB_SHARES * EQUITY_PRICE);
    CurrencyAmount pvEquityUsd = MULTICURVE.getFxRates().convert(pvEquityGbp, USD);
    MultiCurrencyAmount pvPreviousFixing = MultiCurrencyAmount.of(USD,
        -NOTIONAL_TRS_USD * MULTICURVE.getDiscountFactor(USD, TRS_REC_IBOR_PAY_1.getFundingLeg().getNthPayment(0).getPaymentTime()));
    MultiCurrencyAmount pvFunding = FUNDING_LEG_IBOR_PAY_1.getNthPayment(0).accept(PVDC, MULTICURVE);
    MultiCurrencyAmount pvExpected = pvPreviousFixing.plus(pvFunding).plus(pvEquityUsd);
    assertEquals("BondTRSDiscountingMethod: present value", pvExpected.getAmount(USD).getAmount(), pvComputedRec.getAmount(USD).getAmount(), TOLERANCE_PV);
    MultiCurrencyAmount pvAssetLeg = METHOD_TRS_EQT.presentValueAssetLeg(TRS_REC_IBOR_PAY_1, EQUITY_MULTICURVE);
    MultiCurrencyAmount pvFundingLeg = METHOD_TRS_EQT.presentValueFundingLeg(TRS_REC_IBOR_PAY_1, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: present value", pvExpected.getAmount(USD).getAmount(), pvFundingLeg.plus(pvAssetLeg).getAmount(USD).getAmount(), TOLERANCE_PV);
    // Currency exposure
    MultiCurrencyAmount ceComputedRec = METHOD_TRS_EQT.currencyExposure(TRS_REC_IBOR_PAY_1, EQUITY_MULTICURVE);
    ...
    assertEquals("BondTRSDiscountingMethod: currency exposure", pvEquityGbp.getAmount(GBP).getAmount(), ceComputedRec.getAmount(GBP).getAmount(), TOLERANCE_PV);
    // Method vs Calculator
    MultiCurrencyAmount pvCalculatorRec = TRS_REC_IBOR_PAY_1.accept(PVEDC, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: present value", pvCalculatorRec.getAmount(USD).getAmount(), pvComputedRec.getAmount(USD).getAmount(), TOLERANCE_PV);
    // Asset exposure
    MultiCurrencyAmount ae = METHOD_TRS_EQT.assetExposure(TRS_REC_IBOR_PAY_1, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: asset exposure", pvEquityGbp.getAmount(GBP).getAmount(), ae.getAmount(GBP).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueIborDiffCurrencyAfterEffective() {
    MultiCurrencyAmount pvComputedRec = METHOD_TRS_EQT.presentValue(TRS_REC_IBOR_PAY_2, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: present value", 1, pvComputedRec.size()); // Converted in funding: USD
    assertTrue("BondTRSDiscountingMethod: present value", pvComputedRec.getAmount(USD).getAmount() != 0.0);
    MultiCurrencyAmount pvEquityGbp = MultiCurrencyAmount.of(GBP, NB_SHARES * EQUITY_PRICE);
    CurrencyAmount pvEquityUsd = MULTICURVE.getFxRates().convert(pvEquityGbp, USD);
    ...
    MultiCurrencyAmount pvFunding0 = FUNDING_LEG_IBOR_PAY_2.getNthPayment(0).accept(PVDC, MULTICURVE);
    MultiCurrencyAmount pvExpected = pvPreviousFixing.plus(pvFunding0).plus(pvEquityUsd);
    assertEquals("BondTRSDiscountingMethod: present value", pvExpected.getAmount(USD).getAmount(), pvComputedRec.getAmount(USD).getAmount(), TOLERANCE_PV);
    MultiCurrencyAmount pvAssetLeg = METHOD_TRS_EQT.presentValueAssetLeg(TRS_REC_IBOR_PAY_2, EQUITY_MULTICURVE);
    MultiCurrencyAmount pvFundingLeg = METHOD_TRS_EQT.presentValueFundingLeg(TRS_REC_IBOR_PAY_2, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: present value", pvExpected.getAmount(USD).getAmount(), pvFundingLeg.plus(pvAssetLeg).getAmount(USD).getAmount(), TOLERANCE_PV);
    // Currency exposure
    MultiCurrencyAmount ceComputedRec = METHOD_TRS_EQT.currencyExposure(TRS_REC_IBOR_PAY_2, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: currency exposure", 2, ceComputedRec.size()); // GBP equity and USD funding (and notional currency)
    assertEquals("BondTRSDiscountingMethod: currency exposure", MULTICURVE.getFxRates().convert(ceComputedRec, GBP).getAmount(),
        MULTICURVE.getFxRates().convert(pvComputedRec, GBP).getAmount(), TOLERANCE_PV); // CE and PV total should be the same; only conversion is different
    assertEquals("BondTRSDiscountingMethod: currency exposure", pvEquityGbp.getAmount(GBP).getAmount(), ceComputedRec.getAmount(GBP).getAmount(), TOLERANCE_PV);
    // Asset exposure
    MultiCurrencyAmount ae = METHOD_TRS_EQT.assetExposure(TRS_REC_IBOR_PAY_2, EQUITY_MULTICURVE);
    assertEquals("BondTRSDiscountingMethod: asset exposure", pvEquityGbp.getAmount(GBP).getAmount(), ae.getAmount(GBP).getAmount(), TOLERANCE_PV);
  }
