...
public class ZSpreadCalculatorTest {
  ...
  private static final Currency CUR = EUR;
  ...
  static {
    ...
    final Map<Currency, YieldAndDiscountCurve> constantDiscounting = Collections.<Currency, YieldAndDiscountCurve>singletonMap(EUR, EUR_DISCOUNTING);
    ...
    final FxMatrix fxMatrix = FxMatrix.builder().addRate(EUR, Currency.USD, 1.2).build();
    ...
    discounting.put(EUR, YieldCurve.from(InterpolatedDoublesCurve.from(T, R1, INTERPOLATOR)));
  ...
  public void testZSpread() {
    final double price = PAYMENTS.accept(PV_CALCULATOR, CONSTANT_CURVES).getAmount(EUR).getAmount();
    ...
    multicurves.replaceCurve(EUR, YieldCurve.from(InterpolatedDoublesCurve.from(T, rSpread, INTERPOLATOR)));
    assertEquals(price, PAYMENTS.accept(PV_CALCULATOR, multicurves).getAmount(EUR).getAmount(), 1e-12);
  ...
  public void testPriceSensitivityToZSpread() {
    ...
    final MulticurveProviderDiscount multicurves = new MulticurveProviderDiscount(MULTI_CURVES.copy());
    multicurves.replaceCurve(EUR, YieldCurve.from(InterpolatedDoublesCurve.from(T, rSpread, INTERPOLATOR)));
    final double newPrice = PAYMENTS.accept(PV_CALCULATOR, multicurves).getAmount(EUR).getAmount();
  ...
  public void testSensitivities() {
    ...
    while (iter1.hasNext()) {
      ...
      for (int i = 0; i < 5; i++) {
        assertEquals(pairs1.get(i).getFirst(), pairs2.get(i).getFirst(), 1e-15);
        assertEquals(-pairs2.get(i).getSecond() / pairs1.get(i).getSecond(), dPdZ, 1e-15);
      }
    }
    zSpread = 0.0;
    ...
    while (iter1.hasNext()) {
      ...
      for (int i = 0; i < 5; i++) {
        assertEquals(pairs1.get(i).getFirst(), pairs2.get(i).getFirst(), 1e-15);
        assertEquals(pairs2.get(i).getSecond(), pairs1.get(i).getSecond(), 1e-15);
      }
    }
  }
