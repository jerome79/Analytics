...
public class MulticurveBuildingDiscountingDiscountUSD2DemoTest {
    ...
    for (int loopblock = 0; loopblock < nbBlocks; loopblock++) {
      ...
      for (int loopcurve = 0; loopcurve < instruments.length; loopcurve++) {
        ...
        for (int loopins = 0; loopins < instruments[loopcurve].length; loopins++) {
          pv[loopcurve][loopins] = curves.getFxRates().convert(instruments[loopcurve][loopins].accept(PVDC, curves), Currency.USD).getAmount();
  ...
  public void exportForwardCurve() throws IOException {
    CurveCalibrationTestsUtils.exportIborForwardIborCurve(
        CALIBRATION_DATE,
        CURVES_PAR_SPREAD_MQ_WITHOUT_TODAY_BLOCK.get(0).getFirst(),
        USDLIBOR3M,
        NYC,
        Files.createTempFile("demo-test-fwd-usd-libor3m-ncs","csv").toFile(),
        0,
        350,
        7);
  }
