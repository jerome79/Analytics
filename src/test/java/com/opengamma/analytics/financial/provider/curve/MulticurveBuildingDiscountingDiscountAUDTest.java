...
public class MulticurveBuildingDiscountingDiscountAUDTest {
  ...
  private static final HolidayCalendar SYD = HolidayCalendars.SAT_SUN;
  ...
  public void comparison1Unit2Units() {
    ...
    assertArrayEquals("Curve construction: 1 unit / 2 units ", DoubleArrayMath.toPrimitive(((YieldCurve) curveDsc[0]).getCurve().getXData()),
        DoubleArrayMath.toPrimitive(((YieldCurve) curveDsc[1]).getCurve().getXData()), TOLERANCE_CAL);
    assertArrayEquals("Curve construction: 1 unit / 2 units ", DoubleArrayMath.toPrimitive(((YieldCurve) curveDsc[0]).getCurve().getYData()),
        DoubleArrayMath.toPrimitive(((YieldCurve) curveDsc[1]).getCurve().getYData()), TOLERANCE_CAL);
    assertArrayEquals("Curve construction: 1 unit / 2 units ", DoubleArrayMath.toPrimitive(((YieldCurve) curveFwd[0]).getCurve().getXData()),
        DoubleArrayMath.toPrimitive(((YieldCurve) curveFwd[1]).getCurve().getXData()), TOLERANCE_CAL);
    assertArrayEquals("Curve construction: 1 unit / 2 units ", DoubleArrayMath.toPrimitive(((YieldCurve) curveFwd[0]).getCurve().getYData()),
        DoubleArrayMath.toPrimitive(((YieldCurve) curveFwd[1]).getCurve().getYData()), TOLERANCE_CAL);
