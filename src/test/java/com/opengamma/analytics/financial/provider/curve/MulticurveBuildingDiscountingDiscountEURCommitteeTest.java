...
public class MulticurveBuildingDiscountingDiscountEURCommitteeTest {
  ...
  private static final GeneratorDepositON GENERATOR_DEPOSIT_ON_EUR = new GeneratorDepositON("EUR Deposit ON", Currency.EUR, TARGET, EONIA.getDayCount());
  ...
  private static final IborIndex EUROLIBOR6M = new IborIndex(Currency.EUR, Period.ofMonths(6), 2, EURIBOR6M.getDayCount(), EURIBOR6M.getBusinessDayConvention(), true, "EUROLIBOR6M");
  ...
  public void comparison1Unit2Units() {
    ...
    for (int loopblock = 0; loopblock < 2; loopblock++) {
      ...
      curveDsc[loopblock] = units[loopblock].getCurve(Currency.EUR);
  ...
  public void forwardAnalysis() {
    ...
    try {
      ...
      for (int loopdate = 0; loopdate < nbDate; loopdate++) {
        ...
        final double accrualFactor = EONIA.getDayCount().yearFraction(startDate, endDate);
