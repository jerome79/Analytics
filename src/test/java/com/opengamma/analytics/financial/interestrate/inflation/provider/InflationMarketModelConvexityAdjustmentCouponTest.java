...
public class InflationMarketModelConvexityAdjustmentCouponTest {
  ...
  public void valueForZeroCouponMonthly() {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      pricesWithConvexity[i] = METHOD_ZC_MONTHLY_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE).getAmount(zeroCoupons[i].getCurrency()).getAmount();
      pricesWithoutConvexity[i] = METHOD_ZC_MONTHLY.presentValue(zeroCoupons[i], INFLATION_PROVIDER).getAmount(zeroCoupons[i].getCurrency()).getAmount();
  ...
  public void valueForZeroCouponInterpolation() {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      convexitysAdjustments[i] = CONVEXITY_ADJUSTMENT_FUNCTION.getZeroCouponConvexityAdjustment(zeroCoupons[i], PARAMETER_INTERFACE);
      pricesWithConvexity[i] = METHOD_ZC_INTERPOLATION_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE).getAmount(zeroCoupons[i].getCurrency()).getAmount();
      pricesWithoutConvexity[i] = METHOD_ZC_INTERPOLATION.presentValue(zeroCoupons[i], INFLATION_PROVIDER).getAmount(zeroCoupons[i].getCurrency()).getAmount();
  ...
  public void valueForYearOnYearInterpolation() {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      pricesWithConvexity[i] = METHOD_YOY_INTERPOLATION_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE).getAmount(zeroCoupons[i].getCurrency()).getAmount();
      pricesWithoutConvexity[i] = METHOD_YOY_INTERPOLATION.presentValue(zeroCoupons[i], INFLATION_PROVIDER).getAmount(zeroCoupons[i].getCurrency()).getAmount();
  ...
  public void valueForYearOnYearMonthly() {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      zeroCoupons[i] = zeroCouponDefinitions[i].toDerivative(PRICING_DATE);
      convexitysAdjustments[i] = CONVEXITY_ADJUSTMENT_FUNCTION.getYearOnYearConvexityAdjustment(zeroCoupons[i], PARAMETER_INTERFACE);
      pricesWithConvexity[i] = METHOD_YOY_MONTHLY_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE).getAmount(zeroCoupons[i].getCurrency()).getAmount();
      pricesWithoutConvexity[i] = METHOD_YOY_MONTHLY.presentValue(zeroCoupons[i], INFLATION_PROVIDER).getAmount(zeroCoupons[i].getCurrency()).getAmount();
