...
public class CouponIborAverageFixingDatesCompoundingDiscountingMethodTest {
  ...
  static {
    ...
    for (int i = 0; i < NUM_OBS; ++i) {
      for (int j = 0; j < NUM_PRDS; ++j) {
        FIX_ACC_FACTORS1[j][i] = DayCountUtils.yearFraction(INDEX1.getDayCount(), EXP_START_DATES1[j][i], EXP_END_DATES1[j][i], CALENDAR);
        FIX_ACC_FACTORS2[j][i] = DayCountUtils.yearFraction(INDEX2.getDayCount(), EXP_START_DATES2[j][i], EXP_END_DATES2[j][i], CALENDAR);
      }
    }
  }
  ...
  public void presentValueFixedTest() {
    final MultiCurrencyAmount pvComputed = METHOD.presentValue(DER1, MULTICURVES);
    ...
    assertEquals("CouponIborAverageCompoundingDiscountingMethod: present value", pvExpected, pvComputed.getAmount(DER1.getCurrency()).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount pvWithCalc = PVDC.visitCouponIborAverageCompounding(DER1, MULTICURVES);
    assertEquals("CouponIborAverageCompoundingDiscountingMethod: present value", pvWithCalc.getAmount(DER1.getCurrency()).getAmount(),
        pvComputed.getAmount(DER1.getCurrency()).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueNotFixedTest() {
    final MultiCurrencyAmount pvComputed = METHOD.presentValue(DER2, MULTICURVES);
    ...
    assertEquals(pvExpected, pvComputed.getAmount(DER2.getCurrency()).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount pvWithCalc = PVDC.visitCouponIborAverageCompounding(DER2, MULTICURVES);
    assertEquals(pvWithCalc.getAmount(DER2.getCurrency()).getAmount(), pvComputed.getAmount(DER2.getCurrency()).getAmount(), EPS * Math.abs(pvExpected));
  }
  ...
  public void presentValueNotFixedNoSquareTest() {
    final MultiCurrencyAmount pvComputed = METHOD.presentValue(DER3, MULTICURVES);
    ...
    assertEquals(pvExpected, pvComputed.getAmount(DER3.getCurrency()).getAmount(), EPS * Math.abs(pvExpected));
    final MultiCurrencyAmount pvWithCalc = PVDC.visitCouponIborAverageCompounding(DER3, MULTICURVES);
    assertEquals(pvWithCalc.getAmount(DER3.getCurrency()).getAmount(), pvComputed.getAmount(DER3.getCurrency()).getAmount(), EPS * Math.abs(pvExpected));
  }
