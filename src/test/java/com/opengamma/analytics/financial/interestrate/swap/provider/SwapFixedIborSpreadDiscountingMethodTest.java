...
public class SwapFixedIborSpreadDiscountingMethodTest {
  ...
  public void couponEquivalentSpreadModified() {
    ...
    final double pvFixed = SWAP_SPREAD_EUR1Y3M.getFixedLeg().accept(PVDC, MULTICURVES).getAmount(EUR).getAmount();
    final double pvAnnuitySpread = ANNUITY_SPREAD.accept(PVDC, MULTICURVES).getAmount(EUR).getAmount();
    ...
    final double pvIbor = SWAP_SPREAD_EUR1Y3M.getSecondLeg().accept(PVDC, MULTICURVES).getAmount(EUR).getAmount();
  ...
  public void presentValueIborNoSpreadPositiveNotional() {
    ...
    final MultiCurrencyAmount pvAnnuitySpread = ANNUITY_SPREAD.accept(PVDC, MULTICURVES); // Should be negative: pay float
    assertEquals("SwapFixedIborSpreadDiscountingMethod: presentValueIborNoSpreadPositiveNotional", -pvAnnuitySpread.getAmount(EUR).getAmount(), pvs.getAmount(), TOLERANCE_PV);
  }
  ...
  public void forwardSwapSpreadModified() {
    ...
    final double pvAnnuitySpread = ANNUITY_SPREAD.accept(PVDC, MULTICURVES).getAmount(EUR).getAmount();
    final double pvAnnuityIbor = SWAP_SPREAD_EUR1Y3M.getSecondLeg().accept(PVDC, MULTICURVES).getAmount(EUR).getAmount();
