...
public class STIRFuturesOptionMarginTransactionBlackExpLogMoneynessMethodTest {
  ...
  public void presentValue() {
    final MultiCurrencyAmount pvComputed1 = METHOD_OPT_TRA.presentValue(CALL_ERZ4_099_TRA_1, MULTICURVE_BLACK);
    ...
    assertEquals("BondFuturesOptionMarginTransactionBlackFlatMethod: present value", pvExpected1,
        pvComputed1.getAmount(ERZ4_DEFINITION.getCurrency()).getAmount(), TOLERANCE_PV);
    assertTrue("BondFuturesOptionMarginTransactionBlackFlatMethod: present value", pvComputed1.size() == 1);
    final MultiCurrencyAmount pvCalculator = CALL_ERZ4_099_TRA_1.accept(PVBFOC, MULTICURVE_BLACK);
    assertEquals("BondFuturesOptionMarginTransactionBlackFlatMethod: present value", pvExpected1, pvCalculator.getAmount(EUR).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount pvComputed2 = METHOD_OPT_TRA.presentValue(CALL_ERZ4_099_TRA_2, MULTICURVE_BLACK);
    ...
    assertEquals("STIRFuturesOptionMarginTransactionBlackExpLogMoneynessMethod: present value", pvExpected2,
        pvComputed2.getAmount(ERZ4_DEFINITION.getCurrency()).getAmount(), TOLERANCE_PV);
  }

  public void presentValuePuCallParity() {
    final MultiCurrencyAmount priceFutures = METHOD_FUT_TRA.presentValue(ERZ4_TRA_STRIKE, MULTICURVE);
    final MultiCurrencyAmount pvCallComputed = METHOD_OPT_TRA.presentValue(CALL_ERZ4_099_TRA_1, MULTICURVE_BLACK);
    final MultiCurrencyAmount pvPutComputed = METHOD_OPT_TRA.presentValue(PUT_ERZ4_099_TRA_1, MULTICURVE_BLACK);
    assertEquals("STIRFuturesOptionMarginTransactionBlackExpLogMoneynessMethod: put call parity present value",
        pvCallComputed.getAmount(EURIBOR3M.getCurrency()).getAmount() - pvPutComputed.getAmount(EURIBOR3M.getCurrency()).getAmount(),
        priceFutures.getAmount(EURIBOR3M.getCurrency()).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueBlackSensitivity() {
    ...
    final MultiCurrencyAmount pvP = METHOD_OPT_TRA.presentValue(CALL_ERZ4_099_TRA_1, blackShiftedPlus);
    final MultiCurrencyAmount pvM = METHOD_OPT_TRA.presentValue(CALL_ERZ4_099_TRA_1, blackShiftedMinus);
    final double vegaExpected = (pvP.getAmount(CALL_ERZ4_099_TRA_1.getCurrency()).getAmount() - pvM.getAmount(CALL_ERZ4_099_TRA_1.getCurrency()).getAmount()) / (2 * shiftVega);
