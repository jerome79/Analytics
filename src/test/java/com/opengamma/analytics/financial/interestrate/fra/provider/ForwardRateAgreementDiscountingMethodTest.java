...
public class ForwardRateAgreementDiscountingMethodTest {
  ...
  private static final double ACCRUAL_FACTOR_PAYMENT = DayCountUtils.yearFraction(DAY_COUNT_PAYMENT, ACCRUAL_START_DATE, ACCRUAL_END_DATE);
  ...
  public void presentValue() {
    ...
    final MultiCurrencyAmount pv = FRA_METHOD.presentValue(FRA, MULTICURVE);
    assertEquals("FRA discounting: present value", expectedPv, pv.getAmount(CUR).getAmount(), TOLERANCE_PV);
  }

  @Test
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = FRA_METHOD.presentValue(FRA, MULTICURVE);
    final MultiCurrencyAmount pvCalculator = FRA.accept(PVDC, MULTICURVE);
    assertEquals("FRA discounting: present value calculator vs method", pvCalculator.getAmount(CUR).getAmount(), pvMethod.getAmount(CUR).getAmount(), 1.0E-2);
  }
  ...
  public void presentValueBuySellParity() {
    ...
    final MultiCurrencyAmount pvBuy = FRA_METHOD.presentValue(FRA, MULTICURVE);
    final MultiCurrencyAmount pvSell = FRA_METHOD.presentValue(fraSell, MULTICURVE);
    assertEquals("FRA discounting: present value - buy/sell parity", pvSell.getAmount(CUR).getAmount(), -pvBuy.getAmount(CUR).getAmount(), 1.0E-2);
  }
  ...
  public void parSpread() {
    ...
    final MultiCurrencyAmount pv0 = fra0.accept(PVDC, MULTICURVE);
    assertEquals("FRA discounting: par spread", pv0.getAmount(CUR).getAmount(), 0, TOLERANCE_PV);
  }
