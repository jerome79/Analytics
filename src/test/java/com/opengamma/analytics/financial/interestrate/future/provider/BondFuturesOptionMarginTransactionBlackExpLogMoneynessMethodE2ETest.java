...
public class BondFuturesOptionMarginTransactionBlackExpLogMoneynessMethodE2ETest {
  ...
  static {
    ...
    issuerProvider.setCurve(Pair.of((Object) ISSUER_NAME_SCH, filter), yieldCurve);
  ...
  static {
    ...
    YieldPeriodicCurve yieldCurve = YieldPeriodicCurve.from(1, interpolatedCurve);
    LegalEntityFilter<LegalEntity> filter = new LegalEntityShortName();
    issuerProvider.setCurve(Pair.of((Object) ISSUER_NAME_BUN, filter), yieldCurve);
  ...
  static {
    ...
    YieldPeriodicCurve yieldCurve = YieldPeriodicCurve.from(1, interpolatedCurve);
    LegalEntityFilter<LegalEntity> filter = new LegalEntityShortName();
    issuerProvider.setCurve(Pair.of((Object) ISSUER_NAME_BOB, filter), yieldCurve);
  ...
  public void SCHTest() {
    ...
    MultiCurrencyAmount pv = TRANSACTION_SCH.accept(PVBFC, blackNew); // multiplied by notional
    ...
    double pv01 = TRANSACTION_SCH.accept(PV01PC, blackNew).getMap().get(Pair.of(CURVE_NAME_SCH, EUR));
    ...
    assertRelative("SCHTest", 351.83034488344134, pv.getAmount(EUR).getAmount(), TOL);
  ...
  public void BUNTest() {
    ...
    MultiCurrencyAmount pv = TRANSACTION_BUN.accept(PVBFC, blackNew); // multiplied by notional
    ...
    double pv01 = TRANSACTION_BUN.accept(PV01PC, blackNew).getMap().get(Pair.of(CURVE_NAME_BUN, EUR));
    ...
    assertRelative("BUNTest", 301.1303343303409, pv.getAmount(EUR).getAmount(), TOL);
    assertArrayRelative("", bucketExpected, bucketedPv01.getData(), TOL);
  ...
  public void BOBTest() {
    ...
    MultiCurrencyAmount pv = TRANSACTION_BOB.accept(PVBFC, blackNew); // multiplied by notional
    ...
    double pv01 = TRANSACTION_BOB.accept(PV01PC, blackNew).getMap().get(Pair.of(CURVE_NAME_BOB, EUR));
    ...
    assertRelative("BOBTest", 88.37066933825166, pv.getAmount(EUR).getAmount(), TOL);
    assertArrayRelative("", bucketExpected, bucketedPv01.getData(), TOL);
