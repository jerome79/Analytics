...
public class SuccessiveLeastSquareSwaptionPhysicalLMMDDCalibrationObjectiveTest {
  ...
  public void calibration() {
    ...
    final MultiCurrencyAmount[][] pvSabr = new MultiCurrencyAmount[SWAP_TENOR_YEAR.length][NB_STRIKE];
    final MultiCurrencyAmount[][] pvLmm = new MultiCurrencyAmount[SWAP_TENOR_YEAR.length][NB_STRIKE];
    ...
    for (int loopexp = 0; loopexp < SWAP_TENOR_YEAR.length; loopexp++) {
      for (int loopstrike = 0; loopstrike < NB_STRIKE; loopstrike++) {
        ...
        pvDiff[loopexp][loopstrike] = pvSabr[loopexp][loopstrike].getAmount(EUR).getAmount() - pvLmm[loopexp][loopstrike].getAmount(EUR).getAmount();
    ...
    final MultiCurrencyAmount pvDirect = METHOD_LMM.presentValue(SWAPTION_AMORTIZED, objective.getLMMProvider());
    final MultiCurrencyAmount pvMethod = METHOD_CALIBRATION.presentValue(SWAPTION_AMORTIZED, SABR_MULTICURVES);
    assertEquals("LMM calibration least-square: swaption ", pvDirect.getAmount(EUR).getAmount(), pvMethod.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
