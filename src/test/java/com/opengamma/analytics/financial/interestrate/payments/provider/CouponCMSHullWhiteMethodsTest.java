...
public class CouponCMSHullWhiteMethodsTest {
  ...
  public void presentValueNumericalIntegration() {
    final MultiCurrencyAmount pvNumericalIntegration = METHOD_NI.presentValue(CPN_CMS, HW_MULTICURVES);
    final double pvPrevious = 851848.400; // From previous run
    assertEquals("Coupon CMS - Hull-White - present value - numerical integration", pvPrevious, pvNumericalIntegration.getAmount(EUR).getAmount(), TOLERANCE_PV);
    // Comparison with non-adjusted figures: to have the right order of magnitude
    final MultiCurrencyAmount pvDiscounting = METHOD_DSC.presentValue(CPN_CMS, MULTICURVES);
    assertEquals("Coupon CMS - Hull-White - present value - numerical integration", 1.0, pvDiscounting.getAmount(EUR).getAmount() / pvNumericalIntegration.getAmount(EUR).getAmount(), 0.20);
  }

  @Test
  public void presentValueApproximation() {
    final MultiCurrencyAmount pvNumericalIntegration = METHOD_NI.presentValue(CPN_CMS, HW_MULTICURVES);
    final MultiCurrencyAmount pvApproximation = METHOD_APP.presentValue(CPN_CMS, HW_MULTICURVES);
    assertEquals("Coupon CMS - Hull-White - present value - approximation", pvApproximation.getAmount(EUR).getAmount(), pvNumericalIntegration.getAmount(EUR).getAmount(), TOLERANCE_PV_APP);
  }
  ...
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = METHOD_APP.presentValue(CPN_CMS, HW_MULTICURVES);
    final MultiCurrencyAmount pvCalculator = CPN_CMS.accept(PVHWC, HW_MULTICURVES);
