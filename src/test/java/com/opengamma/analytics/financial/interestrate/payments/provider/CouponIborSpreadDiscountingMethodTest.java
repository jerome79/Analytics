...
public class CouponIborSpreadDiscountingMethodTest {
  ...
  public void presentValue() {
    final MultiCurrencyAmount pvComputed = METHOD_CPN_IBOR_SPREAD.presentValue(CPN_IBOR_SPREAD, PROVIDER);
    ...
    assertEquals("CouponIborSpreadDiscountingMethod: present value", pvExpected.getAmount(), pvComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount pvIbor = METHOD_CPN_IBOR.presentValue(CPN_IBOR, PROVIDER);
    final MultiCurrencyAmount pvFixed = METHOD_FIXED.presentValue(CPN_FIXED, PROVIDER);
    assertEquals("CouponIborSpreadDiscountingMethod: present value", pvIbor.plus(pvFixed).getAmount(EUR).getAmount(), pvComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }

  @Test
  public void presentValueNoSpreadPositivNotional() {
    final MultiCurrencyAmount pvComputed = METHOD_CPN_IBOR_SPREAD.presentValueNoSpreadPositiveNotional(CPN_IBOR_SPREAD, PROVIDER);
    final double forward = PROVIDER.getSimplyCompoundForwardRate(EURIBOR3M, CPN_IBOR_SPREAD.getFixingPeriodStartTime(), CPN_IBOR_SPREAD.getFixingPeriodEndTime(),
        CPN_IBOR_SPREAD.getFixingAccrualFactor());
    ...
    assertEquals("CouponIborSpreadDiscountingMethod: present value", pvExpected, pvComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }

  @Test
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = METHOD_CPN_IBOR_SPREAD.presentValue(CPN_IBOR_SPREAD, PROVIDER);
    final MultiCurrencyAmount pvCalculator = CPN_IBOR_SPREAD.accept(PVDC, PROVIDER);
  ...
  public void presentValueMarketQuoteSensitivity() {
    ...
    final MultiCurrencyAmount pvP = cpnSpreadP.accept(PVDC, PROVIDER);
    final MultiCurrencyAmount pvM = cpnSpreadM.accept(PVDC, PROVIDER);
    final double mqsExpected = (pvP.getAmount(EUR).getAmount() - pvM.getAmount(EUR).getAmount()) / (2 * SHIFT);
