...
public class ForwardRateAgreementDiscountingMethodE2ETest {
  ...
  public void parRateSpread() {
    ...
    final MultiCurrencyAmount pvComputed = fra0.accept(PVDC, MULTICURVE);
    assertEquals("ForwardRateAgreementDiscountingMethod: par rate from standard curves", 0.0, pvComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void BucketedPV01() {
    ...
    sensitivity.put(Pair.of(MULTICURVE.getName(USDLIBOR3M), USD), new DoubleMatrix1D(deltaFwd));
  ...
  public void presentValueAfterFee() {
    ...
    Pair<InstrumentDerivative[], MulticurveProviderInterface> data = Pair.of(new InstrumentDerivative[] {fee }, (MulticurveProviderInterface) MULTICURVE);
    final MultiCurrencyAmount pvComputed = FRA.accept(PVMULTIDC, data);
    final MultiCurrencyAmount pvExpected = MultiCurrencyAmount.of(Currency.USD, 0);
    assertEquals("ForwardRateAgreementDiscountingMethod: present value after fee from standard curves", pvExpected.getAmount(USD).getAmount(), pvComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
