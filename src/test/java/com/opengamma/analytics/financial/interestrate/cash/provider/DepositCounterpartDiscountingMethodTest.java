...
public class DepositCounterpartDiscountingMethodTest {
  ...
  public void presentValueTrade() {
    ...
    final MultiCurrencyAmount pvComputed = METHOD_DEPOSIT.presentValue(deposit, PROVIDER_ISSUER);
    ...
    assertEquals("DepositCounterpartDiscountingMethod: present value", pvExpected, pvComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueMethodVsCalculator() {
    final ZonedDateTime referenceDate = DateUtils.getUTCDate(2011, 12, 12);
    final DepositCounterpart deposit = DEPOSIT_CPTY_DEFINITION.toDerivative(referenceDate);
    final MultiCurrencyAmount pvMethod = METHOD_DEPOSIT.presentValue(deposit, PROVIDER_ISSUER);
    final MultiCurrencyAmount pvCalculator = deposit.accept(PVC, PROVIDER_ISSUER);
  ...
  public void parSpreadBeforeStart() {
    ...
    final MultiCurrencyAmount pv0 = METHOD_DEPOSIT.presentValue(deposit0, PROVIDER_ISSUER);
    assertEquals("DepositDefinition: present value", 0, pv0.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void todayPaymentBeforeStart() {
    final ZonedDateTime referenceDate = TRADE_DATE;
    final DepositCounterpart deposit = DEPOSIT_CPTY_DEFINITION.toDerivative(referenceDate);
    final MultiCurrencyAmount cash = deposit.accept(TPC);
    assertEquals("DepositDefinition: today payment", 0.0, cash.getAmount(deposit.getCurrency()).getAmount(), TOLERANCE_PV);
    assertEquals("DepositDefinition: today payment", 1, cash.size());
  }
  ...
  public void todayPaymentOnStart() {
    ...
    final DepositCounterpart deposit = DEPOSIT_CPTY_DEFINITION.toDerivative(referenceDate);
    final MultiCurrencyAmount cash = deposit.accept(TPC);
    assertEquals("DepositDefinition: today payment", -NOTIONAL, cash.getAmount(deposit.getCurrency()).getAmount(), TOLERANCE_PV);
    assertEquals("DepositDefinition: today payment", 1, cash.size());
  }
  ...
  public void todayPaymentOnEnd() {
    ...
    final DepositCounterpart deposit = DEPOSIT_CPTY_DEFINITION.toDerivative(referenceDate);
    final MultiCurrencyAmount cash = deposit.accept(TPC);
    assertEquals("DepositDefinition: today payment", NOTIONAL + deposit.getInterestAmount(), cash.getAmount(deposit.getCurrency()).getAmount(), TOLERANCE_PV);
    assertEquals("DepositDefinition: today payment", 1, cash.size());
  }
