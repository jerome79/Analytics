...
public class InterestRateFutureOptionMarginNormalSmileMethodTest {
  ...
  public void presentValue() {
    ...
    final MultiCurrencyAmount presentValue1Computed = METHOD_TRANSACTION_OPTION_NORMAL.presentValue(TRANSACTION_1, NORMAL_MULTICURVES);
    assertEquals("Future option with Black volatilities: option transaction pv", presentValue1Expected, presentValue1Computed.getAmount(EUR).getAmount(), TOLERANCE_PV);
    final double presentValue2Expected = (priceOption - TRADE_PRICE) * QUANTITY * NOTIONAL * FUTURE_FACTOR;
    final MultiCurrencyAmount presentValue2Computed = METHOD_TRANSACTION_OPTION_NORMAL.presentValue(TRANSACTION_2, NORMAL_MULTICURVES);
    assertEquals("Future option with Black volatilities: option transaction pv", presentValue2Expected, presentValue2Computed.getAmount(EUR).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount presentValue1Calculator = TRANSACTION_1.accept(PVNFC, NORMAL_MULTICURVES);
    assertEquals("Future option with Black volatilities: option transaction pv", presentValue1Computed.getAmount(EUR).getAmount(), presentValue1Calculator.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueMoneyness() {
    ...
    MultiCurrencyAmount presentValue1Computed = METHOD_TRANSACTION_OPTION_NORMAL.presentValue(TRANSACTION_1,
        NORMAL_MULTICURVES_MONEYNESS);
    assertEquals("Future option with volatilities: option transaction pv", presentValue1Expected,
        presentValue1Computed.getAmount(EUR).getAmount(), TOLERANCE_PV);
    double presentValue2Expected = (priceOption - TRADE_PRICE) * QUANTITY * NOTIONAL * FUTURE_FACTOR;
    MultiCurrencyAmount presentValue2Computed = METHOD_TRANSACTION_OPTION_NORMAL.presentValue(TRANSACTION_2,
        NORMAL_MULTICURVES_MONEYNESS);
    assertEquals("Future option with volatilities: option transaction pv", presentValue2Expected,
        presentValue2Computed.getAmount(EUR).getAmount(), TOLERANCE_PV);
    MultiCurrencyAmount presentValue1Calculator = TRANSACTION_1.accept(PVNFC, NORMAL_MULTICURVES_MONEYNESS);
    assertEquals("Future option with volatilities: option transaction pv", presentValue1Computed.getAmount(EUR).getAmount(),
        presentValue1Calculator.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
