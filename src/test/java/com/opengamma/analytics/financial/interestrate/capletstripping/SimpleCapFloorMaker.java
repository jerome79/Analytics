...
public class SimpleCapFloorMaker {
  ...
  private static CapFloorIbor[] makeCapletStrip(final Currency ccy, final IborIndex index, final int start, final int end, final double strike,
    ...
    ArgChecker.notNull(ccy, "null ccy");
    ArgChecker.notNull(index, "null index");
    ArgChecker.isTrue(strike >= 0.0, "negative strike");
    ArgChecker.isTrue(end > start, "end index must be greater that start index");
