...
public class BillTransactionDiscountingMethodTest {
  ...
  public void presentValue() {
    final MultiCurrencyAmount pvTransactionComputed = METHOD_TRANSACTION.presentValue(BILL_TRA, ISSUER_MULTICURVE);
    MultiCurrencyAmount pvSecurity = METHOD_SECURITY.presentValue(BILL_TRA.getBillPurchased(), ISSUER_MULTICURVE);
    ...
    assertEquals("Bill Security: discounting method - present value", pvSecurity.getAmount(EUR).getAmount() + pvSettle, pvTransactionComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueFromYield() {
    final MultiCurrencyAmount pvTransactionComputed = METHOD_TRANSACTION.presentValueFromYield(BILL_TRA, YIELD, ISSUER_MULTICURVE);
    final MultiCurrencyAmount pvSecurity = METHOD_SECURITY.presentValue(BILL_TRA.getBillStandard(), ISSUER_MULTICURVE);
    ...
    final MultiCurrencyAmount pvTransactionExpected = pvSecurity.plus(MultiCurrencyAmount.of(BILL_TRA.getCurrency(), pvSettle));
    assertEquals("Bill Security: discounting method - present value from yield", pvTransactionExpected.getAmount(EUR).getAmount(), pvTransactionComputed.getAmount(EUR).getAmount(), TOLERANCE_PV_DELTA);
  }
  ...
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = METHOD_TRANSACTION.presentValue(BILL_TRA, ISSUER_MULTICURVE);
    final MultiCurrencyAmount pvCalculator = BILL_TRA.accept(PVIC, ISSUER_MULTICURVE);
    assertEquals("Bill Security: discounting method - present value", pvMethod.getAmount(EUR).getAmount(), pvCalculator.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void parSpread() {
    ...
    final MultiCurrencyAmount pv0 = METHOD_TRANSACTION.presentValue(bill0, ISSUER_MULTICURVE);
    assertEquals("Bill Security: discounting method - par spread", 0, pv0.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
