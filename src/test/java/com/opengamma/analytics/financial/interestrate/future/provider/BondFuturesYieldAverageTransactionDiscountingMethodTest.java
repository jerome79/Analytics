...
public class BondFuturesYieldAverageTransactionDiscountingMethodTest {
  ...
  public void presentValueFromPrice() {
    ...
    final MultiCurrencyAmount pvComputed = METHOD_FI_TRA.presentValueFromPrice(FUT_TRA, quotedPrice);
    assertEquals("YieldAverageBondFuturesTransactionDiscountingMethod: presentValueFromPrice", pvExpected, pvComputed.getAmount(AUD).getAmount(), TOLERANCE_PV);
  }

  @Test
  /** Tests present value from the curves **/
  public void presentValue() {
    final MultiCurrencyAmount pvComputed = METHOD_FI_TRA.presentValue(FUT_TRA, ISSUER_MULTICURVE);
    final double priceFromCurves = METHOD_FI_SEC.price(FUT_SEC, ISSUER_MULTICURVE);
    final MultiCurrencyAmount pvExpected = METHOD_FI_TRA.presentValueFromPrice(FUT_TRA, priceFromCurves);
    assertEquals("YieldAverageBondFuturesTransactionDiscountingMethod: presentValue", pvExpected.getAmount(AUD).getAmount(), pvComputed.getAmount(AUD).getAmount(), TOLERANCE_PV);
  }
