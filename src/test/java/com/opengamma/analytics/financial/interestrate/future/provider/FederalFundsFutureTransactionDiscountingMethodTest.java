...
public class FederalFundsFutureTransactionDiscountingMethodTest {
  ...
  public void presentValueFromPrice() {
    ...
    final MultiCurrencyAmount pv = METHOD_TRANSACTION.presentValueFromPrice(FUTURE_TRANSACTION, price);
    ...
    assertEquals("Federal Funds Future transaction: present value", pvExpected, pv.getAmount(USD).getAmount(), TOLERANCE_PV);
  }

  @Test
  public void presentValue() {
    final MultiCurrencyAmount pv = METHOD_TRANSACTION.presentValue(FUTURE_TRANSACTION, MULTICURVES);
    final double price = METHOD_SECURITY.price(FUTURE_SECURITY, MULTICURVES);
    final MultiCurrencyAmount pvExpected = METHOD_TRANSACTION.presentValueFromPrice(FUTURE_TRANSACTION, price);
    assertEquals("Federal Funds Future transaction: present value", pvExpected.getAmount(USD).getAmount(), pv.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void parSpreadMarketQuote() {
    ...
    final MultiCurrencyAmount pv0 = METHOD_TRANSACTION.presentValue(futures0, MULTICURVES);
    assertEquals("FederalFundsFutureTransactionDiscountingMethod: par spread market quote", pv0.getAmount(USD).getAmount(), 0, TOLERANCE_PV);
  }
