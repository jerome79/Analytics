...
public abstract class SimpleInstrumentFactory {
  ...
  public static InstrumentDerivative makeCash(double time, double rate, double notional) {
    return new Cash(DUMMY_CUR, 0, time, notional, rate, time);
  }

  public static InstrumentDerivative makeLibor(double time, double rate, double notional) {
    return new Cash(DUMMY_CUR, 0, time, notional, rate, time);
  }

  /**
   * makes a very simple FRA with  payment time, fixing time and fixing period start being identical
   * and an amount tau before fixing period end. The payment and fixing year fractions are
   ...
   * @return A FRA
   */
  public static InstrumentDerivative makeFRA(double time, Frequency paymentFreq, double rate, double notional) {
    double tau = 1. / paymentFreq.eventsPerYear();
    return new ForwardRateAgreement(DUMMY_CUR, time - tau, tau, notional, DUMMY_INDEX, time - tau, time - tau, time, tau, rate);
  }

  public static InstrumentDerivative makeFuture(double time, Frequency paymentFreq) {
    double tau = 1. / paymentFreq.eventsPerYear();
