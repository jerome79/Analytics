...
public class SwaptionCashFixedIborBlackMethodTest {
  ...
  public void presentValue() {
    ...
    final CurrencyAmount ca = pvMethod.stream().findFirst().get();
  ...
  public void presentValueBlackSensitivity() {
    ...
    assertEquals("Swaption Black method: present value volatility sensitivity", (pvP.stream().findFirst().get().getAmount() - pvM.stream().findFirst().get().getAmount()) / (2 * shift),
  ...
  public void presentValueBlackNodeSensitivity() {
    ...
    for (int loopindex = 0; loopindex < x.length; loopindex++) {
      ...
      final BlackSwaptionFlatProvider curvesBlackM = new BlackSwaptionFlatProvider(MULTICURVES, blackM);
      final MultiCurrencyAmount pvM = METHOD_BLACK.presentValue(SWAPTION_LONG_REC, curvesBlackM);
      assertEquals("Swaption Black method: present value volatility sensitivity", (pvP.stream().findFirst().get().getAmount() - pvM.stream().findFirst().get().getAmount()) / (2 * shift), pvbns
