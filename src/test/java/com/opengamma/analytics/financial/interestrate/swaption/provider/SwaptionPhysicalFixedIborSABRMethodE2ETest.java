...
public class SwaptionPhysicalFixedIborSABRMethodE2ETest {
  ...
  public void presentValue() {
    final MultiCurrencyAmount pvComputed = SWAPTION_P_2Yx7Y.accept(PVSSC, MULTICURVE_SABR);
    final MultiCurrencyAmount pvExpected = MultiCurrencyAmount.of(USD, 3156216.4895777884);
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: present value from standard curves", pvExpected.getAmount(USD).getAmount(), pvComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void pv01() {
    ...
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: pv01 from standard curves", pv01dsc, pv01Computed.getMap().get(Pair.of(MULTICURVE.getName(USD), USD)), TOLERANCE_RATE);
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: pv01 from standard curves", pv01fwd, pv01Computed.getMap().get(Pair.of(MULTICURVE.getName(USDLIBOR3M), USD)), TOLERANCE_RATE);
  }
  ...
  public void BucketedPV01() {
    ...
    sensitivity.put(Pair.of(MULTICURVE.getName(USDLIBOR3M), USD), new DoubleMatrix1D(deltaFwd3));
