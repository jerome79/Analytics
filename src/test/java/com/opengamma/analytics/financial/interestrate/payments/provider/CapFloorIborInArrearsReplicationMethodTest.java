...
public class CapFloorIborInArrearsReplicationMethodTest {
  ...
  private static final double ACCRUAL_FACTOR = EURIBOR6M.getDayCount().yearFraction(START_ACCRUAL_DATE, END_ACCRUAL_DATE, CALENDAR);
  ...
  public void persentValueSABRExtrapolation() {
    ...
    final MultiCurrencyAmount priceStandard = capStandard.accept(PVSCC, SABR_MULTICURVES);
    ...
    final double strikePart = (1.0 + CAP_LONG.getFixingAccrualFactor() * STRIKE) * priceStandard.getAmount(EUR).getAmount();
    ...
    final MultiCurrencyAmount price = METHOD_SABREXTRA_CAP_IA.presentValue(CAP_LONG, SABR_MULTICURVES);
    final double priceExpected = (strikePart + integralPart) / beta;
    assertEquals("Cap/floor IA - SABR pricing", priceExpected, price.getAmount(EUR).getAmount(), TOLERANCE_PV);
  ...
  public void persentValueSABRExtrapolationParity() {
    final MultiCurrencyAmount priceCapLong = METHOD_SABREXTRA_CAP_IA.presentValue(CAP_LONG, SABR_MULTICURVES);
  ...
  private static class InArrearsIntegrant extends Function1D<Double, Double> {
     ...
     * @param sabr The SABR data bundle used in the standard cap/floor pricing.
     */
    ...
    public Double evaluate(final Double x) {
      ...
      return _baseMethod.presentValue(capStrike, _sabrData).getAmount(EUR).getAmount();
    }
  }
