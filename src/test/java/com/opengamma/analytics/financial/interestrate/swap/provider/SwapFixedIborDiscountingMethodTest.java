...
public class SwapFixedIborDiscountingMethodTest {
  ...
  public void presentValue2Iborindex() {
    final MultiCurrencyAmount pvCalcCpn3 = CPN_IBOR_3.accept(PVDC, MULTICURVES);
    final MultiCurrencyAmount pvMethCpn3 = METHOD_CPN_IBOR.presentValue(CPN_IBOR_3, MULTICURVES);
    final MultiCurrencyAmount pvCalcCpn6 = CPN_IBOR_6.accept(PVDC, MULTICURVES);
    final MultiCurrencyAmount pvMethCpn6 = METHOD_CPN_IBOR.presentValue(CPN_IBOR_6, MULTICURVES);
    final MultiCurrencyAmount pvCalcAnnIbor = ANNUITY_IBOR.accept(PVDC, MULTICURVES);
    final MultiCurrencyAmount pvCalcAnnFixed = ANNUITY_FIXED.accept(PVDC, MULTICURVES);
    final MultiCurrencyAmount pvCalcSwap = SWAP.accept(PVDC, MULTICURVES);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvMethCpn3.getAmount(EUR).getAmount(), pvCalcCpn3.getAmount(EUR).getAmount(), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvMethCpn6.getAmount(EUR).getAmount(), pvCalcCpn6.getAmount(EUR).getAmount(), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvCalcCpn3.plus(pvCalcCpn6).getAmount(EUR).getAmount(), pvCalcAnnIbor.getAmount(EUR).getAmount(), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvCalcCpn3.plus(pvCalcCpn6).getAmount(EUR).getAmount(), pvCalcAnnIbor.getAmount(EUR).getAmount(), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvCalcAnnFixed.plus(pvCalcAnnIbor).getAmount(EUR).getAmount(), pvCalcSwap.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
