...
public class SwaptionBermudaFixedIborTest {
  ...
  static {
    for (int loopexp = 0; loopexp < NB_EXPIRY; loopexp++) {
      EXPIRY_TIME[loopexp] = DayCountUtils.yearFraction(ACT_ACT, REFERENCE_DATE, EXPIRY_DATE[loopexp]);
      EXPIRY_SWAP[loopexp] = EXPIRY_SWAP_DEFINITION[loopexp].toDerivative(REFERENCE_DATE);
      SETTLE_TIME[loopexp] = DayCountUtils.yearFraction(ACT_ACT, REFERENCE_DATE, EXPIRY_SWAP_DEFINITION[loopexp].getFixedLeg().getNthPayment(0).getAccrualStartDate());
    }
  }
