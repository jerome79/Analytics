...
public class CapFloorCMSSABRReplicationMethodTest {
  ...
  public void testPriceReplication() {
    ...
    final MultiCurrencyAmount priceCMSCoupon = PVSSC.visit(CMS_COUPON, SABR_MULTICURVES);
    final MultiCurrencyAmount priceCMSCap0 = PVSSC.visit(CMS_CAP_0, SABR_MULTICURVES);
    assertEquals("CapFloorCMSSABRReplicationMethod: present value", priceCMSCoupon.getAmount(EUR).getAmount(), priceCMSCap0.getAmount(EUR).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount priceCMSCap = PVSSC.visit(CMS_CAP, SABR_MULTICURVES);
    assertEquals("CapFloorCMSSABRReplicationMethod: present value", 5224.559, priceCMSCap.getAmount(EUR).getAmount(), TOLERANCE_PV); //From previous run
    final MultiCurrencyAmount priceCMSFloor = PVSSC.visit(CMS_FLOOR, SABR_MULTICURVES);
    assertEquals("CapFloorCMSSABRReplicationMethod: present value", 20149.939, priceCMSFloor.getAmount(EUR).getAmount(), TOLERANCE_PV); //From previous run
    ...
    assertEquals("CapFloorCMSSABRReplicationMethod: present value", priceCMSCap.getAmount(EUR).getAmount() - priceCMSFloor.getAmount(EUR).getAmount(), priceCMSCoupon.getAmount(EUR).getAmount() -
        priceStrike.getAmount(EUR).getAmount(), 2.0E+3);
  }
  ...
  public void presentValueMethodVsCalculator() {
    final double pvMethod = METHOD_CAP_CMS_SABR.presentValue(CMS_CAP, SABR_MULTICURVES).getAmount(EUR).getAmount();
    final double pvCalculator = PVSSC.visit(CMS_CAP, SABR_MULTICURVES).getAmount(EUR).getAmount();
  ...
  public void presentValueSABRSensitivity() {
    final double pv = METHOD_CAP_CMS_SABR.presentValue(CMS_CAP, SABR_MULTICURVES).getAmount(EUR).getAmount();
    ...
    final double pvLongPayerAlphaBumped = METHOD_CAP_CMS_SABR.presentValue(CMS_CAP, sabrBundleAlphaBumped).getAmount(EUR).getAmount();
    ...
    final double pvLongPayerRhoBumped = METHOD_CAP_CMS_SABR.presentValue(CMS_CAP, sabrBundleRhoBumped).getAmount(EUR).getAmount();
    ...
    final double pvLongPayerNuBumped = METHOD_CAP_CMS_SABR.presentValue(CMS_CAP, sabrBundleNuBumped).getAmount(EUR).getAmount();
  ...
  public void presentValueStrikeSensitivityCap() {
    ...
    for (int loopstrike = 0; loopstrike < nbStrikes; loopstrike++) {
      ...
      final double pvShiftUp = METHOD_CAP_CMS_SABR.presentValue(cmsCapShiftUp, SABR_MULTICURVES).getAmount(EUR).getAmount();
      final double pvShiftDo = METHOD_CAP_CMS_SABR.presentValue(cmsCapShiftDo, SABR_MULTICURVES).getAmount(EUR).getAmount();
  ...
  public void presentValueStrikeSensitivityFloor() {
    ...
    for (int loopstrike = 0; loopstrike < nbStrikes; loopstrike++) {
      ...
      final double pvShiftUp = METHOD_CAP_CMS_SABR.presentValue(cmsFloorShiftUp, SABR_MULTICURVES).getAmount(EUR).getAmount();
      final double pvShiftDo = METHOD_CAP_CMS_SABR.presentValue(cmsFloorShiftDo, SABR_MULTICURVES).getAmount(EUR).getAmount();
      final double sensiExpected = (pvShiftUp - pvShiftDo) / (2 * shift);
  ...
  public void presentValueAnnuity() {
    ...
    final double pvCalculator = PVSSC.visit(cap, SABR_MULTICURVES).getAmount(EUR).getAmount();
    double pvExpected = 0.0;
    for (int loopcpn = 0; loopcpn < cap.getNumberOfPayments(); loopcpn++) {
      pvExpected += PVSSC.visit(cap.getNthPayment(loopcpn), SABR_MULTICURVES).getAmount(EUR).getAmount();
    }
