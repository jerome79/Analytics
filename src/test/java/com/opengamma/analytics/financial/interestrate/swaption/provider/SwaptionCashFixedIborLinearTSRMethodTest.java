...
public class SwaptionCashFixedIborLinearTSRMethodTest {
  ...
  public void presentValueLongShortParity() {
    final MultiCurrencyAmount pvLongTSR = METHOD_CASH_TSR.presentValue(SWAPTION_PAYER_LONG, SABR_MULTICURVES);
    final MultiCurrencyAmount pvShortTSR = METHOD_CASH_TSR.presentValue(SWAPTION_PAYER_SHORT, SABR_MULTICURVES);
    assertEquals("Cash-settled swaption: linear TSR: present value - long/short parity", -pvLongTSR.getAmount(EUR).getAmount(), pvShortTSR.getAmount(EUR).getAmount(), 1E-2);
  }
  ...
  public void presentValueMultiStrike() {
    ...
    for (int loopstrike = 0; loopstrike < nbStrike + 1; loopstrike++) {
      pvCashStandard[loopstrike] = METHOD_CASH_SABR.presentValue(swaptionCash[loopstrike], SABR_MULTICURVES).getAmount(EUR).getAmount();
      pvCashTSR[loopstrike] = METHOD_CASH_TSR.presentValue(swaptionCash[loopstrike], SABR_MULTICURVES).getAmount(EUR).getAmount();
      pvPhysical[loopstrike] = METHOD_PHYS_SABR.presentValue(swaptionPhys[loopstrike], SABR_MULTICURVES).getAmount(EUR).getAmount();
    }
  }
