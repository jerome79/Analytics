...
public class CapFloorHullWhiteCalibrationObjectiveTest {
  ...
  public void calibration() {
    ...
    final MultiCurrencyAmount[] pvSabr = new MultiCurrencyAmount[CAP.getNumberOfPayments()];
    final MultiCurrencyAmount[] pvHw = new MultiCurrencyAmount[CAP.getNumberOfPayments()];
    for (int loopexp = 0; loopexp < CAP.getNumberOfPayments(); loopexp++) {
      ...
      assertEquals("Hull-White calibration: cap/floor " + loopexp, pvSabr[loopexp].getAmount(EUR).getAmount(), pvHw[loopexp].getAmount(EUR).getAmount(), TOLERANCE_PV);
    }
  }
