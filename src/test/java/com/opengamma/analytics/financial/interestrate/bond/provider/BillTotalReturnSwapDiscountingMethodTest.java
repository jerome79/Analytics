...
public class BillTotalReturnSwapDiscountingMethodTest {
  ...
  public void presentValueFixedSameCurrencyBeforeEffective() {
    ...
    assertTrue("BillTRSDiscountingMethod: present value", pvComputedPay.getAmount(EUR).getAmount() != 0.0);
    MultiCurrencyAmount pvBillUnit = BELDEC14_1.accept(PVIC, ISSUER_MULTICURVE);
    MultiCurrencyAmount pvFunding = FUNDING_LEG_FIXED_REC_1.accept(PVDC, ISSUER_MULTICURVE.getMulticurveProvider());
    MultiCurrencyAmount pvExpected = pvBillUnit.multipliedBy(-NOTIONAL_BILL).plus(pvFunding);
    assertEquals("BillTRSDiscountingMethod: present value", pvExpected.getAmount(EUR).getAmount(), pvComputedPay.getAmount(EUR).getAmount(), TOLERANCE_PV);
    MultiCurrencyAmount pvComputedRec = METHOD_TRS_BND.presentValue(TRS_REC_FIXED_PAY_1, ISSUER_MULTICURVE);
    assertEquals("BillTRSDiscountingMethod: present value", -pvComputedPay.getAmount(EUR).getAmount(), pvComputedRec.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueFixedSameCurrencyAfterEffective() {
    ...
    assertTrue("BillTRSDiscountingMethod: present value", pvComputed.getAmount(EUR).getAmount() != 0.0);
    MultiCurrencyAmount pvBillUnit = BELDEC14_2.accept(PVIC, ISSUER_MULTICURVE);
    MultiCurrencyAmount pvFunding = FUNDING_LEG_FIXED_REC_2.accept(PVDC, ISSUER_MULTICURVE.getMulticurveProvider());
    MultiCurrencyAmount pvExpected = pvBillUnit.multipliedBy(-NOTIONAL_BILL).plus(pvFunding);
    assertEquals("BillTRSDiscountingMethod: present value", pvExpected.getAmount(EUR).getAmount(), pvComputed.getAmount(EUR).getAmount(), TOLERANCE_PV); // Bill and funding in same currency
  }
  ...
  public void presentValueIborDiffCurrencyBeforeEffective() {
    ...
    assertTrue("BillTRSDiscountingMethod: present value", pvComputedRec.getAmount(EUR).getAmount() != 0.0);
    assertTrue("BillTRSDiscountingMethod: present value", pvComputedRec.getAmount(USD).getAmount() != 0.0);
    MultiCurrencyAmount pvBillUnit = BELDEC14_1.accept(PVIC, ISSUER_MULTICURVE);
    MultiCurrencyAmount pvFunding = FUNDING_LEG_IBOR_PAY_1.accept(PVDC, ISSUER_MULTICURVE.getMulticurveProvider());
    MultiCurrencyAmount pvExpected = pvBillUnit.multipliedBy(NOTIONAL_BILL).plus(pvFunding);
    assertEquals("BillTRSDiscountingMethod: present value", pvExpected.getAmount(EUR).getAmount(), pvComputedRec.getAmount(EUR).getAmount(), TOLERANCE_PV);
    assertEquals("BillTRSDiscountingMethod: present value", pvFunding.getAmount(USD).getAmount(), pvComputedRec.getAmount(USD).getAmount(), TOLERANCE_PV);
  }

  @Test
  public void presentValueLegs() {
    MultiCurrencyAmount pvBillLegExpected = TRS_REC_IBOR_PAY_1_EFF.getAsset().accept(PVIC, ISSUER_MULTICURVE).multipliedBy(NOTIONAL_BILL);
    MultiCurrencyAmount pvBillLegComputed = METHOD_TRS_BND.presentValueAssetLeg(TRS_REC_IBOR_PAY_1_EFF, ISSUER_MULTICURVE);
    assertEquals("BillTRSDiscountingMethod: present value", pvBillLegExpected.getAmount(EUR).getAmount(), pvBillLegComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
    ...
    assertEquals("BillTRSDiscountingMethod: present value", pvFundingLegExpected.getAmount(USD).getAmount(), pvFundingLegComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueMethodVsCalculator() {
    ...
    assertEquals("BillTRSDiscountingMethod: present value", pvMethod.getAmount(EUR).getAmount(), pvCalculator.getAmount(EUR).getAmount(), TOLERANCE_PV);
    assertEquals("BillTRSDiscountingMethod: present value", pvMethod.getAmount(USD).getAmount(), pvCalculator.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void pv01() {
    ...
    assertEquals("BillTRSDiscountingMethod: pv01", pv01Bill.getMap().get(Pair.of(ISSUER_MULTICURVE.getName(BELDEC14_DEFINITION.getIssuerEntity()), EUR)), pv01BillExpected, TOLERANCE_PV_DELTA);
    assertEquals("BillTRSDiscountingMethod: pv01", pv01Bill.getMap().get(Pair.of(ISSUER_MULTICURVE.getName(BELDEC14_DEFINITION.getIssuerEntity()), EUR)),
        pv01Computed.getMap().get(Pair.of(ISSUER_MULTICURVE.getName(BELDEC14_DEFINITION.getIssuerEntity()), EUR)), TOLERANCE_PV_DELTA);
  }
