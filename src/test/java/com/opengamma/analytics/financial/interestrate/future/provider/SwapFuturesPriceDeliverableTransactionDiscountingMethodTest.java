...
public class SwapFuturesPriceDeliverableTransactionDiscountingMethodTest {
  ...
  public void presentValue() {
    ...
    final MultiCurrencyAmount pvComputed = FTMC.presentValue(SWAP_FUTURES_TRANSACTION, MULTICURVE);
    final MultiCurrencyAmount pvExpected1 = FTMC.presentValueFromPrice(SWAP_FUTURES_TRANSACTION, price);
    assertEquals("SwapFuturesPriceDeliverableTransactionDiscountingMethod: present value", pvExpected1.getAmount(USD).getAmount(), pvComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
    ...
    assertEquals("SwapFuturesPriceDeliverableTransactionDiscountingMethod: present value", pvExpected2, pvComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }

  @Test
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = FTMC.presentValue(SWAP_FUTURES_TRANSACTION, MULTICURVE);
    final MultiCurrencyAmount pvCalculator = SWAP_FUTURES_TRANSACTION.accept(PVDC, MULTICURVE);
    assertEquals("SwapFuturesPriceDeliverableTransactionDiscountingMethod: present value", pvMethod.getAmount(USD).getAmount(), pvCalculator.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void parSpread() {
    ...
    final MultiCurrencyAmount pvComputed = FTMC.presentValue(fut0, MULTICURVE);
    assertEquals("SwapFuturesPriceDeliverableTransactionDiscountingMethod: present value", 0, pvComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
