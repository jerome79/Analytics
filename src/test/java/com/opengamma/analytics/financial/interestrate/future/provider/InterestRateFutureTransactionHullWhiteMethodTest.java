...
public class InterestRateFutureTransactionHullWhiteMethodTest {
  ...
  public void presentValue() {
    final MultiCurrencyAmount pvComputed = METHOD_IRFUT_TRA_HW.presentValue(ERU2_TRA, HW_MULTICURVES);
    ...
    assertEquals("InterestRateFutureSecurityHullWhiteProviderMethod: present value", pvExpected, pvComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }

  @Test
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = METHOD_IRFUT_TRA_HW.presentValue(ERU2_TRA, HW_MULTICURVES);
    final MultiCurrencyAmount pvCalculator = ERU2_TRA.accept(PVHWC, HW_MULTICURVES);
    assertEquals("InterestRateFutureSecurityHullWhiteProviderMethod: present value - calculator vs method", pvCalculator.getAmount(EUR).getAmount(), pvMethod.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
