...
public class CouponIborAverageFixingDatesDiscountingMethodTest {
  ...
  public void presentValueTest() {
    ...
    assertEquals(pvExpected, pvComputed.getAmount(DER1.getCurrency()).getAmount(), EPS * pvExpected);
    final MultiCurrencyAmount pvWithCalc = PVDC.visitCouponIborAverageFixingDates(DER1, MULTICURVES);
    assertEquals(pvWithCalc.getAmount(DER1.getCurrency()).getAmount(), pvComputed.getAmount(DER1.getCurrency()).getAmount(), EPS * pvExpected);
  }
  ...
  public void presentValueNoFixTest() {
    ...
    assertEquals(pvExpected, pvComputed.getAmount(DER2.getCurrency()).getAmount(), EPS * pvExpected);
    final MultiCurrencyAmount pvWithCalc = PVDC.visitCouponIborAverageFixingDates(DER2, MULTICURVES);
    assertEquals(pvWithCalc.getAmount(DER2.getCurrency()).getAmount(), pvComputed.getAmount(DER2.getCurrency()).getAmount(), EPS * pvExpected);
  }
