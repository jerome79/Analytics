...
public class SwapMultilegCalculatorTest {
  ...
  public void presentValueDiscountingCalculator() {
    final MultiCurrencyAmount pvSwap = SWAP_MULTI_LEG.accept(PVDC, MULTICURVES);
    MultiCurrencyAmount pvLegs = MultiCurrencyAmount.of(EUR, 0.0);
    for (int loopleg = 0; loopleg < NB_LEGS; loopleg++) {
      pvLegs = pvLegs.plus(SWAP_MULTI_LEG.getLegs()[loopleg].accept(PVDC, MULTICURVES));
    }
    assertEquals("SwapMultileg: presentValueDiscountingCalculator", pvSwap.getAmount(EUR).getAmount(), pvLegs.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
