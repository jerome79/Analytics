...
public class SwapCalculatorE2ETest {
  ...
  private void presentValueTest(InstrumentDerivative ins, MulticurveProviderDiscount multicurve, Currency ccy,
    ...
    assertEquals(msg, expectedPv, pvComputed.getAmount(ccy).getAmount(), TOLERANCE_PV);
  }
  ...
  public void BucketedPV01ONAA3M() {
    ...
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USD), USD), new DoubleMatrix1D(deltaDsc));
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USDLIBOR3M), USD), new DoubleMatrix1D(deltaFwd3));
    ...
    sensitivity2.put(Pair.of(MULTICURVE_OIS.getName(USD), USD), new DoubleMatrix1D(deltaDsc2));
    sensitivity2.put(Pair.of(MULTICURVE_OIS.getName(USDLIBOR3M), USD), new DoubleMatrix1D(deltaFwd32));
  ...
  public void BucketedPV01BS3M6M() {
    ...
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USD), USD), new DoubleMatrix1D(deltaDsc));
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USDLIBOR3M), USD), new DoubleMatrix1D(deltaFwd3));
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USDLIBOR6M), USD), new DoubleMatrix1D(deltaFwd6));
    final MultipleCurrencyParameterSensitivity pvpsExpected = new MultipleCurrencyParameterSensitivity(sensitivity);
    final MultipleCurrencyParameterSensitivity pvpsComputed =
  ...
  public void BucketedPV011M() {
    ...
    final LinkedHashMap<Pair<String, Currency>, DoubleMatrix1D> sensitivity = new LinkedHashMap<>();
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USD), USD), new DoubleMatrix1D(deltaDsc));
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USDLIBOR1M), USD), new DoubleMatrix1D(deltaFwd1));
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USDLIBOR3M), USD), new DoubleMatrix1D(deltaFwd3));
    final MultipleCurrencyParameterSensitivity pvpsExpected = new MultipleCurrencyParameterSensitivity(sensitivity);
  ...
  public void bucketedPV01Amortizing() {
    ...
    double[] deltaFwd = new double[] {2687.6311396467036, 3150.9120736542027, 1573.1143397288454, -2009.7387593026897,
      ...
      0.9985350036032012 };
    final LinkedHashMap<Pair<String, Currency>, DoubleMatrix1D> sensitivity = new LinkedHashMap<>();
    sensitivity.put(Pair.of(MULTICURVE_FFS.getName(USD), USD), new DoubleMatrix1D(deltaDsc));
    sensitivity.put(Pair.of(MULTICURVE_FFS.getName(USDLIBOR3M), USD), new DoubleMatrix1D(deltaFwd));
    final MultipleCurrencyParameterSensitivity pvpsExpected = new MultipleCurrencyParameterSensitivity(sensitivity);
    final MultipleCurrencyParameterSensitivity pvpsComputed =
