...
public class CapFloorIborSABRExtrapolationRightMethodTest {
  ...
  public void presentValueBelowCutOff() {
    final MultiCurrencyAmount methodPrice = METHOD_CAP_X.presentValue(CAP_LONG, SABR_MULTICURVES);
    ...
    assertEquals("Cap/floor: SABR with extrapolation pricing", expectedPrice, methodPrice.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueLongShortParityBelowCutOff() {
    final MultiCurrencyAmount priceLong = METHOD_CAP_X.presentValue(CAP_LONG, SABR_MULTICURVES);
    final MultiCurrencyAmount priceShort = METHOD_CAP_X.presentValue(CAP_SHORT, SABR_MULTICURVES);
    assertEquals("Cap/floor: SABR with extrapolation pricing: long/short parity", priceLong.getAmount(EUR).getAmount(), -priceShort.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueLongShortParityAboveCutOff() {
    final MultiCurrencyAmount priceLong = METHOD_CAP_X.presentValue(CAP_HIGH_LONG, SABR_MULTICURVES);
    final MultiCurrencyAmount priceShort = METHOD_CAP_X.presentValue(CAP_HIGH_SHORT, SABR_MULTICURVES);
    assertEquals("Cap/floor: SABR with extrapolation pricing: long/short parity", priceLong.getAmount(EUR).getAmount(), -priceShort.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueCapFloorParityBelowCutOff() {
    final MultiCurrencyAmount priceCap = METHOD_CAP_X.presentValue(CAP_LONG, SABR_MULTICURVES);
    final MultiCurrencyAmount priceFloor = METHOD_CAP_X.presentValue(FLOOR_SHORT, SABR_MULTICURVES);
    final MultiCurrencyAmount priceCouponStrike = COUPON_STRIKE.accept(PVDC, MULTICURVES);
    final MultiCurrencyAmount priceCouponIbor = COUPON_IBOR.accept(PVDC, MULTICURVES);
    assertEquals("Cap/floor: SABR with extrapolation pricing: cap/floor parity", priceCouponIbor.getAmount(EUR).getAmount() - priceCouponStrike.getAmount(EUR).getAmount(),
        priceCap.getAmount(EUR).getAmount() + priceFloor.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueCapFloorParityAboveCutOff() {
    final MultiCurrencyAmount priceCap = METHOD_CAP_X.presentValue(CAP_HIGH_LONG, SABR_MULTICURVES);
    final MultiCurrencyAmount priceFloor = METHOD_CAP_X.presentValue(FLOOR_HIGH_SHORT, SABR_MULTICURVES);
    final MultiCurrencyAmount priceCouponStrike = COUPON_STRIKE_HIGH.accept(PVDC, MULTICURVES);
    final MultiCurrencyAmount priceCouponIbor = COUPON_IBOR.accept(PVDC, MULTICURVES);
    assertEquals("Cap/floor: SABR with extrapolation pricing: cap/floor parity", priceCouponIbor.getAmount(EUR).getAmount() - priceCouponStrike.getAmount(EUR).getAmount(),
        priceCap.getAmount(EUR).getAmount() + priceFloor.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = METHOD_CAP_X.presentValue(CAP_LONG, SABR_MULTICURVES);
    final MultiCurrencyAmount pvCalculator = CAP_LONG.accept(PVSCXC, SABR_MULTICURVES);
    assertEquals("Cap/floor: SABR with extrapolation pricing - Method vs Calculator", pvMethod.getAmount(EUR).getAmount(), pvCalculator.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void testPresentValueSABRSensitivityBelowCutOff() {
    final MultiCurrencyAmount pv = METHOD_CAP_X.presentValue(CAP_LONG, SABR_MULTICURVES);
    ...
    final MultiCurrencyAmount pvLongPayerAlphaBumped = METHOD_CAP_X.presentValue(CAP_LONG, sabrBundleAlphaBumped);
    final double expectedAlphaSensi = (pvLongPayerAlphaBumped.getAmount(EUR).getAmount() - pv.getAmount(EUR).getAmount()) / shiftAlpha;
    ...
    final MultiCurrencyAmount pvLongPayerRhoBumped = METHOD_CAP_X.presentValue(CAP_LONG, sabrBundleRhoBumped);
    final double expectedRhoSensi = (pvLongPayerRhoBumped.getAmount(EUR).getAmount() - pv.getAmount(EUR).getAmount()) / shift;
    ...
    final MultiCurrencyAmount pvLongPayerNuBumped = METHOD_CAP_X.presentValue(CAP_LONG, sabrBundleNuBumped);
    final double expectedNuSensi = (pvLongPayerNuBumped.getAmount(EUR).getAmount() - pv.getAmount(EUR).getAmount()) / shift;
  ...
  public void testPresentValueSABRSensitivityAboveCutOff() {
    final MultiCurrencyAmount pv = METHOD_CAP_X.presentValue(CAP_HIGH_LONG, SABR_MULTICURVES);
    ...
    final SABRInterestRateParameters sabrParameterAlphaBumped = SABRDataSets.createSABR1AlphaBumped(shiftAlpha);
    final SABRCapProviderDiscount sabrBundleAlphaBumped = new SABRCapProviderDiscount(MULTICURVES, sabrParameterAlphaBumped, EURIBOR3M);
    final MultiCurrencyAmount pvLongPayerAlphaBumped = METHOD_CAP_X.presentValue(CAP_HIGH_LONG, sabrBundleAlphaBumped);
    final double expectedAlphaSensi = (pvLongPayerAlphaBumped.getAmount(EUR).getAmount() - pv.getAmount(EUR).getAmount()) / shiftAlpha;
    assertEquals("Number of alpha sensitivity", pvsCapLong.getAlpha().getMap().keySet().size(), 1);
    assertEquals("Alpha sensitivity expiry/tenor", pvsCapLong.getAlpha().getMap().keySet().contains(expectedExpiryTenor), true);
    ...
    final SABRInterestRateParameters sabrParameterRhoBumped = SABRDataSets.createSABR1RhoBumped();
    final SABRCapProviderDiscount sabrBundleRhoBumped = new SABRCapProviderDiscount(MULTICURVES, sabrParameterRhoBumped, EURIBOR3M);
    final MultiCurrencyAmount pvLongPayerRhoBumped = METHOD_CAP_X.presentValue(CAP_HIGH_LONG, sabrBundleRhoBumped);
    final double expectedRhoSensi = (pvLongPayerRhoBumped.getAmount(EUR).getAmount() - pv.getAmount(EUR).getAmount()) / shift;
    assertEquals("Number of rho sensitivity", pvsCapLong.getRho().getMap().keySet().size(), 1);
    assertEquals("Rho sensitivity expiry/tenor", pvsCapLong.getRho().getMap().keySet().contains(expectedExpiryTenor), true);
    ...
    final SABRInterestRateParameters sabrParameterNuBumped = SABRDataSets.createSABR1NuBumped();
    final SABRCapProviderDiscount sabrBundleNuBumped = new SABRCapProviderDiscount(MULTICURVES, sabrParameterNuBumped, EURIBOR3M);
    final MultiCurrencyAmount pvLongPayerNuBumped = METHOD_CAP_X.presentValue(CAP_HIGH_LONG, sabrBundleNuBumped);
    final double expectedNuSensi = (pvLongPayerNuBumped.getAmount(EUR).getAmount() - pv.getAmount(EUR).getAmount()) / shift;
    assertEquals("Number of nu sensitivity", pvsCapLong.getNu().getMap().keySet().size(), 1);
    assertEquals("Nu sensitivity expiry/tenor", pvsCapLong.getNu().getMap().keySet().contains(expectedExpiryTenor), true);
