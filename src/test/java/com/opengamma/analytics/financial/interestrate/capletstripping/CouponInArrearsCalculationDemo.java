...
public class CouponInArrearsCalculationDemo {
  ...
  public void consistencyCapCouponTest() {
    ...
    double pv = cal.presentValue(CAPLET_LONG, sabrExtrap).getAmount(USD).getAmount();
    assertTrue(stdPv < pv);

    //IBOR
    SmileInterpolatorSABRWithExtrapolation sabrExtrap1 = new SmileInterpolatorSABRWithExtrapolation(provider);
    double pvIbor = cal.presentValue(COUPON_IBOR, sabrExtrap1).getAmount(USD).getAmount();
    ...
    CouponIborInArrearsSmileModelReplicationMethod inArrearsCal = new CouponIborInArrearsSmileModelReplicationMethod(
        smileFunction);
    double pvIborRe = inArrearsCal.presentValue(COUPON_IBOR, CURVES).getAmount(USD).getAmount();
  ...
  public void consistencySwapTest() {
    ...
    MultiCurrencyAmount firstPV = SWAP_FIXED_1M.getFirstLeg().accept(pvdc, MULTICURVE_OIS);
    MultiCurrencyAmount secondPV = cal.presentValue((CouponIbor) SWAP_FIXED_1M.getSecondLeg().getNthPayment(0),
        sabrExtrapQuiet);
    ...
    MultiCurrencyAmount ref = firstPV.plus(secondPV);

    SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet1 = new SmileInterpolatorSABRWithExtrapolation(provider);
    MultiCurrencyAmount res1 = cal.presentValue(SWAP_FIXED_1M, sabrExtrapQuiet1);
    ...
    MultiCurrencyAmount res2 = cal.presentValue(swap, sabrExtrapQuiet2);

    assertEquals(ref.getAmount(USD).getAmount(), res1.getAmount(USD).getAmount(), Math.abs(ref.getAmount(USD).getAmount()) * 1.0e-12);
    assertEquals(ref.getAmount(USD).getAmount(), res2.getAmount(USD).getAmount(), Math.abs(ref.getAmount(USD).getAmount()) * 1.0e-12);
    ...
    double firstPVMod = firstPV.getAmount(USD).getAmount() * parRate / 0.0125;
    assertTrue(Math.abs((secondPV.getAmount(USD).getAmount() + firstPVMod)) < Math.abs(ref.getAmount(USD).getAmount()) * 1.e-10);
  ...
  public void SwapFixed1MExampleTest() {
    ...
    for (int i = 0; i < 11; i++) {
      ...
      BenaimDodgsonKainthExtrapolationFunctionProvider provider = new BenaimDodgsonKainthExtrapolationFunctionProvider(
          mu, mu);
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet = new SmileInterpolatorSABRWithExtrapolation(provider);
      MultiCurrencyAmount pv = cal.presentValue(SWAP_FIXED_1M, sabrExtrapQuiet);
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet1 = new SmileInterpolatorSABRWithExtrapolation(provider);
      Double parRate = cal.parRate(SWAP_FIXED_1M, sabrExtrapQuiet1);
      System.out.println("mu: " + mu + ", pv: " + pv + ", parRate: " + parRate);
      assertEquals(pvExp[pos], pv.getAmount(USD).getAmount(), Math.abs(pvExp[pos]) * 1.0e-10);
      pos++;
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet2 = new SmileInterpolatorSABRWithExtrapolation(provider);
    ...
    for (int i = 0; i < 11; i++) {
      ...
      BenaimDodgsonKainthExtrapolationFunctionProvider provider = new BenaimDodgsonKainthExtrapolationFunctionProvider(
          mu, mu);
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet = new SmileInterpolatorSABRWithExtrapolation(provider);
      MultiCurrencyAmount pv = calLarge.presentValue(SWAP_FIXED_1M, sabrExtrapQuiet);
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet1 = new SmileInterpolatorSABRWithExtrapolation(provider);
  ...
  public void SwapFixed3MFFSExampleTest() {
    ...
    for (int i = 0; i < 11; i++) {
      ...
      BenaimDodgsonKainthExtrapolationFunctionProvider provider = new BenaimDodgsonKainthExtrapolationFunctionProvider(
          mu, mu);
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet = new SmileInterpolatorSABRWithExtrapolation(provider);
      MultiCurrencyAmount pv = cal.presentValue(SWAP_FIXED_3M, sabrExtrapQuiet);
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet1 = new SmileInterpolatorSABRWithExtrapolation(provider);
    ...
    for (int i = 0; i < 11; i++) {
      ...
      BenaimDodgsonKainthExtrapolationFunctionProvider provider = new BenaimDodgsonKainthExtrapolationFunctionProvider(
          mu, mu);
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet = new SmileInterpolatorSABRWithExtrapolation(provider);
      MultiCurrencyAmount pv = calLarge.presentValue(SWAP_FIXED_3M, sabrExtrapQuiet);
      SmileInterpolatorSABRWithExtrapolation sabrExtrapQuiet1 = new SmileInterpolatorSABRWithExtrapolation(provider);
  ...
  public void couponIborDirectAllTest() {
    ...
    for (int i = 0; i < 11; i++) {
      ...
      SmileInterpolatorSABRWithExtrapolation sabrExtrap = new SmileInterpolatorSABRWithExtrapolation(provider);
      double pv = cal.presentValue(couponIbor, sabrExtrap).getAmount(USD).getAmount();
      System.out.println("mu: " + mu + ", pv: " + pv);
    }
    System.out.println();
    System.out.println("<<lambda = 1.0>>");
    for (int i = 0; i < 11; i++) {
      ...
      provider = new BenaimDodgsonKainthExtrapolationFunctionProvider(mu, mu);
      SmileInterpolatorSABRWithExtrapolation sabrExtrap = new SmileInterpolatorSABRWithExtrapolation(provider);
      double pv = calLarge.presentValue(couponIbor, sabrExtrap).getAmount(USD).getAmount();
      System.out.println("mu: " + mu + ", pv: " + pv);
    }
    System.out.println();
    ...
    double pv = cal.presentValue(couponIbor, sabrExtrapQuiet).getAmount(USD).getAmount();
    System.out.println("pv(Quiet): " + pv);
    ...
    pv = cal.presentValue(couponIbor, sabrExtrapFlat).getAmount(USD).getAmount();
    System.out.println("pv(Flat): " + pv);
    System.out.println();
    System.out.println("<<lambda = 1.0>>");
    pv = calLarge.presentValue(couponIbor, sabrExtrapQuiet).getAmount(USD).getAmount();
    System.out.println("pv(Quiet): " + pv);
    pv = calLarge.presentValue(couponIbor, sabrExtrapFlat).getAmount(USD).getAmount();
    System.out.println("pv(Flat): " + pv);
    System.out.println();
  ...
  public void couponIborSABRParameterAllTest() {
    ...
    provider = new ShiftedLogNormalExtrapolationFunctionProvider("Flat");
    sabrExtrap = new SmileInterpolatorSABRWithExtrapolation(provider);
    pv = cal.presentValue(couponIbor, sabrExtrap).getAmount(USD).getAmount();
    System.out.println("pv(Flat): " + pv);
    System.out.println();
  }
}
