...
public class SwapCleanDiscountingCalculatorTest {
      ...
      for (MulticurveProviderDiscount multicurve : MULTICURVE_SET) {
        ...
        for (int i = 0; i < n; ++i) {
          ...
          double accruedInterestPar = CPRC.accruedInterest(swapWithParRateDfn, USD6MLIBOR3M.getFixedLegDayCount(),
              ...
              multicurve).getAmount(Currency.USD).getAmount();
          double dirtyPV = swapWithParRate.accept(PVDC, multicurve).getAmount(Currency.USD).getAmount();
          ...
          accruedInterest[i] = CPRC.accruedInterest(swapDefinition, USD6MLIBOR3M.getFixedLegDayCount(), USD6MLIBOR3M
              .getIborIndex().getDayCount(), USD6MLIBOR3M.getCalendar(), valuationDate, TS_USDLIBOR3M, multicurve)
              .getAmount(Currency.USD).getAmount();
        }
  ...
  public void paidAfterPeriodEndTest() {
    ...
    for (int i = 0; i < nPaymentsIbor; ++i) {
      double paymentYearFraction = DayCountUtils.yearFraction(dcIbor, accStartDatesIbor[i], accEndDatesIbor[i], NYC);
    ...
    for (int i = 0; i < nPaymentsFixed; ++i) {
      double paymentYearFraction = DayCountUtils.yearFraction(dcFixed, accStartDatesFixed[i], accEndDatesFixed[i], calendar);
    ...
    double accruedInterest = CPRC.accruedInterest(swapDefinition, dcFixed, dcIbor, calendar, valuationDate,
        timeSeries, multicurve).getAmount(usd).getAmount();

    /* Compute accrued amounts manually */
    int pIbor = 1;
    double accYFIbor = DayCountUtils.yearFraction(dcIbor, accStartDatesIbor[pIbor], valuationDate, calendar);
    ...
    ++pIbor;
    accYFIbor = DayCountUtils.yearFraction(dcIbor, accStartDatesIbor[pIbor], valuationDate, calendar);
    ...
    int pFixed = 0;
    double accYFFixed = DayCountUtils.yearFraction(dcFixed, accStartDatesFixed[pFixed], valuationDate, calendar);
    double notionalFixed = fixed[pFixed].getNotional();
    double accFixed = accYFFixed * notionalFixed;
    ++pFixed;
    accYFFixed = DayCountUtils.yearFraction(dcFixed, accStartDatesFixed[pFixed], valuationDate, calendar);
    ...
    double pvIborLeg = iborLeg.toDerivative(valuationDate, timeSeries).accept(PVDC, multicurve).getAmount(usd).getAmount();
  ...
  public void paymentDateTest() {
    ...
    for (SwapFixedIborDefinition swapDefinition : new SwapFixedIborDefinition[] {SWAP_FIXED_3M_PAY_DEFINITION,
      ...
      double parRate = CPRC.parRate(swapDefinition, dcFixed, dcIbor, calendar, valuationDate, TS_USDLIBOR3M,
          multicurve);
      double accruedInterest = CPRC.accruedInterest(swapDefinition, dcFixed, dcIbor, calendar, valuationDate,
          TS_USDLIBOR3M, multicurve).getAmount(Currency.USD).getAmount();
      ...
      double pvFirstFixed = fixedCouponSwap.getFirstLeg().getNthPayment(0).accept(PVDC, multicurve)
          .getAmount(Currency.USD).getAmount();
      double pvLibor = fixedCouponSwap.getSecondLeg().accept(PVDC, multicurve).getAmount(Currency.USD).getAmount();
      double pvFixed = fixedCouponSwap.getFirstLeg().accept(PVDC, multicurve).getAmount(Currency.USD).getAmount();
