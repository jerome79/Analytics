...
public class SwapFuturesPriceDeliverableTransactionDiscountingMethodE2ETest {
  ...
  public void EURTest() {
    ...
    MultiCurrencyAmount pv = TRANSACTION_EUR.accept(PVC, MULTI_CURVE_EUR);
    ...
    sensitivityM.put(Pair.of(MULTI_CURVE_EUR.getName(INDEX_EUR), EUR), new DoubleMatrix1D(fwd));
    sensitivityM.put(Pair.of(MULTI_CURVE_EUR.getName(EUR), EUR), new DoubleMatrix1D(dsc));
    ...
    assertRelative("EURTest", 1738.6305164144287, pv.getAmount(EUR).getAmount(), TOL);
    assertRelative("EURTest", -49.62595585858386,
        pv01.getMap().get(Pair.of(MULTI_CURVE_EUR.getName(INDEX_EUR), EUR)), TOL);
    assertRelative("EURTest", -0.21960838158859544,
        pv01.getMap().get(Pair.of(MULTI_CURVE_EUR.getName(EUR), EUR)), TOL);
  ...
  public void GBPTest() {
    ...
    MultiCurrencyAmount pv = TRANSACTION_GBP.accept(PVC, MULTI_CURVE_GBP);
    ...
    LinkedHashMap<Pair<String, Currency>, DoubleMatrix1D> sensitivityM = new LinkedHashMap<>();
    sensitivityM.put(Pair.of(MULTI_CURVE_GBP.getName(INDEX_GBP), GBP), new DoubleMatrix1D(fwd));
    sensitivityM.put(Pair.of(MULTI_CURVE_GBP.getName(GBP), GBP), new DoubleMatrix1D(dsc));
    MultipleCurrencyParameterSensitivity bucketedPv01Exp = new MultipleCurrencyParameterSensitivity(sensitivityM);

    assertRelative("GBPTest", 100.52514560825863, price, TOL);
    assertRelative("GBPTest", 525.1456082586228, pv.getAmount(GBP).getAmount(), TOL);
    assertRelative("GBPTest", -91.14076218376711,
        pv01.getMap().get(Pair.of(MULTI_CURVE_GBP.getName(INDEX_GBP), GBP)), TOL);
    assertRelative("GBPTest", 2.337054652895098,
        pv01.getMap().get(Pair.of(MULTI_CURVE_GBP.getName(GBP), GBP)), TOL);
  ...
  public void USDTest() {
    ...
    MultiCurrencyAmount pv = TRANSACTION_USD.accept(PVC, MULTI_CURVE_USD);
    ...
    LinkedHashMap<Pair<String, Currency>, DoubleMatrix1D> sensitivityM = new LinkedHashMap<>();
    sensitivityM.put(Pair.of(MULTI_CURVE_USD.getName(INDEX_USD), USD), new DoubleMatrix1D(fwd));
    sensitivityM.put(Pair.of(MULTI_CURVE_USD.getName(USD), USD), new DoubleMatrix1D(dsc));
    MultipleCurrencyParameterSensitivity bucketedPv01Exp = new MultipleCurrencyParameterSensitivity(sensitivityM);

    assertRelative("USDTest", 99.91455744642417, price, TOL);
    assertRelative("USDTest", -85.44255357583461, pv.getAmount(USD).getAmount(), TOL);
    assertRelative("USDTest", -194.45550972289809,
        pv01.getMap().get(Pair.of(MULTI_CURVE_USD.getName(INDEX_USD), USD)), TOL);
    assertRelative("USDTest", 11.21923705469224,
        pv01.getMap().get(Pair.of(MULTI_CURVE_USD.getName(USD), USD)), TOL);
