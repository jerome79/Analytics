...
public class CouponFixedAccruedCompoundingDiscountingMethodTest {
  ...
  public void presentValue() {
    final MultiCurrencyAmount pvRecComputed = METHOD.presentValue(CPN_REC, MULTICURVES);
    final double pvExpected = CPN_REC.getAmount() * MULTICURVES.getDiscountFactor(CPN_REC.getCurrency(), CPN_REC.getPaymentTime());
    assertEquals("CouponFixedAccruedCompounding: Present value by discounting", pvExpected, pvRecComputed.getAmount(CPN_REC.getCurrency()).getAmount(), TOLERANCE_PV);
    final double pvPayExpected = CPN_PAY.getAmount() * MULTICURVES.getDiscountFactor(CPN_PAY.getCurrency(), CPN_PAY.getPaymentTime());
    assertEquals("CouponFixedAccruedCompounding: Present value by discounting", pvPayExpected, -pvRecComputed.getAmount(CPN_REC.getCurrency()).getAmount(), TOLERANCE_PV);
    final CurrencyAmount pvPosPayComputed = METHOD.presentValuePositiveNotional(CPN_REC, MULTICURVES);
    assertEquals("CouponFixedAccruedCompounding: Present value by discounting", pvRecComputed.getAmount(CPN_REC.getCurrency()).getAmount(), pvPosPayComputed.getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueWithAccrualDates() {
    final MultiCurrencyAmount pvRecComputed = METHOD.presentValue(CPN_REC_WITH_ACCRUAL_DATES, MULTICURVES);
    ...
    assertEquals("CouponFixedAccruedCompounding: Present value by discounting", pvExpected, pvRecComputed.getAmount(CPN_REC_WITH_ACCRUAL_DATES.getCurrency()).getAmount(), TOLERANCE_PV);
