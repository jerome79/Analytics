...
public class BillTransactionDiscountingMethodE2ETest {
  ...
  static {
    ISSUER_SPECIFIC.put(Pair.of((Object) USGOVT_NAME, SHORT_NAME_FILTER), MULTICURVE.getCurve(USD));
  }
  ...
  public void presentValueOIS() {
    final MultiCurrencyAmount pvComputed = B140814_TRA_1.accept(PVIC, ISSUER_MULTICURVE);
    final MultiCurrencyAmount pvExpected = MultiCurrencyAmount.of(USD, -1368.887395d);
    assertEquals("BillTransactionDiscountingMethodE2E: discounting method - present value", 1, pvComputed.size());
    assertEquals("BillTransactionDiscountingMethodE2E: discounting method - present value", pvExpected.getAmount(USD).getAmount(), pvComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
