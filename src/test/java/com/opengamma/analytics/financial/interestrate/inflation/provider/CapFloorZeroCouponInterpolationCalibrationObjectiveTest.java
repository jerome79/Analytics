...
public class CapFloorZeroCouponInterpolationCalibrationObjectiveTest {
  ...
  private static final ZonedDateTime LAST_KNOWN_FIXING_DATE = DateUtils.getUTCDate(2008, 7, 1);
  ...
  public void calibration() {
    ...
    for (int loop1 = 0; loop1 < STRIKES.length; loop1++) {
      for (int loop2 = 0; loop2 < availabelTenor.length; loop2++) {
        ...
        assertEquals("Inflaiton year on year calibration: cap/floor " + loop1, pvCapYearOnYear[loop1][loop2].getAmount(CUR).getAmount(), marketPrices[loop1][loop2], 1E-2);
      }
    }
  }
