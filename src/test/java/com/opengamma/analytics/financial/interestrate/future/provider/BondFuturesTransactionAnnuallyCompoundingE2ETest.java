...
public class BondFuturesTransactionAnnuallyCompoundingE2ETest {
  ...
  static {
    ...
    ISSUER_PROVIDER_LGT.setCurve(Pair.of((Object) ISSUER_NAME_LGT, filter), yieldCurve);
  ...
  static {
    ...
    YieldPeriodicCurve yieldCurve = YieldPeriodicCurve.from(NUM_PERIODS, interpolatedCurve);
    LegalEntityFilter<LegalEntity> filter = new LegalEntityShortName();
    ISSUER_PROVIDER_SCH.setCurve(Pair.of((Object) ISSUER_NAME_SCH, filter), yieldCurve);
  ...
  static {
    ...
    YieldPeriodicCurve yieldCurve = YieldPeriodicCurve.from(NUM_PERIODS, interpolatedCurve);
    LegalEntityFilter<LegalEntity> filter = new LegalEntityShortName();
    ISSUER_PROVIDER_BUN.setCurve(Pair.of((Object) ISSUER_NAME_BUN, filter), yieldCurve);
  ...
  static {
    ...
    YieldPeriodicCurve yieldCurve = YieldPeriodicCurve.from(NUM_PERIODS, interpolatedCurve);
    LegalEntityFilter<LegalEntity> filter = new LegalEntityShortName();
    ISSUER_PROVIDER_BOB.setCurve(Pair.of((Object) ISSUER_NAME_BOB, filter), yieldCurve);
  ...
  public void LGTTest() {
    ...
    MultiCurrencyAmount pvTransactionComputed = TRANSACTION_LGT.accept(PVIC, curveWithSpread);
    ...
    double pv01TransactionComputed = TRANSACTION_LGT.accept(PV01PC, curveWithSpread).getMap()
        .get(Pair.of(CURVE_NAME_LGT, GBP));
    ...
    assertRelative("LGT", 1097.8905882774707, pvTransactionComputed.getAmount(GBP).getAmount(), TOL);
  ...
  public void SCHTest() {
    ...
    double priceFuturesComputed = futures.accept(FPIC, curveWithSpread) * HUNDRED;
    MultiCurrencyAmount pvTransactionComputed = TRANSACTION_SCH.accept(PVIC, curveWithSpread);
    ...
    double pv01TransactionComputed = TRANSACTION_SCH.accept(PV01PC, curveWithSpread).getMap()
        .get(Pair.of(CURVE_NAME_SCH, EUR));
    ...
    assertRelative("SCH", 1106.6399796476674, pvTransactionComputed.getAmount(EUR).getAmount(), TOL);
  ...
  public void BUNTest() {
    ...
    double priceFuturesComputed = futures.accept(FPIC, curveWithSpread) * HUNDRED;
    MultiCurrencyAmount pvTransactionComputed = TRANSACTION_BUN.accept(PVIC, curveWithSpread);
    ...
    double pv01TransactionComputed = TRANSACTION_BUN.accept(PV01PC, curveWithSpread).getMap()
        .get(Pair.of(CURVE_NAME_BUN, EUR));
    ...
    assertRelative("BUN", 1405.760079833893, pvTransactionComputed.getAmount(EUR).getAmount(), TOL);
  ...
  public void BOBTest() {
    ...
    double priceFuturesComputed = futures.accept(FPIC, curveWithSpread) * HUNDRED;
    MultiCurrencyAmount pvTransactionComputed = TRANSACTION_BOB.accept(PVIC, curveWithSpread);
    ...
    double pv01TransactionComputed = TRANSACTION_BOB.accept(PV01PC, curveWithSpread).getMap()
        .get(Pair.of(CURVE_NAME_BOB, EUR));
    ...
    assertRelative("BOB", 1250.1222173864887, pvTransactionComputed.getAmount(EUR).getAmount(), TOL);
  ...
  public void calculatorTest() {
    ...
    MultiCurrencyAmount pv = BOND_METHOD.presentValue(bondAtSpot, curveWithSpread);
    double cleanPriceFromPV = HUNDRED * (pv.getAmount(EUR).getAmount() / curveWithSpread.getMulticurveProvider()
    ...
    MultiCurrencyAmount pvComputed = TRANSACTION_SCH.accept(PVIC, curveWithSpread);
    double pvExpected = bondAtDeliv.accept(BCPCC, curveWithSpread) * futures.getNotional() / HUNDRED /
        futures.getConversionFactor()[0];
    assertRelative("calculatorTest", pvExpected, pvComputed.getAmount(EUR).getAmount(), TOL);
  }
