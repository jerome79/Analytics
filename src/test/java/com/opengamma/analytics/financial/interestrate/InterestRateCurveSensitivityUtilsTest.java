...
public class InterestRateCurveSensitivityUtilsTest {
  ...
  public void testCleanList1() {
    ...
    final List<DoublesPair> res = InterestRateCurveSensitivityUtils.clean(old, 1e-9, 1e-12);
    assertEquals(old.size(), res.size());

    for (int i = 0; i < old.size(); i++) {
      assertEquals(old.get(i).getFirst(), res.get(i).getFirst(), 0.0);
      assertEquals(old.get(i).getSecond(), res.get(i).getSecond(), 1e-9);
    }
  }
  ...
  public void testCleanList2() {
    ...
    for (int i = 0; i < expected.size(); i++) {
      assertEquals(expected.get(i).getFirst(), res.get(i).getFirst(), 0.0);
      assertEquals(expected.get(i).getSecond(), res.get(i).getSecond(), 1e-9);
    }
  }
  ...
  public void testMultiplySensitivityList() {
    ...
    for (int i = 0; i < res.size(); i++) {
      assertEquals(l1.get(i).getFirst(), res.get(i).getFirst(), 0.0);
      assertEquals(2.0 * l1.get(i).getSecond(), res.get(i).getSecond(), 1e-9);
    }
  }
  ...
  private static void assertSensitivityEquals(final List<DoublesPair> expected, final List<DoublesPair> accual, final double tol) {
    ...
    for (int i = 0; i < expected.size(); i++) {
      assertEquals(expected.get(i).getFirst(), accual.get(i).getFirst(), 0.0);
      assertEquals(expected.get(i).getSecond(), accual.get(i).getSecond(), tol);
    }
  }
