...
public class ParameterizedVolatilitySurfaceProviderTest {
  ...
  static {
    SURFACE = new ParameterizedSurface() {
      ...
      public Double evaluate(final DoublesPair tk, final DoubleMatrix1D parameters) {
        ...
        final double t = tk.getFirst();
        final double k = tk.getSecond();
  ...
  public void test() {
    ...
    for (int i = 0; i < nSamples; i++) {
      ...
      final DoubleMatrix1D s2 = senseSurf.getZValue(point.toPair());
    ...
    for (int i = 0; i < nSamples; i++) {
      ...
      final DoubleMatrix1D sense = senseSurf.getZValue(point.toPair());
      final Pair<Double, DoubleMatrix1D> volAndSense = vsSurf.getZValue(point.toPair());
