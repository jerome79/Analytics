...
public class TodayPaymentCalculatorTest {
  ...
  public void todayPaymentCalculatorOnDayOfPayment() {
    ...
    final MultiCurrencyAmount paymentToday = swapToday.accept(paymentCalculator);

    assertEquals(todayCash, paymentToday.getAmount(USD6MLIBOR3M.getCurrency()).getAmount(), TOLERANCE_PV);
  }
  ...
  public void tpcLookingBackwardAndForwardOnDayOfPayment() {
    ...
    final MultiCurrencyAmount paymentIfLookingForward = swapToday.accept(forwardCalculator);
    ...
    final MultiCurrencyAmount paymentIfLookingBackward = swapToday.accept(backwardCalculator);

    assertEquals(paymentIfLookingForward.getAmount(USD6MLIBOR3M.getCurrency()).getAmount(), paymentIfLookingBackward.getAmount(USD6MLIBOR3M.getCurrency()).getAmount(),
        TOLERANCE_PV);
  }
  ...
  public void tpcWontProvidePaymentFromOneWeekBack() {
    ...
    final TodayPaymentCalculator paymentCalculator = TodayPaymentCalculator.getInstance(horizon);
    final MultiCurrencyAmount paymentToday = swapToday.accept(paymentCalculator);
    assertEquals(0.0, paymentToday.getAmount(USD6MLIBOR3M.getCurrency()).getAmount(), TOLERANCE_PV);
  }
}
