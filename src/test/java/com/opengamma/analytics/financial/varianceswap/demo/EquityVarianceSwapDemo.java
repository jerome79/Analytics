...
public class EquityVarianceSwapDemo {
  ...
  public void buildSwap() {
    ...
    LocalDateDoubleTimeSeries ts = buildTimeSeries(dates, prices);
  ...
  private LocalDateDoubleTimeSeries buildTimeSeries(LocalDate[] dates, double[] prices) {
    return IntStream.range(0, dates.length)
        .mapToObj(i -> LocalDateDoublePoint.of(dates[i], prices[i]))
