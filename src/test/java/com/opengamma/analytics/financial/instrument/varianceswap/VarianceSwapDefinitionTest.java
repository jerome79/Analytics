...
public class VarianceSwapDefinitionTest extends AnalyticsTestBase {
  ...
  public void testForwardStarting() {
    final VarianceSwap varianceSwap = DEFINITION.toDerivative(NOW, LocalDateDoubleTimeSeries.empty());
  ...
  public void testSeasoned() {
    ...
    final LocalDateDoubleTimeSeries ts = IntStream.range(0, 365)
        .mapToObj(i -> LocalDateDoublePoint.of(dates[i], vars[i]))
