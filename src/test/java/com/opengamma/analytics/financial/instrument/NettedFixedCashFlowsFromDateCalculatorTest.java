...
public class NettedFixedCashFlowsFromDateCalculatorTest {
  ...
  private static final InstrumentDefinitionVisitor<LocalDateDoubleTimeSeries, Map<LocalDate, MultiCurrencyAmount>> ALL_CASH_FLOWS = NettedFixedCashFlowVisitor.getVisitor();
  ...
  public void testDateBeforeAnyCashFlows() {
    ...
    for (final InstrumentDefinition<?> definition : INSTRUMENTS) {
      final TreeMap<LocalDate, MultiCurrencyAmount> allFlows = new TreeMap<>(definition.accept(ALL_CASH_FLOWS, IBOR_FIXING_SERIES));
      final Map<LocalDate, MultiCurrencyAmount> trimmed = CALCULATOR.getCashFlows(definition, IBOR_FIXING_SERIES, date);
      assertEquals(allFlows, trimmed);
    }
    for (final InstrumentDefinition<?> definition : NON_FIXING_INSTRUMENTS) {
      final TreeMap<LocalDate, MultiCurrencyAmount> allFlows = new TreeMap<>(definition.accept(ALL_CASH_FLOWS));
      final Map<LocalDate, MultiCurrencyAmount> trimmed = CALCULATOR.getCashFlows(definition, date);
      assertEquals(allFlows, trimmed);
    }
  }
  ...
  public void testTrimming() {
    ...
    for (final InstrumentDefinition<?> definition : allInstruments) {
      final TreeMap<LocalDate, MultiCurrencyAmount> allFlows = new TreeMap<>(definition.accept(ALL_CASH_FLOWS, IBOR_FIXING_SERIES));
      final Map<LocalDate, MultiCurrencyAmount> trimmed = CALCULATOR.getCashFlows(definition, IBOR_FIXING_SERIES, date);
      final Iterator<Map.Entry<LocalDate, MultiCurrencyAmount>> iterator = trimmed.entrySet().iterator();
      for (final Map.Entry<LocalDate, MultiCurrencyAmount> entry : allFlows.entrySet()) {
        if (entry.getKey().isBefore(date)) {
          assertFalse(trimmed.containsKey(entry.getKey()));
        } else {
          final Map.Entry<LocalDate, MultiCurrencyAmount> other = iterator.next();
