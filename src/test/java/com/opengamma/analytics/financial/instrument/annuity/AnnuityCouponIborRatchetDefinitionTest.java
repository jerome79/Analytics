...
public class AnnuityCouponIborRatchetDefinitionTest {
  ...
  public void constructorFixed() {
    ...
    for (int loopcpn = 1; loopcpn < NB_COUPON; loopcpn++) {
      cpn[loopcpn] = new CouponIborRatchetDefinition(CUR, paymentDates[loopcpn], paymentDates[loopcpn - 1], paymentDates[loopcpn], DayCountUtils.yearFraction(DAY_COUNT, paymentDates[loopcpn - 1],
  ...
  public void constructorIborGearing() {
    final ZonedDateTime[] paymentDates = ScheduleCalculator.getAdjustedDateSchedule(SETTLEMENT_DATE, ANNUITY_TENOR, INDEX_TENOR, BUSINESS_DAY, CALENDAR, IS_EOM);
    final CouponDefinition[] cpn = new CouponDefinition[NB_COUPON];
    cpn[0] = CouponIborGearingDefinition.from(SETTLEMENT_DATE, paymentDates[0], DayCountUtils.yearFraction(DAY_COUNT, SETTLEMENT_DATE, paymentDates[0]), NOTIONAL, IBOR_INDEX, MAIN_COEF[2],
        MAIN_COEF[1],
        CALENDAR);
    for (int loopcpn = 1; loopcpn < NB_COUPON; loopcpn++) {
      cpn[loopcpn] = new CouponIborRatchetDefinition(CUR, paymentDates[loopcpn], paymentDates[loopcpn - 1], paymentDates[loopcpn], DayCountUtils.yearFraction(DAY_COUNT, paymentDates[loopcpn - 1],
