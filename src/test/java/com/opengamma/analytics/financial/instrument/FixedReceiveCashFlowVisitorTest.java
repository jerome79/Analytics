...
public class FixedReceiveCashFlowVisitorTest {
  ...
  public void testMissingFixingData() {
    ...
    final LocalDateDoubleTimeSeries fixingSeries = LocalDateDoubleTimeSeries.builder().putAll(dates, fixings).build();
  ...
  public void testCash() {
    final Map<LocalDate, MultiCurrencyAmount> payment = RECEIVE_CASH.accept(VISITOR);
  ...
  public void testFixedPayment() {
    final Map<LocalDate, MultiCurrencyAmount> payment = RECEIVE_FIXED_PAYMENT.accept(VISITOR);
    assertEquals(1, payment.size());
  ...
  public void testFixedCoupon() {
    final Map<LocalDate, MultiCurrencyAmount> payment = RECEIVE_FIXED_COUPON.accept(VISITOR);
    assertEquals(1, payment.size());
  ...
  public void testIborCoupon() {
    Map<LocalDate, MultiCurrencyAmount> payment = RECEIVE_IBOR_COUPON.accept(VISITOR, IBOR_FIXING_SERIES);
    assertEquals(1, payment.size());
  ...
  public void testPayerFRA() {
    final Map<LocalDate, MultiCurrencyAmount> payment = PAYER_FRA.accept(VISITOR, IBOR_FIXING_SERIES);
    assertEquals(1, payment.size());
  ...
  public void testReceiverFRA() {
    final Map<LocalDate, MultiCurrencyAmount> payment = RECEIVER_FRA.accept(VISITOR);
    assertEquals(1, payment.size());
    assertEquals(FRA_START.toLocalDate(), Iterables.getOnlyElement(payment.keySet()));
    final MultiCurrencyAmount mca = Iterables.getOnlyElement(payment.values());
    assertEquals(1, mca.size());
  ...
  public void testPayerSwap() {
    Map<LocalDate, MultiCurrencyAmount> payments = new TreeMap<>(PAYER_SWAP.accept(VISITOR, IBOR_FIXING_SERIES));
  ...
  public void testReceiverSwap() {
    Map<LocalDate, MultiCurrencyAmount> payments = new TreeMap<>(RECEIVER_SWAP.accept(VISITOR));
  ...
  @Test
  public void testNDF() {
    Map<LocalDate, MultiCurrencyAmount> payment = SHORT_NDF.accept(VISITOR);
