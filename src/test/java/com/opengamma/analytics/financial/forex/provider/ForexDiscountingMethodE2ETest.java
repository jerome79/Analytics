...
//  private static final double TOLERANCE_RATE_DELTA = 1.0E-8;
  ...
  public void presentValueEUROISUSDOIS() {
    final MultiCurrencyAmount pvComputed = FX.accept(PVDC, MULTICURVE_OIS);
    final double pvEURExpected = 9982954.0610;
    final double pvUSDExpected = -13984806.7328;
    assertEquals("ForexDiscountingMethod: presentValue - standard data set", pvComputed.getAmount(EUR).getAmount(), pvEURExpected, TOLERANCE_PV);
    assertEquals("ForexDiscountingMethod: presentValue - standard data set", pvComputed.getAmount(USD).getAmount(), pvUSDExpected, TOLERANCE_PV);
  }
...
//    final MultiCurrencyAmount pvComputed = FX.accept(PVDC, MULTICURVES_EURFX);
...
//  }
  ...
  public void currencyExposureEUROISUSDOIS() {
    final MultiCurrencyAmount ceComputed = FX.accept(CEDC, MULTICURVE_OIS);
    final double ceEURExpected = 9982954.0610;
    final double ceUSDExpected = -13984806.7328;
    assertEquals("ForexDiscountingMethod: presentValue - standard data set", ceComputed.getAmount(EUR).getAmount(), ceEURExpected, TOLERANCE_PV);
    assertEquals("ForexDiscountingMethod: presentValue - standard data set", ceComputed.getAmount(USD).getAmount(), ceUSDExpected, TOLERANCE_PV);
  }
...
//  }
  ...
  public void BucketedPV01() {
    ...
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(EUR), EUR), new DoubleMatrix1D(deltaDscEUR));
    sensitivity.put(Pair.of(MULTICURVE_OIS.getName(USD), USD), new DoubleMatrix1D(deltaDscUSD));
