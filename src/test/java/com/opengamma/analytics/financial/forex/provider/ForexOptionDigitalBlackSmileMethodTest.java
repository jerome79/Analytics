...
public class ForexOptionDigitalBlackSmileMethodTest {
  ...
  public void persentValueAtGridPoint() {
    ...
    final double pvExpected = Math.abs(forexOption.getUnderlyingForex().getPaymentCurrency2().getAmount()) * dfDomestic * NORMAL.getCDF(omega * dM)
        * (isLong ? 1.0 : -1.0);
    final MultiCurrencyAmount pvComputed = METHOD_BLACK_DIGITAL.presentValue(forexOption, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: present value", pvExpected, pvComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueDomesticProfile() {
    ...
    for (int loopspot = 0; loopspot <= nbSpot; loopspot++) {
      ...
      final FxMatrix fxMatrix = FxMatrix.builder().addRate(EUR, USD, spot[loopspot]).build();
      ...
      pv[loopspot] = METHOD_BLACK_DIGITAL.presentValue(forexOption, smile).getAmount(USD).getAmount();
    }
  }
  ...
  public void presentValueForeign() {
    ...
    final double pvExpected = Math.abs(forexOption.getUnderlyingForex().getPaymentCurrency1().getAmount()) * dfDomestic * NORMAL.getCDF(omega * dM)
        * (isLong ? 1.0 : -1.0);
    final MultiCurrencyAmount pvComputed = METHOD_BLACK_DIGITAL.presentValue(forexOption, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: present value", pvExpected, pvComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueDomesticForeign() {
    ...
    for (int loopspot = 0; loopspot <= nbSpot; loopspot++) {
      ...
      multicurveForex.setForexMatrix(fxMatrix);
      final BlackForexSmileProviderDiscount smile = new BlackForexSmileProviderDiscount(multicurveForex, SMILE_TERM, Pair.of(EUR, USD));
      pv[loopspot] = METHOD_BLACK_DIGITAL.presentValue(forexOption, smile).getAmount(EUR).getAmount();
    }
  }
  ...
  public void presentValuePutCallParity() {
    ...
    final MultiCurrencyAmount pvCall = METHOD_BLACK_DIGITAL.presentValue(call, SMILE_MULTICURVES);
    final MultiCurrencyAmount pvPut = METHOD_BLACK_DIGITAL.presentValue(put, SMILE_MULTICURVES);
    final MultiCurrencyAmount pvCash = put.getUnderlyingForex().getPaymentCurrency2().accept(PVDC, MULTICURVES);
    assertEquals("Forex Digital option: present value", pvCall.getAmount(USD).getAmount() + pvPut.getAmount(USD).getAmount(), Math.abs(pvCash.getAmount(USD).getAmount()), TOLERANCE_PV);
  }
  ...
  public void presentValueMethodVsCalculator() {
    ...
    final ForexOptionDigitalDefinition forexOptionDefinition = new ForexOptionDigitalDefinition(forexUnderlyingDefinition, expDate, isCall, isLong);
    final ForexOptionDigital forexOption = forexOptionDefinition.toDerivative(REFERENCE_DATE);
    final MultiCurrencyAmount pvMethod = METHOD_BLACK_DIGITAL.presentValue(forexOption, SMILE_MULTICURVES);
    final MultiCurrencyAmount pvCalculator = forexOption.accept(PVFBC, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: present value Method vs Calculator", pvMethod.getAmount(USD).getAmount(), pvCalculator.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueLongShort() {
    ...
    final MultiCurrencyAmount pvShort = METHOD_BLACK_DIGITAL.presentValue(forexOptionShort, SMILE_MULTICURVES);
    final MultiCurrencyAmount pvLong = METHOD_BLACK_DIGITAL.presentValue(FOREX_DIGITAL_CALL_DOM, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: present value long/short parity", pvLong.getAmount(USD).getAmount(), -pvShort.getAmount(USD).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount ceShort = METHOD_BLACK_DIGITAL.currencyExposure(forexOptionShort, SMILE_MULTICURVES);
    final MultiCurrencyAmount ceLong = METHOD_BLACK_DIGITAL.currencyExposure(FOREX_DIGITAL_CALL_DOM, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: currency exposure long/short parity", ceLong.getAmount(USD).getAmount(), -ceShort.getAmount(USD).getAmount(), TOLERANCE_PV);
    assertEquals("Forex Digital option: currency exposure long/short parity", ceLong.getAmount(EUR).getAmount(), -ceShort.getAmount(EUR).getAmount(), TOLERANCE_PV);
  }
  ...
  public void currencyExposure() {
    ...
    final MultiCurrencyAmount priceComputedCall = METHOD_BLACK_DIGITAL.presentValue(forexOptionCall, SMILE_MULTICURVES);
    final MultiCurrencyAmount priceComputedPut = METHOD_BLACK_DIGITAL.presentValue(forexOptionPut, SMILE_MULTICURVES);
    final MultiCurrencyAmount currencyExposureCallComputed = METHOD_BLACK_DIGITAL.currencyExposure(forexOptionCall, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: currency exposure foreign - call", deltaSpotCall, currencyExposureCallComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
    assertEquals("Forex Digital option: currency exposure domestic - call", -deltaSpotCall * SPOT + priceComputedCall.getAmount(USD).getAmount(),
        currencyExposureCallComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount currencyExposurePutComputed = METHOD_BLACK_DIGITAL.currencyExposure(forexOptionPut, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: currency exposure foreign- put", deltaSpotPut, currencyExposurePutComputed.getAmount(EUR).getAmount(), TOLERANCE_PV);
    assertEquals("Forex Digital option: currency exposure domestic - put", -deltaSpotPut * SPOT + priceComputedPut.getAmount(USD).getAmount(),
        currencyExposurePutComputed.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void currencyExposureForeign2() {
    ...
    final MultiCurrencyAmount ce = METHOD_BLACK_DIGITAL.currencyExposure(FOREX_DIGITAL_CALL_FOR, SMILE_FLAT_MULTICURVES);
    final MultiCurrencyAmount pv = METHOD_BLACK_DIGITAL.presentValue(FOREX_DIGITAL_CALL_FOR, SMILE_FLAT_MULTICURVES);
    final MultiCurrencyAmount pvP = METHOD_BLACK_DIGITAL.presentValue(FOREX_DIGITAL_CALL_FOR, smileP);
    assertEquals("Forex Digital option: call spread method - currency exposure - PL EUR",
        pvP.getAmount(EUR).getAmount() - pv.getAmount(EUR).getAmount(),
        ce.getAmount(USD).getAmount() * (1.0 / (SPOT + shift) - 1 / SPOT), TOLERANCE_PV);
    assertEquals("Forex Digital option: call spread method - currency exposure - PL USD",
        ...
        ce.getAmount(EUR).getAmount() * (SPOT + shift - SPOT), TOLERANCE_PV);
  }
  ...
  public void currencyExposureVsPresentValue() {
    final MultiCurrencyAmount pv = METHOD_BLACK_DIGITAL.presentValue(FOREX_DIGITAL_CALL_DOM, SMILE_MULTICURVES);
    final MultiCurrencyAmount ce = METHOD_BLACK_DIGITAL.currencyExposure(FOREX_DIGITAL_CALL_DOM, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: currency exposure vs present value", ce.getAmount(USD).getAmount() + ce.getAmount(EUR).getAmount() * SPOT, pv.getAmount(USD).getAmount(), 1E-2);
  }
  ...
  public void currencyExposurePutCallParity() {
    ...
    final ForexOptionDigital forexOptionCall = forexOptionDefinitionCall.toDerivative(REFERENCE_DATE);
    final ForexOptionDigital forexOptionPut = forexOptionDefinitionPut.toDerivative(REFERENCE_DATE);
    final MultiCurrencyAmount currencyExposureCall = METHOD_BLACK_DIGITAL.currencyExposure(forexOptionCall, SMILE_MULTICURVES);
    final MultiCurrencyAmount currencyExposurePut = METHOD_BLACK_DIGITAL.currencyExposure(forexOptionPut, SMILE_MULTICURVES);
    final MultiCurrencyAmount pvCash = forexOptionPut.getUnderlyingForex().getPaymentCurrency2().accept(PVDC, MULTICURVES);
    assertEquals("Forex Digital option: currency exposure put/call parity foreign", 0, currencyExposureCall.getAmount(EUR).getAmount() + currencyExposurePut.getAmount(EUR).getAmount(),
        TOLERANCE_PV);
    assertEquals("Forex Digital option: currency exposure put/call parity domestic", Math.abs(pvCash.getAmount(USD).getAmount()), currencyExposureCall.getAmount(USD).getAmount()
        + currencyExposurePut.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void currencyExposureMethodVsCalculator() {
    ...
    final ForexOptionDigitalDefinition forexOptionDefinition = new ForexOptionDigitalDefinition(forexUnderlyingDefinition, expDate, isCall, isLong);
    final ForexOptionDigital forexOption = forexOptionDefinition.toDerivative(REFERENCE_DATE);
    final MultiCurrencyAmount ceMethod = METHOD_BLACK_DIGITAL.currencyExposure(forexOption, SMILE_MULTICURVES);
    final MultiCurrencyAmount ceCalculator = forexOption.accept(CEFBC, SMILE_MULTICURVES);
    assertEquals("Forex Digital option: currency exposure Method vs Calculator", ceMethod.getAmount(EUR).getAmount(), ceCalculator.getAmount(EUR).getAmount(), 1E-2);
    assertEquals("Forex Digital option: currency exposure Method vs Calculator", ceMethod.getAmount(USD).getAmount(), ceCalculator.getAmount(USD).getAmount(), 1E-2);
  }
