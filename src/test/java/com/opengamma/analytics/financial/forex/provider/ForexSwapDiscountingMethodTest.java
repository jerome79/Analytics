...
public class ForexSwapDiscountingMethodTest {
  ...
  public void presentValue() {
    final MultiCurrencyAmount pv = METHOD_FX_SWAP.presentValue(FX_SWAP, MULTICURVES);
    final MultiCurrencyAmount pvNear = METHOD_FX.presentValue((FX_SWAP).getNearLeg(), MULTICURVES);
    final MultiCurrencyAmount pvFar = METHOD_FX.presentValue((FX_SWAP).getFarLeg(), MULTICURVES);
    assertEquals(pvNear.getAmount(CUR_1).getAmount() + pvFar.getAmount(CUR_1).getAmount(), pv.getAmount(CUR_1).getAmount());
    assertEquals(pvNear.getAmount(CUR_2).getAmount() + pvFar.getAmount(CUR_2).getAmount(), pv.getAmount(CUR_2).getAmount());
  }
  ...
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = METHOD_FX_SWAP.presentValue(FX_SWAP, MULTICURVES);
    final MultiCurrencyAmount pvCalculator = FX_SWAP.accept(PVDC, MULTICURVES);
  ...
  public void currencyExposure() {
    final MultiCurrencyAmount exposureMethod = METHOD_FX_SWAP.currencyExposure(FX_SWAP, MULTICURVES);
    final MultiCurrencyAmount pv = METHOD_FX_SWAP.presentValue(FX_SWAP, MULTICURVES);
    assertEquals("Currency exposure", pv, exposureMethod);
    final MultiCurrencyAmount exposureCalculator = FX_SWAP.accept(CEDC, MULTICURVES);
  ...
  public void forexTodayPaymentBeforeNearDate() {
    ...
    final MultiCurrencyAmount cash = fx.accept(TPC);
    assertEquals("TodayPaymentCalculator: forex", 0.0, cash.getAmount(FX_SWAP_DEFINITION.getNearLeg().getCurrency1()).getAmount(), TOLERANCE_PV);
    assertEquals("TodayPaymentCalculator: forex", 0.0, cash.getAmount(FX_SWAP_DEFINITION.getNearLeg().getCurrency2()).getAmount(), TOLERANCE_PV);
    assertEquals("TodayPaymentCalculator: forex", 2, cash.size());
  }
  ...
  public void forexTodayPaymentOnNearDate() {
    ...
    assertEquals("TodayPaymentCalculator: forex", FX_SWAP_DEFINITION.getNearLeg().getPaymentCurrency1().getReferenceAmount(),
        cash.getAmount(FX_SWAP_DEFINITION.getNearLeg().getCurrency1()).getAmount(), TOLERANCE_PV);
    assertEquals("TodayPaymentCalculator: forex", FX_SWAP_DEFINITION.getNearLeg().getPaymentCurrency2().getReferenceAmount(),
        cash.getAmount(FX_SWAP_DEFINITION.getNearLeg().getCurrency2()).getAmount(), TOLERANCE_PV);
    assertEquals("TodayPaymentCalculator: forex", 2, cash.size());
  }
  ...
  public void forexTodayPaymentBeforeFarDate() {
    ...
    final MultiCurrencyAmount cash = fx.accept(TPC);
    assertEquals("TodayPaymentCalculator: forex", 0.0, cash.getAmount(FX_SWAP_DEFINITION.getFarLeg().getCurrency1()).getAmount(), TOLERANCE_PV);
    assertEquals("TodayPaymentCalculator: forex", 0.0, cash.getAmount(FX_SWAP_DEFINITION.getFarLeg().getCurrency2()).getAmount(), TOLERANCE_PV);
    assertEquals("TodayPaymentCalculator: forex", 2, cash.size());
  }
  ...
  public void forexTodayPaymentOnFarDate() {
    ...
    assertEquals("TodayPaymentCalculator: forex", FX_SWAP_DEFINITION.getFarLeg().getPaymentCurrency1().getReferenceAmount(),
        cash.getAmount(FX_SWAP_DEFINITION.getFarLeg().getCurrency1()).getAmount(), TOLERANCE_PV);
    assertEquals("TodayPaymentCalculator: forex", FX_SWAP_DEFINITION.getFarLeg().getPaymentCurrency2().getReferenceAmount(),
        cash.getAmount(FX_SWAP_DEFINITION.getFarLeg().getCurrency2()).getAmount(), TOLERANCE_PV);
    assertEquals("TodayPaymentCalculator: forex", 2, cash.size());
  }
  ...
  public void parSpread() {
    ...
    final MultiCurrencyAmount pv0 = METHOD_FX_SWAP.presentValue(fxSwap0, MULTICURVES);
