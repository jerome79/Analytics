...
public class ForexOptionSingleBarrierBlackMethodTest {
  ...
  private static final ParameterSensitivityParameterCalculator<BlackForexSmileProviderInterface> PS_FBS_C =
      new ParameterSensitivityParameterCalculator<>(PVCSFBC);
  private static final ParameterSensitivityForexBlackSmileDiscountInterpolatedFDCalculator PS_FBS_FDC =
      new ParameterSensitivityForexBlackSmileDiscountInterpolatedFDCalculator(PVFBC, SHIFT_FD);
  ...
  public void comparisonOfBarrierPricersWithVeryHighAndVeryLowBarrierLevels() {
    final MultiCurrencyAmount priceBarrierUp = METHOD_BARRIER.presentValue(BARRIER_LIKE_VANILLA_UP, SMILE_MULTICURVES);
    final MultiCurrencyAmount priceBarrierDown = METHOD_BARRIER.presentValue(BARRIER_LIKE_VANILLA_DOWN, SMILE_MULTICURVES);
    assertTrue("PV : Knock-Out Barrier with unreachably HIGH level doesn't match one with unreachably LOWlevel",
        Math.abs(priceBarrierDown.getAmount(USD).getAmount() / priceBarrierUp.getAmount(USD).getAmount() - 1) < 1e-4);
  }
  ...
  public void testKnockInOutParity() {
    ...
    final double priceVanilla = METHOD_VANILLA.presentValue(vanillaExpiryEqualsPay, SMILE_MULTICURVES).getAmount(USD).getAmount();

    final double priceBarrierKO = METHOD_BARRIER.presentValue(knockOut, SMILE_MULTICURVES).getAmount(USD).getAmount();
    final double priceBarrierKI = METHOD_BARRIER.presentValue(knockIn, SMILE_MULTICURVES).getAmount(USD).getAmount();

    // First, check that KO+KI on arbitrary Barrier match the price of the KnockIn on an impossibly low barrier
    final double priceBarrierDown = METHOD_BARRIER.presentValue(impossibleKnockOut, SMILE_MULTICURVES).getAmount(USD).getAmount();
  ...
  public void comparisonVanilla() {
    final MultiCurrencyAmount priceVanilla = METHOD_VANILLA.presentValue(VANILLA_LONG, SMILE_MULTICURVES);
    final MultiCurrencyAmount priceBarrier = METHOD_BARRIER.presentValue(OPTION_BARRIER, SMILE_MULTICURVES);
    assertTrue("Barriers are cheaper than vanilla", priceVanilla.getAmount(USD).getAmount() > priceBarrier.getAmount(USD).getAmount());
  }
  ...
  public void presentValue() {
    final MultiCurrencyAmount priceMethod = METHOD_BARRIER.presentValue(OPTION_BARRIER, SMILE_MULTICURVES);
    ...
    assertEquals("Barriers present value", priceComputed, priceMethod.getAmount(USD).getAmount(), TOLERANCE_PV);
  }
  ...
  public void scaleLongShortParity() {
    ...
    final MultiCurrencyAmount priceBarrierScale = METHOD_BARRIER.presentValue(optionBarrierScale, SMILE_MULTICURVES);
    assertEquals(
        "Barriers are cheaper than vanilla",
        priceBarrier.getAmount(USD).getAmount() * scale,
        priceBarrierScale.getAmount(USD).getAmount(),
        TOLERANCE_PV);
    final MultiCurrencyAmount priceBarrierShort = METHOD_BARRIER.presentValue(BARRIER_SHORT, SMILE_MULTICURVES);
    assertEquals(
        "Barriers are cheaper than vanilla",
        -priceBarrier.getAmount(USD).getAmount(),
        priceBarrierShort.getAmount(USD).getAmount(),
        TOLERANCE_PV);
  }
  ...
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = METHOD_BARRIER.presentValue(OPTION_BARRIER, SMILE_MULTICURVES);
    final MultiCurrencyAmount pvCalculator = OPTION_BARRIER.accept(PVFBC, SMILE_MULTICURVES);
    assertEquals(
        "Forex vanilla option: present value Method vs Calculator",
        pvMethod.getAmount(USD).getAmount(),
        pvCalculator.getAmount(USD).getAmount(),
        TOLERANCE_PV);
  }
  ...
  public void currencyExposure() {
    final MultiCurrencyAmount ce = METHOD_BARRIER.currencyExposure(OPTION_BARRIER, SMILE_FLAT_MULTICURVES);
    final double shiftSpotEURUSD = 1E-6;
    final MultiCurrencyAmount pv = METHOD_BARRIER.presentValue(OPTION_BARRIER, SMILE_FLAT_MULTICURVES);
    final FxMatrix fxMatrixShift = FxMatrix.builder().addRate(EUR, USD, SPOT + shiftSpotEURUSD).build();
    ...
    final MultiCurrencyAmount pvBumpedSpot = METHOD_BARRIER.presentValue(OPTION_BARRIER, smileBumpedSpot);
    final double ceDomesticFD = (pvBumpedSpot.getAmount(USD).getAmount() - pv.getAmount(USD).getAmount());
    assertEquals("Barrier currency exposure: domestic currency", ceDomesticFD, ce.getAmount(EUR).getAmount() * shiftSpotEURUSD, 2.0E-4);
    ...
    final double pvInGBPBeforeShift = pv.getAmount(USD).getAmount() / spotGBPUSD;
    final double pvInGBPAfterShift = pvBumpedSpot.getAmount(USD).getAmount() / spotGBPUSDshifted;
    assertEquals(
        "Barrier currency exposure: all currencies",
        pvInGBPAfterShift - pvInGBPBeforeShift,
        ce.getAmount(EUR).getAmount() * (1 / spotGBPEURshifted - 1 / spotGBPEUR) + ce.getAmount(USD).getAmount() * (1 / spotGBPUSDshifted - 1 / spotGBPUSD), 1.0E-4);
  }
  ...
  public void currencyExposureMethodVsCalculator() {
    final MultiCurrencyAmount ceMethod = METHOD_BARRIER.currencyExposure(OPTION_BARRIER, SMILE_MULTICURVES);
    final MultiCurrencyAmount ceCalculator = OPTION_BARRIER.accept(CEFBC, SMILE_MULTICURVES);
    assertThat(ceCalculator.getAmount(EUR)).isEqualTo(ceMethod.getAmount(EUR), offset(TOLERANCE_PV));
    assertThat(ceCalculator.getAmount(USD)).isEqualTo(ceMethod.getAmount(USD), offset(TOLERANCE_PV));
  }
  ...
  public void longShort() {
    final MultiCurrencyAmount pvShort = METHOD_BARRIER.presentValue(BARRIER_SHORT, SMILE_MULTICURVES);
    final MultiCurrencyAmount pvLong = METHOD_BARRIER.presentValue(OPTION_BARRIER, SMILE_MULTICURVES);
    Offset<Double> tol = offset(TOLERANCE_PV);

    assertThat(pvShort.getAmount(USD)).isEqualTo(pvLong.getAmount(USD).negated(), tol);

    final MultiCurrencyAmount ceShort = METHOD_BARRIER.currencyExposure(BARRIER_SHORT, SMILE_MULTICURVES);
    final MultiCurrencyAmount ceLong = METHOD_BARRIER.currencyExposure(OPTION_BARRIER, SMILE_MULTICURVES);
    assertThat(ceShort.getAmount(USD)).isEqualTo(ceLong.getAmount(USD).negated(), tol);
    assertThat(ceShort.getAmount(EUR)).isEqualTo(ceLong.getAmount(EUR).negated(), tol);
