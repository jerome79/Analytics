...
public class ForexForwardPointsMethodTest {
  ...
  public void presentValueDirectOrder() {
    ...
    final MultiCurrencyAmount pvComputed = METHOD_FX_PTS.presentValue(FX, MULTICURVES, FWD_RATES, PAIR);
    assertEquals("ForexForwardPointsMethod: presentValue", 1, pvComputed.size());
    assertEquals("ForexForwardPointsMethod: presentValue", pvExpected, pvComputed.getAmount(CUR_2).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount pvComputed2 = METHOD_FX_PTS.presentValue(FX, MULTICURVES_FWD);
    assertEquals("ForexForwardPointsMethod: presentValue", pvComputed.getAmount(CUR_2).getAmount(), pvComputed2.getAmount(CUR_2).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueOppositeOrder() {
    final MultiCurrencyAmount pvComputedIndirect = METHOD_FX_PTS.presentValue(FX_INDIRECT, MULTICURVES, FWD_RATES, PAIR);
    final MultiCurrencyAmount pvComputedDirect = METHOD_FX_PTS.presentValue(FX, MULTICURVES, FWD_RATES, PAIR);
    assertEquals("ForexForwardPointsMethod: presentValue", 1, pvComputedIndirect.size());
    assertEquals("ForexForwardPointsMethod: presentValue", pvComputedIndirect.getAmount(CUR_2).getAmount(), pvComputedDirect.getAmount(CUR_2).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount pvComputed2 = METHOD_FX_PTS.presentValue(FX_INDIRECT, MULTICURVES_FWD);
    assertEquals("ForexForwardPointsMethod: presentValue", pvComputedIndirect.getAmount(CUR_2).getAmount(), pvComputed2.getAmount(CUR_2).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueMethodVsCalculator() {
    final MultiCurrencyAmount pvMethod = METHOD_FX_PTS.presentValue(FX, MULTICURVES, FWD_RATES, PAIR);
    final MultiCurrencyAmount pvCalculator = FX.accept(PVFFPC, MULTICURVES_FWD);
    assertEquals("ForexForwardPointsMethod: presentValue", pvMethod.getAmount(CUR_2).getAmount(), pvCalculator.getAmount(CUR_2).getAmount(), TOLERANCE_PV);
  }
  ...
  public void currencyExposure() {
    ...
    MultiCurrencyAmount ceExpected = MultiCurrencyAmount.of(CUR_1, ce1);
    ceExpected = ceExpected.plus(CUR_2, ce2);
    final MultiCurrencyAmount ceComputed = METHOD_FX_PTS.currencyExposure(FX, MULTICURVES, FWD_RATES, PAIR);
    assertEquals("ForexForwardPointsMethod: currencyExposure", 2, ceComputed.size());
    assertEquals("ForexForwardPointsMethod: currencyExposure", ceExpected.getAmount(CUR_1).getAmount(), ceComputed.getAmount(CUR_1).getAmount(), TOLERANCE_PV);
    assertEquals("ForexForwardPointsMethod: currencyExposure", ceExpected.getAmount(CUR_2).getAmount(), ceComputed.getAmount(CUR_2).getAmount(), TOLERANCE_PV);
    final MultiCurrencyAmount pvComputed = METHOD_FX_PTS.presentValue(FX, MULTICURVES, FWD_RATES, PAIR);
    assertEquals("ForexForwardPointsMethod: currencyExposure", MULTICURVES.getFxRates().convert(ceComputed, CUR_2).getAmount(), pvComputed.getAmount(CUR_2).getAmount(), TOLERANCE_PV);
  }
  ...
  public void presentValueForwardPointsSensitivity() {
    ...
    final MultiCurrencyAmount pv = METHOD_FX_PTS.presentValue(FX, MULTICURVES, FWD_RATES, PAIR);
    ...
    for (int loopt = 0; loopt < NB_MARKET_QUOTES; loopt++) {
      ...
      final MultiCurrencyAmount pvShift = METHOD_FX_PTS.presentValue(FX, MULTICURVES, fwdPtsShift, PAIR);
      fpsExpected[loopt] = (pvShift.getAmount(CUR_2).getAmount() - pv.getAmount(CUR_2).getAmount()) / shift;
